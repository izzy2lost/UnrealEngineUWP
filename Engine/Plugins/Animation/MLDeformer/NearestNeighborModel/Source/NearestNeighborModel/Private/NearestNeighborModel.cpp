...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateForTraining()
{
	...
	const EOpFlag Result = UpdateVertexMap();
...
void UNearestNeighborModelSection::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (Property->GetFName() == GET_MEMBER_NAME_CHECKED(UNearestNeighborModelSection, VertexMapString))
	{
		InvalidateTraining();
		UpdateVertexMap();
	}
...
UE::NearestNeighborModel::EOpFlag UNearestNeighborModelSection::UpdateVertexMap()
{
	using namespace UE::NearestNeighborModel;
	if (!GetModel())
	{
...
FString UNearestNeighborModelSection::BackwardCompatibleVertexString = TEXT("Saved with backward version");
#endif
