...
bool URigHierarchyController::SelectElement(FRigElementKey InKey, bool bSelect, bool bClearSelection)
{
	...
	if(Hierarchy->ElementKeyRedirector)
	{
		...
		{
			if(const_cast<FCachedRigElement*>(Cache)->UpdateCache(Hierarchy.Get()))
			{
				Key = Cache->GetKey();
			}
		}
			...
			if(Connector->Settings.Type == EConnectorType::Primary)
			{
				if(Hierarchy->HasExecuteContext())
				{
					...
					if(!NameSpace.IsEmpty())
					{
						...
						if(!ConnectorNameSpace.IsNone() && ConnectorNameSpace.ToString().Equals(NameSpace, ESearchCase::CaseSensitive))
						{
	...
	if(Hierarchy->HasExecuteContext())
	{
		...
		if(!CRContext.GetRigModuleNameSpace().IsEmpty())
		{
			if(InElementToAdd->GetName().StartsWith(CRContext.GetRigModuleNameSpace(), ESearchCase::CaseSensitive))
			{
				...
				if(Hierarchy->ElementKeyRedirector)
				{
					Hierarchy->ElementKeyRedirector->Add(FRigElementKey(DesiredName.GetFName(), InElementToAdd->Key.Type), InElementToAdd->Key, Hierarchy.Get());
				}
			}
		}
	}
