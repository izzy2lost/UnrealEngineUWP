...
void FAnimNode_ControlRig_PoseAdapter::UnlinkCurveStorage()
{
	check(Hierarchy);

	const TArray<FRigBaseElement*> CurveElements = Hierarchy->GetCurvesFast();
	for(const FRigBaseElement* CurveElement : CurveElements)
	{
		RestoreCurveStorage(CurveElement->GetKeyAndIndex(), false);
	}
}
...
void FAnimNode_ControlRigBase::UpdateInput(UControlRig* ControlRig, FPoseContext& InOutput)
{
	...
	if (PoseAdapter && InputSettings.bUpdatePose)
	{
		...
		if(InputSettings.bUpdateCurves || OutputSettings.bUpdateCurves)
		{
			...
			if(PoseAdapter->PoseCurveToStorageIndex.Num() != InOutput.Curve.Num())
			{
				PoseAdapter->PoseCurveToStorageIndex.Reset();
				PoseAdapter->PoseCurveToStorageIndex.Reserve(InOutput.Curve.Num());
				InOutput.Curve.ForEachElement([this, Hierarchy](const UE::Anim::FCurveElement& InCurveElement)
				{
					const int32 StorageIndex = Hierarchy->GetIndex({InCurveElement.Name, ERigElementType::Curve});
					PoseAdapter->PoseCurveToStorageIndex.Add(StorageIndex);
				});
			}

			int32 CurveIndex = 0;
			InOutput.Curve.ForEachElement([this, &CurveIndex, Hierarchy](const UE::Anim::FCurveElement& InCurveElement)
			{
				const int32& StorageIndex = PoseAdapter->PoseCurveToStorageIndex[CurveIndex];
				if(StorageIndex != INDEX_NONE)
				{
					if(FRigCurveElement* HierarchyCurve = Hierarchy->Get<FRigCurveElement>(StorageIndex))
					{
...
void FAnimNode_ControlRigBase::UpdateOutput(UControlRig* ControlRig, FPoseContext& InOutput)
{
	...
	if(OutputSettings.bUpdateCurves)
	{
		...
		if(ChangedCurveIndices.Num() > 0)
		{
			...
			PoseAdapter->HierarchyCurveCopied.SetNumUninitialized(HierarchyCurves.Num());
			...
			InOutput.Curve.ForEachElement([this, &CurveIndex, &CurvesCopied, &HierarchyCurves](const UE::Anim::FCurveElement& InCurveElement)
			{
				...
				if(StorageIndex != INDEX_NONE)
				{
					const FRigCurveElement* HierarchyCurve = CastChecked<FRigCurveElement>(HierarchyCurves[StorageIndex]);
					if(HierarchyCurve->IsValueSet())
					{
						const_cast<UE::Anim::FCurveElement*>(&InCurveElement)->Value = HierarchyCurve->Get();
					}
					if(!PoseAdapter->HierarchyCurveCopied[HierarchyCurve->GetSubIndex()])
					{
						PoseAdapter->HierarchyCurveCopied[HierarchyCurve->GetSubIndex()] = true;
						CurvesCopied++;
					}
				}
				CurveIndex++;
			...
			if(CurvesCopied < HierarchyCurves.Num())
			{
				...
				{
					if(!Hierarchy->IsValidIndex(ChangedCurveIndex))
					{
						continue;
					}
					const FRigCurveElement* HierarchyCurve = Hierarchy->Get<FRigCurveElement>(ChangedCurveIndex);
					if(PoseAdapter->HierarchyCurveCopied[HierarchyCurve->GetSubIndex()])
					{
						continue;
					}
				...
				if(InOutput.Curve.Num() > ControlRigCurves.Num())
				{
					InOutput.Curve.Combine(ControlRigCurves);
				}
				else if(InOutput.Curve.Num() == 0)
				{
					InOutput.Curve = ControlRigCurves;
				}
				else
				{
					Swap(InOutput.Curve, ControlRigCurves);
					InOutput.Curve.Combine(ControlRigCurves);
				}
			}
		}
...
void FAnimNode_ControlRigBase::UpdateInputOutputMappingIfRequired(UControlRig* InControlRig, const FBoneContainer& InRequiredBones)
{
	...
	PoseAdapter->PoseCurveToStorageIndex.Reset();
	PoseAdapter->HierarchyCurveCopied.Reset();
