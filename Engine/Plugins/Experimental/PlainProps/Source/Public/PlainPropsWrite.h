...
{
public:
	PLAINPROPS_API FWriter(const FIdIndexerBase& DeclaredIds, const FBuiltSchemas& InSchemas);
	PLAINPROPS_API ~FWriter();
	
	PLAINPROPS_API bool								Uses(FNameId BuiltId) const;
	PLAINPROPS_API TConstArrayView<FNestedScope>	GetUsedScopes() const;
	PLAINPROPS_API TConstArrayView<FParametricType>	GetUsedParametrics() const;
	...
	PLAINPROPS_API void								WriteMembers(TArray64<uint8>& Out, FStructSchemaId BuiltId, const FBuiltStruct& Struct) const;
