...
{ \
	...
	FRegistryTest::Get()->OpAdd(OpDesc, FOperatorTest##OpName##::Create, FOperatorTest##OpName##::Validate); \
	TestOps.Add({OpDesc, FOperatorTest##OpName##::Create, FOperatorTest##OpName##::Validate}); \
} \
...
{ \
	...
	FRegistryTest::Get()->OpAdd(OpDesc, FOperatorTest##OpName##<##OpVer##>::Create, FOperatorTest##OpName##<##OpVer##>::Validate); \
	TestOps.Add({OpDesc, FOperatorTest##OpName##<##OpVer##>::Create, FOperatorTest##OpName##<##OpVer##>::Validate}); \
} \
const int Op##OpName####OpVer##Idx = TestOps.Num() - 1;
	...
	for (int32 Idx = 0; Idx < TestOps.Num(); ++Idx)
	{
		...
		UTEST_NOT_NULL(TEXT("OpFind()"), CreateFunc);
	...
	for (int32 Idx = 0; Idx < TestOps.Num(); ++Idx)
	{
		...
		UTEST_NOT_NULL(TEXT("OpFindValidation()"), ValidateFunc);
    ...
    UTEST_NULL(TEXT("FindUnversionedInClipVersioned"), 
        FRegistryTest::Get()->OpFind({{TEXT("Clip"), TEXT("Onnx")}})
    );
