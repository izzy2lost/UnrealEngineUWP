...
MixerInsightSession::MixerInsightSession() :
...
void MixerInsightSession::LiveCache::AddBlob(RecordID RecId, BlobPtr BlobObj)
{
	if (RecId.IsBlob())
	{
...
BlobPtr MixerInsightSession::LiveCache::GetBlob(RecordID RecId) const
{
...
void MixerInsightSession::LiveCache::AddBatch(RecordID RecId, JobBatchPtr Batch)
{
	if (RecId.IsBatch())
	{
...
JobBatchPtr MixerInsightSession::LiveCache::GetBatch(RecordID RecId) const
{
...
void MixerInsightSession::LiveCache::AddMix(RecordID RecId, MixPtr MixObj)
{
	if (RecId.IsMix())
	{
...
MixPtr MixerInsightSession::LiveCache::GetMix(RecordID RecId) const
{
...
DeviceBufferPtr MixerInsightSession::LiveCache::GetDeviceBuffer(RecordID RecId) const
{
	if (RecId.IsBuffer())
	{
		...
		if (!Buffer) 
		{
			...
			{
				UE_LOG(LogMixerInsight, Log, TEXT("MixerInsightActionManagerObserver::GetDeviceBuffer find the buffer from hash but no buffer"));
			}
		}
		...
		if (!Buffer) {
			if (BufferRecord.PrevHashValue != 0)
			{
				...
				{
					UE_LOG(LogMixerInsight, Log, TEXT("MixerInsightActionManagerObserver::GetDeviceBuffer find the buffer from prevHash but no buffer"));
				}
			}
			else
			{
				UE_LOG(LogMixerInsight, Log, TEXT("MixerInsightActionManagerObserver::GetDeviceBuffer cannot find the buffer in the cache, missing second hash"));
			}
		}
...
void MixerInsightSession::EmitOnEngineReset(bool createOrDestroy)
{
...
uint32 MixerInsightSession::ParseDeviceBuffersCache(DeviceType DevType, RecordIDArray& AddedBufferIDs)
{
	...
	if (NumNewBuffers)
	{
		...
		UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::ParseDeviceBuffersCache[%s]: %d new unknown buffers"), *t, NumNewBuffers);
	}
	
	return NumNewBuffers;
}

void MixerInsightSession::DeviceBuffersUpdated(DeviceType DevType, MixerInsightDeviceObserver::HashNDescArray&& AddedBuffers, MixerInsightDeviceObserver::HashArray&& RemovedBuffers)
{
	...
	for (auto& AddedBuffer : AddedBuffers)
	{
		...
		UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::DeviceBuffersAdded : ptr 0x%llx - hash %llu"), key, AddedBuffer.First);
		...
		if (RecId.IsValid())
		{
			if (bAdded)
			{
				UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::DeviceBuffersAdded : %d - hash %llu"), RecId.Buffer(), DevBuffRec.HashValue);
				AddedRecIds.emplace_back(RecId);
			}
			else
			{
				UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::DeviceBuffers Updated hash : %d - hash %llu"), RecId.Buffer(), DevBuffRec.HashValue);
			}
		}
		else
		{
			UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::DeviceBuffersAdded ERROR invalid buffer: ptr 0x%llx - hash %llu"), DevBuffRec.ID, DevBuffRec.HashValue);
		}
	}
	...
	for (int i = 0; i < RemovedBuffers.size(); ++i)
	{
		...
		UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::DeviceBuffersRemoved : ptr 0x%llx - hash %llu - prevhash %llu"), bp, h, ph);
		auto RecId = GetRecord().EraseDeviceBufferRecord(DevType, bp);
		if (RecId.IsValid())
		{
			UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::DeviceBuffersRemoved : %d"), RecId.Buffer());
			RemovedRecIds.emplace_back(RecId);
		}
		else
		{
			UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::DeviceBuffersAdded ERROR invalid buffer ?"));
		}
	}
...
void MixerInsightSession::BlobberHashesUpdated(MixerInsightBlobberObserver::HashArray&& AddedHashes, MixerInsightBlobberObserver::HashArray&& RemappedHashes)
{
...
//		UE_LOG(LogMixerInsight, Log, TEXT("MixerInsightSession::BlobberHashesUpdated::Added hash %llu"), a);
...
//		auto n = FindOrCreateBlobRecordFromHash(a);
	...
	{
		UE_LOG(LogMixerInsight, VeryVerbose, TEXT("MixerInsightSession::BlobberHashesUpdated::Remapped hash old %llu ==> new %llu"), RemappedHashes[2 * i], RemappedHashes[2 * i + 1]);
...
void MixerInsightSession::UpdateIdle()
{
	OnUpdateIdleEvent.Broadcast();
}

void MixerInsightSession::BatchAdded(JobBatchPtr Batch)
{
	UE_LOG(LogMixerInsight, Log, TEXT("MixerInsightSchedulerObserver::BatchAdded"));
}

void MixerInsightSession::BatchDone(JobBatchPtr Batch)
{
	UE_LOG(LogMixerInsight, Log, TEXT("MixerInsightSchedulerObserver::BatchDone"));
...
void MixerInsightSession::BatchJobsDone(JobBatchPtr Batch)
{
	UE_LOG(LogMixerInsight, Log, TEXT("MixerInsightSchedulerObserver::BatchDone"));
...
void MixerInsightSession::SendToInspector(RecordID recordID) const
{
	if (recordID.IsValid())
	{
		if (recordID.IsBatch())
...
BatchRecord MixerInsightSession::MakeBatchRecord(RecordID RecId, JobBatchPtr Batch, bool isDone)
{
...
void MixerInsightSession::UpdateBatchRecord(JobBatchPtr Batch, BatchRecord& record) {
	record.BeginTimeMS = Batch->GetStartTime();
...
JobRecord MixerInsightSession::MakeJobRecord(JobPtrW job, uint32 idx, int32 phaseIdx)
{
	JobRecord record;
...
void MixerInsightSession::UpdateJobRecord(JobPtrW job, JobRecord& record, double batchBeginTime_ms, double batchEndTime_ms)
{
	JobPtr sjob = job.lock();
...
RecordID MixerInsightSession::FindOrCreateBlobRecordFromHash(HashType hash)
{
...
RecordID MixerInsightSession::FindOrCreateBlobRecord(BlobPtr BlobObj, RecordID sourceID)
{
	if (BlobObj)
	{
...
RecordID MixerInsightSession::RemapBlobHash(HashType oldHash, HashType newHash)
{
...
BlobRecord MixerInsightSession::MakeBlobRecord(RecordID RecId, BlobPtr BlobObj, RecordID sourceID)
{
	BlobRecord record;
...
void MixerInsightSession::EditTiledBlobRecord(BlobRecord& rd, const RecordIDTiles& subTiles)
{
...
void  MixerInsightSession::UpdateBlobRecord(RecordID RecId, BlobPtr BlobObj)
{
...
void MixerInsightSession::EmitOnNewBlob()
{
...
void MixerInsightSession::EmitOnMappedBlob()
{
...
bool MixerInsightSession::ReplayBatch(RecordID batchRecordID, bool captureRenderDoc)
{
...
bool MixerInsightSession::ReplayJob(RecordID jobRecordID, bool captureRenderDoc)
{
	// Get to the Batch ptr if still alive:
...
RecordID MixerInsightSession::AssociateBatchToMix(RecordID Batch, MixPtr MixObj)
{
...
RecordID MixerInsightSession::FindOrCreateMixRecord(MixPtr MixObj)
{
	if (MixObj)
	{
