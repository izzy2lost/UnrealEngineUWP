...
class SMixerInsightDeviceListViewRow : public SMultiColumnTableRow<SMixerInsightDeviceListView::FItem>
{
public:
	using FItem = SMixerInsightDeviceListView::FItem;
	using FItemW = SMixerInsightDeviceListView::FItemW;

	SLATE_BEGIN_ARGS(SMixerInsightDeviceListViewRow) {}
...
public:
			...
			else
			{
				return SNew(SBox)
					...
					[
						SAssignNew(_textBoxes[column], STextBlock)
						.Text(GetTextForColumn(column))
						.OnDoubleClicked(this, &SMixerInsightDeviceListViewRow::OnDoubleClickedResultColumn)
					];
			}
	...
	FText GetTextForColumn(Column column) const
	{
		...
		const auto& br = MixerInsight::Instance()->GetSession()->GetRecord().GetBuffer(_recordID);
...
SMixerInsightDeviceListViewRow::ColumnInfo SMixerInsightDeviceListViewRow::s_columnNames[] = {
...
void SMixerInsightDeviceListView::Construct(const FArguments& Args)
{
	...
	for (int i = 0; i < SMixerInsightDeviceListViewRow::NUM_COLUMNS; i++)
	{
		headerRow->AddColumn(
			SHeaderRow::Column(SMixerInsightDeviceListViewRow::s_columnNames[i].first)
			.DefaultLabel(FText::FromString(SMixerInsightDeviceListViewRow::s_columnNames[i].first.ToString()))
			.FillWidth(SMixerInsightDeviceListViewRow::s_columnNames[i].second)
		);
	}
	...
	[
		...
		.OnGenerateRow(this, &SMixerInsightDeviceListView::OnGenerateRowForView)
		.OnGetChildren(this, &SMixerInsightDeviceListView::OnGetChildrenForView)

		.OnMouseButtonDoubleClick(this, &SMixerInsightDeviceListView::OnDoubleClickItemForView)
		.OnMouseButtonClick(this, &SMixerInsightDeviceListView::OnClickItemForView)
		.HeaderRow(headerRow)
	];
	...
	auto sr = StaticCastSharedRef<SMixerInsightDeviceListView>(this->AsShared());
	MixerInsight::Instance()->GetSession()->OnDeviceBufferAdded().AddSP(sr, &SMixerInsightDeviceListView::OnDeviceBufferAdded);
	MixerInsight::Instance()->GetSession()->OnDeviceBufferRemoved().AddSP(sr, &SMixerInsightDeviceListView::OnDeviceBufferRemoved);
	MixerInsight::Instance()->GetSession()->OnEngineReset().AddSP(sr, &SMixerInsightDeviceListView::OnEngineReset);
}

void SMixerInsightDeviceListView::Reset()
{
	for (int i = 0; i < (int)DeviceType::Count; ++i)
	{
		auto deviceType = (DeviceType)i;
		...
		if (MixerInsight::Instance()->GetSession())
		{
			auto deviceBufferIDs = MixerInsight::Instance()->GetSession()->GetRecord().FetchActiveDeviceBufferIDs((DeviceType)i);
...
TSharedRef<ITableRow> SMixerInsightDeviceListView::OnGenerateRowForView(FItem item, const TSharedRef<STableViewBase>& OwnerTable)
{
	return SAssignNew(item->_widget, SMixerInsightDeviceListViewRow, OwnerTable).Item(item);
}

void SMixerInsightDeviceListView::OnClickItemForView(FItem item)
{
	if (_inspectOnSimpleClick)
		OnInspect(item);
}
void SMixerInsightDeviceListView::OnDoubleClickItemForView(FItem item)
{
	OnInspect(item);
}

void SMixerInsightDeviceListView::OnInspect(FItem item)
{
...
void SMixerInsightDeviceListView::OnDeviceBufferAdded(const RecordIDArray& rids)
{
...
void SMixerInsightDeviceListView::OnDeviceBufferRemoved(const RecordIDArray& rids)
{
	for (const auto& rid : rids)
	{
		if (!rid.IsBuffer() || (rid.Buffer() == INVALID_INDEX))
...
void SMixerInsightDeviceListView::OnEngineReset(int id)
{
	Reset();
}

bool operator <(const SMixerInsightDeviceListView::FItem& a, const  SMixerInsightDeviceListView::FItem& b)
{
	return ((uint32) a->_rank) < ((uint32) b->_rank);
}

void SMixerInsightDeviceListView::RefreshRootItems()
{
	for (auto& i: _rootItems)
	{
		...
		{
			const auto& br = MixerInsight::Instance()->GetSession()->GetRecord().GetBuffer(c->_recordID);
			c->_rank = br.Rank;
		}
		i->_children.Sort();
...
void SMixerInsightDeviceView::Construct(const FArguments& Args)
{
...
void SMixerInsightDeviceManagerView::Construct(const FArguments& Args)
{
	// install the observer notifications
	auto sr = StaticCastSharedRef<SMixerInsightDeviceManagerView>(this->AsShared());
	MixerInsight::Instance()->GetSession()->OnUpdateIdle().AddSP(sr, &SMixerInsightDeviceManagerView::OnUpdate);
	MixerInsight::Instance()->GetSession()->OnEngineReset().AddSP(sr, &SMixerInsightDeviceManagerView::OnEngineReset);
}

void SMixerInsightDeviceManagerView::Reset()
{
	_deviceViews.clear();
	...
	if (MixerEngine::GetInstance())
	{
		for (int i = 0; i < (int)DeviceType::Count; ++i)
		{
			...
			if (device)
			{
				...
				vbox->AddSlot()
					[
						SAssignNew(deviceView, SMixerInsightDeviceView)
...
void SMixerInsightDeviceManagerView::OnUpdate()
{
...
void SMixerInsightDeviceManagerView::OnEngineReset(int32 id)
{
	Reset();
}