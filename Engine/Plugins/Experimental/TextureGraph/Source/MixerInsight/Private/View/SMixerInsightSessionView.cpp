...
class SMixerInsightBatchJobViewRow : public SMultiColumnTableRow<SMixerInsightBatchJobView::FItem>
{
public:
	using FItem = SMixerInsightBatchJobView::FItem;

	SLATE_BEGIN_ARGS(SMixerInsightBatchJobViewRow) {}
	...
	virtual TSharedRef<SWidget> GenerateWidgetForColumn(const FName& columnName) override
	{
		...
		if (_recordID.IsValid()) {
			...
			else if (column == Timeline)
			{
				...
				auto& sr = MixerInsight::Instance()->GetSession()->GetRecord();
			...
			else if (column == Name && _recordID.IsBatch())
			{
				return 	SNew(SHorizontalBox)
					...
					[
						...
						.Justification(ETextJustify::Left)
						.ColorAndOpacity(_accentColor)
						.OnDoubleClicked(this, &SMixerInsightBatchJobViewRow::OnDoubleClickedResultColumn)
					];
			}
			else
			{
				return 	SNew(SHorizontalBox)
					...
					[	
						SAssignNew(_textBoxes[column], STextBlock)
						.Text(GetTextForColumn(column))
						.Justification(std::get<2>(SMixerInsightBatchJobViewRow::s_columnNames[column]))
						.ColorAndOpacity(_accentColor)
						.OnDoubleClicked(this, &SMixerInsightBatchJobViewRow::OnDoubleClickedResultColumn)
					];
			}
...
SMixerInsightBatchJobViewRow::ColumnInfo SMixerInsightBatchJobViewRow::s_columnNames[] = {
...
void SMixerInsightBatchJobView::Construct(const FArguments& Args)
{
	_inspectOnSimpleClick = Args._inspectOnSimpleClick;

	TSharedPtr<SHeaderRow> headerRow = SNew(SHeaderRow);

	for (int i = 0; i < SMixerInsightBatchJobViewRow::NUM_COLUMNS; i++)
	{
		headerRow->AddColumn(
			SHeaderRow::Column(std::get<0>(SMixerInsightBatchJobViewRow::s_columnNames[i]))
			.DefaultLabel(FText::FromString(std::get<0>(SMixerInsightBatchJobViewRow::s_columnNames[i]).ToString()))
			.HAlignHeader(EHorizontalAlignment::HAlign_Center)
			.HAlignCell(EHorizontalAlignment::HAlign_Fill)
			.FillWidth(std::get<1>(SMixerInsightBatchJobViewRow::s_columnNames[i]))
		);
	}
	...
	SAssignNew(_treeView, SItemTreeView)
		...
		.OnGenerateRow(this, &SMixerInsightBatchJobView::OnGenerateRowForTree)
		.OnGetChildren(this, &SMixerInsightBatchJobView::OnGetChildrenForView)
		.OnMouseButtonDoubleClick(this, &SMixerInsightBatchJobView::OnDoubleClickItemForTree)
		.OnMouseButtonClick(this, &SMixerInsightBatchJobView::OnClickItemForTree)
		.OnContextMenuOpening(this, &SMixerInsightBatchJobView::OnContextMenuOpeningForTree)
		.HeaderRow(headerRow)
	];
...
TSharedRef<ITableRow> SMixerInsightBatchJobView::OnGenerateRowForTree(FItem item, const TSharedRef<STableViewBase>& OwnerTable)
{
	return SAssignNew(item->_widget, SMixerInsightBatchJobViewRow, OwnerTable).Item(item);
}

void SMixerInsightBatchJobView::OnClickItemForTree(FItem item)
{
...
void SMixerInsightBatchJobView::OnDoubleClickItemForTree(FItem item)
{
...
TSharedPtr<SWidget> SMixerInsightBatchJobView::OnContextMenuOpeningForTree()
{
...
TSharedPtr<SWidget> SMixerInsightBatchJobView::OnContextMenuBatch(FItem batchItem)
{
	...
	menuBuilder.BeginSection("BatchContextMenu");
	{
		...
		{
			FUIAction action = FUIAction(FExecuteAction::CreateRaw(this, &SMixerInsightBatchJobView::OnReplayBatch, batchItem->_recordID, false));
			menuBuilder.AddMenuEntry(FText::FromString(FString(TEXT("Replay Batch"))), FText(), FSlateIcon(), action);
		}
		{
			FUIAction action = FUIAction(FExecuteAction::CreateRaw(this, &SMixerInsightBatchJobView::OnReplayBatch, batchItem->_recordID, true));
...
TSharedPtr<SWidget> SMixerInsightBatchJobView::OnContextMenuJob(FItem jobItem)
{
	const bool closeAfterSelection = true;
	FMenuBuilder menuBuilder(closeAfterSelection, NULL);
	...
	menuBuilder.BeginSection("JobContextMenu");
	{
		...
		// Create action delegate and Add menu entry
		{
			FUIAction action = FUIAction(FExecuteAction::CreateRaw(this, &SMixerInsightBatchJobView::OnReplayJob, jobItem->_recordID, false));
			menuBuilder.AddMenuEntry(FText::FromString(FString(TEXT("Replay Job"))), FText(), FSlateIcon(), action);
		}
		{
			FUIAction action = FUIAction(FExecuteAction::CreateRaw(this, &SMixerInsightBatchJobView::OnReplayJob, jobItem->_recordID, true));
...
void SMixerInsightBatchJobView::OnReplayBatch(RecordID batchId, bool captureRenderDoc)
{
	if (!batchId.IsBatch())
		return;

	// Trigger a replay!
	bool result = MixerInsight::Instance()->GetSession()->ReplayBatch(batchId, captureRenderDoc);

}

void SMixerInsightBatchJobView::OnReplayJob(RecordID jobId, bool captureRenderDoc)
{
	if (!jobId.IsJob())
		return;

	// Trigger a replay!
	bool result = MixerInsight::Instance()->GetSession()->ReplayJob(jobId, captureRenderDoc);

}

void SMixerInsightBatchJobView::OnBatchNew(RecordID batchRecordID)
{

	// Fetch the actual data from the record and add job's items
	const auto& br = MixerInsight::Instance()->GetSession()->GetRecord().GetBatch(batchRecordID);
...
void SMixerInsightBatchJobView::OnBatchUpdate(RecordID batchRecordID)
{
...
void SMixerInsightBatchJobView::OnEngineReset(int32 id)
{
	_rootItems.Empty();
	_treeView->RequestTreeRefresh();
}
...
void SMixerInsightSessionView::Construct(const FArguments& Args)
{
	ChildSlot
	[
		SAssignNew(_batchJobView, SMixerInsightBatchJobView)
		.inspectOnSimpleClick(true)
	];


	// install the observer notifications
	auto sr = StaticCastSharedRef<SMixerInsightBatchJobView>(this->_batchJobView->AsShared());

	MixerInsight::Instance()->GetSession()->OnBatchAdded().AddSP(sr, &SMixerInsightBatchJobView::OnBatchNew);
	MixerInsight::Instance()->GetSession()->OnBatchDone().AddSP(sr, &SMixerInsightBatchJobView::OnBatchUpdate);
	MixerInsight::Instance()->GetSession()->OnBatchJobsDone().AddSP(sr, &SMixerInsightBatchJobView::OnBatchUpdate);
	MixerInsight::Instance()->GetSession()->OnEngineReset().AddSP(sr, &SMixerInsightBatchJobView::OnEngineReset);
}
