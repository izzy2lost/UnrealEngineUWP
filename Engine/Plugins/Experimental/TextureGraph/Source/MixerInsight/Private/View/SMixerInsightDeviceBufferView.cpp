...
void SMixerInsightDeviceBufferView::Construct(const FArguments& InArgs)
{
	...
	if (useBlobId)
	{
		blob = MixerInsight::Instance()->GetSession()->GetCache().GetBlob(blobID);
	}
	...
	{
		buffer = MixerInsight::Instance()->GetSession()->GetCache().GetDeviceBuffer(recordID);
	}
...
void SMixerInsightDeviceBufferView::CreateWidgetBrush(UTexture* blobTexture) {
...
void SMixerInsightDeviceBufferView::MakeWidgetMaterial() {
