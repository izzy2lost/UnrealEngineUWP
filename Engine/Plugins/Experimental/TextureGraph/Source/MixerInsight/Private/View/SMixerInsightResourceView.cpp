...
class SMixerInsightResourceViewRow : public SMultiColumnTableRow<SMixerInsightResourceView::FItem>
{
public:
	using FItem = SMixerInsightResourceView::FItem;

	SLATE_BEGIN_ARGS(SMixerInsightResourceViewRow) {}
...
public:
			...
			else
			{
				return SNew(SBox)
					...
					[
						SAssignNew(_textBoxes[column], STextBlock)
						.Text(GetTextForColumn(column))
					.OnDoubleClicked(this, &SMixerInsightResourceViewRow::OnDoubleClickedResultColumn)
					];
			}
		}
	...
	{
		const auto& br = MixerInsight::Instance()->GetSession()->GetRecord().GetBlob(_recordID);
		...
		case RefCount:
			{
				auto refCount = MixerInsight::Instance()->GetSession()->GetCache().GetBlob(_recordID).use_count();
...
SMixerInsightResourceViewRow::ColumnInfo SMixerInsightResourceViewRow::s_columnNames[] = {
...
void SMixerInsightResourceView::Construct(const FArguments& Args)
{

	TSharedPtr<SHeaderRow> headerRow = SNew(SHeaderRow);

	for (int i = 0; i < SMixerInsightResourceViewRow::NUM_COLUMNS; i++)
	{
		headerRow->AddColumn(
			SHeaderRow::Column(SMixerInsightResourceViewRow::s_columnNames[i].first)
			.DefaultLabel(FText::FromString(SMixerInsightResourceViewRow::s_columnNames[i].first.ToString()))
			.FillWidth(SMixerInsightResourceViewRow::s_columnNames[i].second)
		);
	}
	...
	[
		...
		.OnGenerateRow(this, &SMixerInsightResourceView::OnGenerateRowForView)
		.OnGetChildren(this, &SMixerInsightResourceView::OnGetChildrenForView)

		.OnMouseButtonDoubleClick(this, &SMixerInsightResourceView::OnDoubleClickItemForView)
		.OnMouseButtonClick(this, &SMixerInsightResourceView::OnClickItemForView)
		.HeaderRow(headerRow)
	];

	// install the observer notifications
	auto sr = StaticCastSharedRef<SMixerInsightResourceView>(this->AsShared());
	MixerInsight::Instance()->GetSession()->OnBlobAdded().AddSP(sr, &SMixerInsightResourceView::OnBlobNew);
	MixerInsight::Instance()->GetSession()->OnBlobMapped().AddSP(sr, &SMixerInsightResourceView::OnBlobMapped);
	MixerInsight::Instance()->GetSession()->OnEngineReset().AddSP(sr, &SMixerInsightResourceView::OnEngineReset);

}

TSharedRef<ITableRow> SMixerInsightResourceView::OnGenerateRowForView(FItem item, const TSharedRef<STableViewBase>& OwnerTable)
{
	return SAssignNew(item->_widget, SMixerInsightResourceViewRow, OwnerTable).Item(item);
}

void SMixerInsightResourceView::OnClickItemForView(FItem item)
{
...
void SMixerInsightResourceView::OnDoubleClickItemForView(FItem item)
{
...
void SMixerInsightResourceView::OnBlobNew(const RecordIDArray& rids)
{
	for (auto& rid : rids)
	{
		const auto& br = MixerInsight::Instance()->GetSession()->GetRecord().GetBlob(rid);
...
bool operator ==(const SMixerInsightResourceView::FItem& a, const  RecordID& i)
{
	return a->_recordID.id == i.id;
}

void SMixerInsightResourceView::OnBlobMapped(const RecordIDArray& rids)
{
	RefreshRootItems();
}

		
void SMixerInsightResourceView::RefreshRootItems()
{
...
void SMixerInsightResourceView::OnEngineReset(int id)
{
	_rootItems.Empty();
	_tableView->RequestListRefresh();
}
