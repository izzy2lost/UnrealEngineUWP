...
#include "MixerInsightRecord.h"
#include "MixerInsightObserver.h"
...
class MIXERINSIGHT_API MixerInsightSession
{
public:
	...
	{
	public:
		LiveCache(MixerInsightSession* InSession) : Session(InSession) {}
		MixerInsightSession*		Session = nullptr;
...
private:
protected:
	friend class MixerInsightEngineObserver;
	...
	friend class MixerInsightDeviceObserver;
	uint32							ParseDeviceBuffersCache(DeviceType type, RecordIDArray& addedBufferIds);
	void							DeviceBuffersUpdated(DeviceType DevType, MixerInsightDeviceObserver::HashNDescArray&& AddedBuffers, MixerInsightDeviceObserver::HashArray&& RemovedBuffers);

	/// Manage Blobber update
	friend class MixerInsightBlobberObserver;
	void							BlobberHashesUpdated(MixerInsightBlobberObserver::HashArray&& AddedHashes, MixerInsightBlobberObserver::HashArray&& RemappedHashes);

	friend class MixerInsightSchedulerObserver;
