...
class SMixerInsightDeviceListViewRow; /// Declare the concrete type of widget used for the raws of the view. Defined in the cpp file
class STableViewBase;
class ITableRow;
class STextBlock;

class MIXERINSIGHT_API SMixerInsightDeviceListView : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SMixerInsightDeviceListView)
	...
	public:
		...
		TSharedPtr < SMixerInsightDeviceListViewRow > _widget;
		RecordID	_recordID;
...
inline bool operator ==(const SMixerInsightDeviceListView::FItem& i,  const RecordID& rid) { return i->_recordID.Buffer() == rid.Buffer(); }

class MIXERINSIGHT_API SMixerInsightDeviceView : public SCompoundWidget
{
	SLATE_BEGIN_ARGS(SMixerInsightDeviceView) {}
...
class MIXERINSIGHT_API SMixerInsightDeviceManagerView : public SCompoundWidget
{
	SLATE_BEGIN_ARGS(SMixerInsightDeviceManagerView) {}
	SLATE_END_ARGS()

	void Construct(const FArguments& Args);

	std::vector < TSharedPtr< SMixerInsightDeviceView >> _deviceViews;
