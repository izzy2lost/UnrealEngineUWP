...
class SMixerInsightBatchJobViewRow; /// Declare the concrete type of widget used for the raws of the tree view. Defined in the cpp file
class ITableRow;
class STableViewBase;

class MIXERINSIGHT_API SMixerInsightBatchJobView : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SMixerInsightBatchJobView)
	...
	public:
		...
		TSharedPtr < SMixerInsightBatchJobViewRow > _widget;
		RecordID _recordID;
...
class MIXERINSIGHT_API SMixerInsightSessionView : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SMixerInsightSessionView) {}
	SLATE_END_ARGS()

	void Construct(const FArguments& Args);

	TSharedPtr<SMixerInsightBatchJobView> _batchJobView;
};
