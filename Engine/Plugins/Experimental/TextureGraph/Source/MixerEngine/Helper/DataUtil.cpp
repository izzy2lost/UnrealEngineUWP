	...
	if (TempHashValue != nullptr)
	{
		...
		if (bDidUpdate)
		{
			...
			if (HashValue != DataUtil::GNullHash && OldHash != HashValue && UpdateBlobber)
			{
				...
				MixerEngine::GetBlobber()->UpdateHash(OldHash, shared_from_this());
			}
...
CHashPtr CHash::UpdateHash(CHashPtr NewHash, CHashPtr PrevHash)
{
	...
	if (PrevHash)
	{
		...
		if (PrevHashValue != PrevHash->Value() && !MixerEngine::IsDestroying() && MixerEngine::GetBlobber())
		{
			MixerEngine::GetBlobber()->UpdateHash(PrevHashValue, NewHash);
			NewHash->UpdateLinks();
		}
	}
