...
DECLARE_CYCLE_STAT(TEXT("Blobber_Update"), STAT_Blobber_Update, STATGROUP_MixerEngine);
DECLARE_CYCLE_STAT(TEXT("Blobber_Find"), STAT_Blobber_Find, STATGROUP_MixerEngine);
...
{
	Device* NullDevice = MixerEngine::GetDeviceManager()->GetDevice(DeviceType::Null);
...
{
	check(IsInGameThread());
	check(MixerEngine::IsTestMode()); /// Only allowed in test mode
	...
	for (size_t di = 0; di < MixerEngine::GetDeviceManager()->GetNumDevices(); di++)
	{
		if (MixerEngine::GetDeviceManager()->GetDevice(di))
			MixerEngine::GetDeviceManager()->GetDevice(di)->ClearCache();
	}
}
...
BlobRef Blobber::AddInternal(BlobPtr BlobObj, BlobCacheOptions Options)
{
	...
	if (!BlobObj->IsTiled())
	{
		...
		if (!bIsTransient && !Options.Discard && !Options.NoCacheBatch && !BlobObj->IsTiled())
		{
			...
			{
				BlobHasherServicePtr BlobHasher = MixerEngine::GetScheduler()->GetBlobHasherService().lock();
				if (BlobHasher)
				{
