...
DECLARE_CYCLE_STAT(TEXT("MixerEngine-Update"), STAT_MixerEngine_Update, STATGROUP_MixerEngine);
DEFINE_LOG_CATEGORY(LogMixerEngine);

void FMixerEngineModule::StartupModule()
{
...
bool FMixerEngineModule::MapShaders()
{
	...
	realPluginName = rightSanitized.Replace(TEXT("/Source/MixerEngine"), TEXT(""));
...
FString FMixerEngineModule::GetParentPluginName()
{
	return _pluginName;
}

void FMixerEngineModule::ShutdownModule()
{
	FShaderType::Uninitialize();
}

IMPLEMENT_MODULE(FMixerEngineModule, MixerEngine)

//////////////////////////////////////////////////////////////////////////

MixerEngine* MixerEngine::GInstance = nullptr;
std::atomic<bool> MixerEngine::bGIsEngineDestroying(false);

/// Initialize the observer to default no-op
EngineObserverSourcePtr MixerEngine::GObserverSource = std::make_shared<EngineObserverSource>();

MixerEngine::MixerEngine(bool bInIsTestMode) : 
...
{
	UE_LOG(LogMixerEngine, Log, TEXT("Initialising the MixerEngine!"));
...
void MixerEngine::InitEngineInternal()
{
	...
	RenderDocMgrObj = std::make_unique<MixerEditor::RenderDocManager>();
#endif 
	...
	ErrorReporters.Add(nullptr, std::make_shared<FMixerErrorReporter>());
...
MixerEngine::~MixerEngine()
{
	...
	UE_LOG(LogMixerEngine, Log, TEXT("Destroying the MixerEngine!"));
...
void MixerEngine::FirstRunInit()
{
	check(IsInGameThread());
	UE_LOG(LogMixerEngine, Log, TEXT("Setting MixerEngine to run mode = true!"));
...
void MixerEngine::Create(bool bIsTestMode /* = false */)
{
	check(IsInGameThread());
	...
	GInstance = new MixerEngine(bIsTestMode);
	bGIsEngineDestroying = false;
	GInstance->InitEngineInternal();
}

void MixerEngine::Destroy()
{
	check(IsInGameThread());
...
void MixerEngine::Update(float dt)
{
	check(IsInGameThread());
	...
	SCOPE_CYCLE_COUNTER(STAT_MixerEngine_Update);
	GInstance->FrameId++;
...
void MixerEngine::RegisterObserverSource(const EngineObserverSourcePtr& observerSource)
{
	if (GObserverSource)
		GObserverSource->Destroyed();
...
void MixerEngine::RegisterErrorReporter(const UMixInterface* MixKey, const ErrorReporterPtr& InErrorReporterPtr)
{
	if (GInstance)
	{
