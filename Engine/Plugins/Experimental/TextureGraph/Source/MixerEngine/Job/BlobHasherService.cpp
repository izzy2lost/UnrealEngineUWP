...
AsyncJobResultPtr BlobHasherService::Tick()
{
	...
	while (!MixerEngine::IsDestroying() && NumBlobs < MaxBlobs && Blobs.size() > 0 && Iter != Blobs.end())
	{
	...
	if (NumBlobs > 0)
	{
		...
		for (size_t BlobIndex = 0; !MixerEngine::IsDestroying() && BlobIndex < NumBlobs; BlobIndex++)
		...
		if (!MixerEngine::IsDestroying())
		{
			return cti::when_all(promises.begin(), promises.end()).then([this, BlobObjs, PrevHashes, NumBlobs, TotalCount](std::vector<CHashPtr> hashes) mutable
			{
				for (size_t BlobIndex = 0; BlobIndex < NumBlobs; BlobIndex++)
				{
					...
					if (!MixerEngine::IsDestroying())
						MixerEngine::GetBlobber()->UpdateBlobHash(PrevHash, BlobObj);
				}
