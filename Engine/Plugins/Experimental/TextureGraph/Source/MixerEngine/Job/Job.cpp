...
DECLARE_CYCLE_STAT(TEXT("Job_PrepareResources"), STAT_Job_PrepareResources, STATGROUP_MixerEngine);
DECLARE_CYCLE_STAT(TEXT("Job_PrepareTargets"), STAT_Job_PrepareTargets, STATGROUP_MixerEngine);
...
CHashPtr Job::TileHash(int32 TileX, int32 TileY) const
{
	...
	return MixerEngine::GetBlobber()->AddGloballyUniqueHash(CHash::ConstructFromSources(ArgHashes));
}
...
bool Job::CheckCached()
{
	...
	TiledBlobRef ExistingResult = MixerEngine::GetBlobber()->FindTiled(Hash()->Value());
	if (ExistingResult)
	{
...
bool Job::CheckCulled(JobRunInfo InRunInfo)
{
	...
	for (size_t TileX = 0; TileX < ResultPromise->Rows(); TileX++)
	{
		for (size_t TileY = 0; TileY < ResultPromise->Cols(); TileY++)
		{
			...
			BlobRef TileBlob = MixerEngine::GetBlobber()->FindSingle(ResultTileHash->Value());
...
AsyncPrepareResult Job::PrepareTargets(JobBatch* Batch)
{
	...
	for (size_t TileX = 0; TileX < ResultPromise->Rows(); TileX++)
	{
		...
		{
			CHashPtr ResultTileHash = TileHash(TileX, TileY);
			BlobRef TileBlob = !(Batch->IsNoCache() || bIsNoCache) ? MixerEngine::GetBlobber()->FindSingle(ResultTileHash->Value()) : BlobRef();
			...
			if (!TileBlob || !bCanHandleTiles)
			{
				...
				TileBlob = MixerEngine::GetBlobber()->AddResult(TileHashCopy, TileBlobNew, CacheOpt);
			}
			else
			{
...
void Job::AddResultToBlobber()
{
	if (ResultOrg && !IsCulled())
	{
		...
		check(!MixerEngine::Blobber()->IsBlobReferenced(ResultOrg.get()));
#endif 

		Result = MixerEngine::GetBlobber()->AddTiledResult(TempHash, ResultOrg, CacheOpt);
		ResultOrg = nullptr;
