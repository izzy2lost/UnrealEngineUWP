...
AsyncJobResultPtr ThumbnailsService::Tick()
{
	...
	MixerEngine::GetInstance()->GetScheduler()->GetObserverSource()->BatchAdded(CurrentBatch); // notify observer
	...
	{
		MixerEngine::GetInstance()->GetScheduler()->GetObserverSource()->BatchJobsDone(CurrentBatch);
	})
 	.then([this, CurrentBatch]()
 	{
 		MixerEngine::GetInstance()->GetScheduler()->GetObserverSource()->BatchDone(CurrentBatch); // notify observer
