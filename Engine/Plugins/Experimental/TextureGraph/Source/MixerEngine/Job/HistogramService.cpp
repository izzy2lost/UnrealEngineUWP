...
AsyncJobResultPtr HistogramService::Tick()
{
	...
	if (Batch)
	{
		...
		//MixerEngine::GetScheduler()->AddBatch(LastBatch);

		MixerEngine::GetInstance()->GetScheduler()->GetObserverSource()->BatchAdded(LastBatch); // notify observer
		...
		return LastBatch->Exec([=](JobBatch*)	/// Instead of passing it as an argument, JobBatch should be a return type; this is to keep it from cyclic dependancy
			{
				MixerEngine::GetInstance()->GetScheduler()->GetObserverSource()->BatchJobsDone(LastBatch);
			})
			.then([this, LastBatch]()
			{
				MixerEngine::GetInstance()->GetScheduler()->GetObserverSource()->BatchDone(LastBatch); // notify observer
