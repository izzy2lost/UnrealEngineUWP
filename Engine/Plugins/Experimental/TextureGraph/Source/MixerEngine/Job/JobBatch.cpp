...
DECLARE_CYCLE_STAT(TEXT("JobBatch_Exec"), STAT_JobBatch_Exec, STATGROUP_MixerEngine);
...
JobBatch::JobBatch(const FInvalidationDetails& Details) 
	: FrameId(MixerEngine::GetFrameId())
...
AsyncJobResultPtr JobBatch::BeginQueue_Native(JobPriorityQueue& InQueue)
{
	...
	RunInfoDefault.JobScheduler = MixerEngine::GetScheduler();
