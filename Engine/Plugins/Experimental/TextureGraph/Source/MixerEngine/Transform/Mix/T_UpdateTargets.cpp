...
using namespace MixerEditor;

DECLARE_CYCLE_STAT(TEXT("T_UpdateTargets_PrepareResources"), STAT_UpdateTargets_PrepareResources, STATGROUP_MixerEngine);
DECLARE_CYCLE_STAT(TEXT("T_UpdateTargets_Create"), STAT_UpdateTargets_Create, STATGROUP_MixerEngine);
...
AsyncTransformResultPtr T_UpdateTargets::Exec(const TransformArgs& Args)
{
	...
	if (Args.Cycle->GetBatch() && Args.Cycle->GetBatch()->IsCaptureRenderDoc())
		MixerEngine::GetRenderDocManager()->EndCapture();
