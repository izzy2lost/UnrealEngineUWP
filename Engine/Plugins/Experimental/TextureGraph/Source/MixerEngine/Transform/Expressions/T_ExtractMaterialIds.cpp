...
cti::continuable<int32> Job_ExtractMaterialIds::PreExecAsync(ENamedThreads::Type execThread, ENamedThreads::Type returnThread)
{
	...
	BlobRef CachedResult = MixerEngine::GetBlobber()->Find(HashPtr->Value());
		...
		.then([this](RawBufferPtr BufferPtr)
		{
			...
			if(Pixels.Num() > 0)
			{
				...
				if(UniqueColors.Num() > 0)
				{
					...
					Device* Device = MixerEngine::GetDeviceManager()->GetDevice(DeviceType::Mem);
					
					auto Blob = MixerEngine::GetBlobber()->Create(Device, RawBufferPtr);
					
					auto JobHash = Job::Hash();
					auto FinalBlob = MixerEngine::GetBlobber()->AddResult(JobHash, Blob);
					
					// Record the loaded blob under the job JobHash so it is cached
					MixerEngine::GetBlobber()->UpdateBlobHash(JobHash->Value(), Blob);
