...
{
	ThumbnailsServicePtr SvcThumbnail = MixerEngine::GetScheduler()->GetThumbnailsService().lock();
	if (!SvcThumbnail)
		return nullptr;
...
TiledBlobRef T_Thumbnail::Bind(UMixInterface* Mix, UObject* Model, TiledBlobPtr InBlobToBind, int32 InTargetId)
{
	...
	//RenderMaterial_ThumbPtr MatThumb = MixerEngine::GetMaterialManager()->CreateMaterial_Thumbnail(TEXT("T_Thumbnail"), TEXT("Util/CopyUnlit"));
