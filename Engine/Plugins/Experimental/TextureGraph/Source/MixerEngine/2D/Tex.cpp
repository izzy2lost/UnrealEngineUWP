...
DECLARE_CYCLE_STAT(TEXT("Tex_InitRT"), STAT_Tex_InitRT, STATGROUP_MixerEngine);
...
AsyncTiledBlobRef Tex::ToSingleBlob(CHashPtr Hash, bool TransferToRT /* = false */, bool ResolveOnRenderThread /* = false */, bool NoCache /* = false */)
{
	...
	return Device_FX::Get()->Use()
		.then([=, this]() mutable
		{
			...
			HashObj = MixerEngine::GetBlobber()->AddGloballyUniqueHash(Buffer->Hash());

			const BlobRef BlobObj = MixerEngine::GetBlobber()->Create(Buffer, NoCache);
		...
		.then([=, this]() mutable
		{
			...
			for (int32 TileX = 0; TileX < XTiles; TileX++)
			{
				for (int32 TileY = 0; TileY < YTiles; TileY++)
				{
					...
					MixerEngine::GetBlobber()->AddGloballyUniqueHash(TileHash);
					...
					ResultTiles[TileX][TileY] = MixerEngine::GetBlobber()->AddResult(TileHash, std::move(Tile));
				}
			}
			...
			{
				MixerEngine::GetBlobber()->AddResult(HashObj, TiledBlobObj);
			}
			
			auto TiledBlobRef = MixerEngine::GetBlobber()->AddTiledResult(TiledBlobObj);
...
RenderMaterialPtr Tex::GetDefaultMaterial()
{
	...
	CopyMat = MixerEngine::GetMaterialManager()->CreateMaterialOfType_FX<Fx_FullScreenCopy>(TEXT("Tex::FullScreenCopy"));
	check(CopyMat);
