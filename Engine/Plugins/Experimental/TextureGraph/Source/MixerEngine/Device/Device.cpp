...
void Device::CollectBuffer(DeviceBuffer* Buffer)
{
	...
	if (!MixerEngine::IsDestroying() && !Buffer->IsNull())
	{
...
void Device::Collect(DeviceBuffer* Buffer)
{
	...
	DeviceTransferServicePtr transferSvc = MixerEngine::GetScheduler()->GetDeviceTransferService().lock();
	transferSvc->AbortTransfer(Hash->Value());

	BlobRef blob = MixerEngine::GetBlobber()->Find(Hash->Value());
	...
	/// Over here we evaluate Dev transfers
	{
		...
		while (DidRemove && Iter != LRU.end())
		{
				...
				if (DidRemove)
				{
					...
					if (NextDevice)
					{
						if (NextDevice != Device_Null::Get())
						{
							...
							BlobRef BlobObj = MixerEngine::GetBlobber()->Find(BufferHash);
			...
			if (bShouldCollect && Buffer)
			{
				...
				MixerEngine::GetBlobber()->Remove(Buffer->Hash()->Value());
			}
		}
	}
