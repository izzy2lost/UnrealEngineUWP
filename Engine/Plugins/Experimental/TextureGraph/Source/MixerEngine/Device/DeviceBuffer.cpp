...
AsyncRawBufferPtr DeviceBuffer::Raw()
{
	...
	return GetOwnerDevice()->Use()
		.then([this](int32) mutable
		{
			...
			/// Add to blobber
			NewHash = MixerEngine::GetBlobber()->AddGloballyUniqueHash(NewHash);
...
Device* DeviceBuffer::GetDowngradeDevice() const
{
	...
	{
		if (Chain[ChainIndex])
			return MixerEngine::GetDeviceManager()->GetDevice((DeviceType)ChainIndex);
	}
...
Device* DeviceBuffer::GetUpgradeDevice() const
{
	...
	{
		if (Chain[ci])
			return MixerEngine::GetDeviceManager()->GetDevice((DeviceType)ci);
	}
...
DeviceBufferRef::DeviceBufferRef(DeviceBufferPtr RHS) : Buffer(RHS) 
{
	...
	if (Buffer)
	{
		...
		{
			BlobRef BlobRef = MixerEngine::GetBlobber()->FindSingle(NewHash->Value());
