...
#define LOCTEXT_NAMESPACE "STG_GraphActionMenu"
...
FString STG_GraphActionMenu::LastUsedFilterText;

void STG_GraphActionMenu::Construct(const FArguments& InArgs, bool bIsReadOnly/* = true*/)
{
	...
	TreeView = SNew(STreeView< TSharedPtr<FGraphActionNode> >)
		...
		.OnGenerateRow(this, &STG_GraphActionMenu::GenerateChildForTree, bIsReadOnly)
		.OnSelectionChanged(this, &STG_GraphActionMenu::OnItemSelected)
		.OnMouseButtonDoubleClick(this, &STG_GraphActionMenu::OnItemDoubleClicked)
		.OnContextMenuOpening(InArgs._OnContextMenuOpening)
		.OnGetChildren(this, &STG_GraphActionMenu::OnGetChildrenForCategory)
		.SelectionMode(ESelectionMode::Single)
		.OnSetExpansionRecursive(this, &STG_GraphActionMenu::OnSetExpansionRecursive)
	...
	this->ChildSlot
		[
			...
			[
				...
				.OnTextChanged(this, &STG_GraphActionMenu::OnFilterTextChanged)
				.OnTextCommitted(this, &STG_GraphActionMenu::OnFilterTextCommitted)
	...
	FilterTextBox->SetOnKeyDownHandler(FOnKeyDown::CreateSP(this, &STG_GraphActionMenu::OnKeyDown));
...
void STG_GraphActionMenu::RefreshAllActions(bool bPreserveExpansion, bool bHandleOnSelectionEvent/*=true*/)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(STG_GraphActionMenu::RefreshAllActions);
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(STG_GraphActionMenu::CollectAllActions);
...
void STG_GraphActionMenu::GetSectionExpansion(TMap<int32, bool>& SectionExpansion) const
{

}

void STG_GraphActionMenu::SetSectionExpansion(const TMap<int32, bool>& InSectionExpansion)
{
...
void STG_GraphActionMenu::GetSelectedActions(TArray< TSharedPtr<FEdGraphSchemaAction> >& OutSelectedActions) const
{
...
void STG_GraphActionMenu::OnRequestRenameOnActionNode()
{
	TArray< TSharedPtr<FGraphActionNode> > SelectedNodes = GetSelectedItems();
	if (SelectedNodes.Num() > 0)
	{
...
bool STG_GraphActionMenu::CanRequestRenameOnActionNode() const
{
	TArray< TSharedPtr<FGraphActionNode> > SelectedNodes = GetSelectedItems();
...
FString STG_GraphActionMenu::GetSelectedCategoryName() const
{
	TArray< TSharedPtr<FGraphActionNode> > SelectedNodes = GetSelectedItems();
	return (SelectedNodes.Num() > 0) ? SelectedNodes[0]->GetDisplayName().ToString() : FString();
}

void STG_GraphActionMenu::GetSelectedCategorySubActions(TArray<TSharedPtr<FEdGraphSchemaAction>>& OutActions) const
{
	TArray< TSharedPtr<FGraphActionNode> > SelectedNodes = GetSelectedItems();
...
void STG_GraphActionMenu::GetCategorySubActions(TWeakPtr<FGraphActionNode> InAction, TArray<TSharedPtr<FEdGraphSchemaAction>>& OutActions) const
{
	if (InAction.IsValid())
	{
...
bool STG_GraphActionMenu::SelectItemByName(const FName& ItemName, ESelectInfo::Type SelectInfo, int32 SectionId/* = INDEX_NONE */, bool bIsCategory/* = false*/)
{
...
void STG_GraphActionMenu::ExpandCategory(const FText& CategoryName)
{
...
void STG_GraphActionMenu::UpdateForNewActions(int32 IdxStart)
{
...
void STG_GraphActionMenu::GenerateFilteredItems(bool bPreserveExpansion)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(STG_GraphActionMenu::GenerateFilteredItems);
...
bool STG_GraphActionMenu::ShouldExpandNodes() const
{
...
bool STG_GraphActionMenu::CanRenameNode(TWeakPtr<FGraphActionNode> InNode) const
{
	return !OnCanRenameSelectedAction.Execute(InNode);
}

void STG_GraphActionMenu::OnFilterTextChanged(const FText& InFilterText)
{
...
void STG_GraphActionMenu::OnFilterTextCommitted(const FText& InText, ETextCommit::Type CommitInfo)
{
...
bool STG_GraphActionMenu::TryToSpawnActiveSuggestion()
{
...
void STG_GraphActionMenu::OnGetChildrenForCategory(TSharedPtr<FGraphActionNode> InItem, TArray< TSharedPtr<FGraphActionNode> >& OutChildren)
{
	//Intentionally leaving it unimplemented as we do not want the children to spawn for tree
}

TSharedRef<ITableRow> STG_GraphActionMenu::MakeWidget(TSharedPtr<FGraphActionNode> InItem, const TSharedRef<STableViewBase>& OwnerTable, bool bIsReadOnly, bool bMakeTiles)
{
	...
	else
	{
		if (bMakeTiles)
		{
			...
			.OnDragDetected(this, &STG_GraphActionMenu::OnItemDragDetected)
	...
	if (InItem->IsActionNode())
	{
		...
		CreateData.HighlightText = TAttribute<FText>(this, &STG_GraphActionMenu::GetFilterText);
		CreateData.MouseButtonDownDelegate = FCreateWidgetMouseButtonDown::CreateSP(this, &STG_GraphActionMenu::OnMouseButtonDownEvent);
	...
	else if (InItem->IsCategoryNode())
	{
		...
		{
			ReadOnlyArgument.IsReadOnly(this, &STG_GraphActionMenu::CanRenameNode, WeakItem);
		}

		TSharedRef<STG_GraphActionCategoryWidget> CategoryWidget =
			SNew(STG_GraphActionCategoryWidget, InItem)
			.HighlightText(this, &STG_GraphActionMenu::GetFilterText)
		...
		auto TileView = SNew(STileView<TSharedPtr<FGraphActionNode>>)
			...
			.OnGenerateTile(this, &STG_GraphActionMenu::GenerateTileForChildItem)
			.OnSelectionChanged(this, &STG_GraphActionMenu::OnTileItemSelected)
			.ItemWidth(100)
			.ItemHeight(140)
		...
		auto ChildWidget = SNew(SBorder)
			...
			.Padding(FMargin(20.0f, 2.0f, 5.0f, 2.0f))
			.Visibility_Lambda([=]() {
			...
			[
				...
				[
					...
					[
						SNew(STextBlock)
						.Text(FText::FromString(ChildCount))
...
TSharedRef<ITableRow> STG_GraphActionMenu::GenerateChildForTree(TSharedPtr<FGraphActionNode> InItem, const TSharedRef<STableViewBase>& OwnerTable, bool bIsReadOnly)
{
	return MakeWidget(InItem, OwnerTable, bIsReadOnly, false);
}

TSharedRef<ITableRow> STG_GraphActionMenu::GenerateTileForChildItem(TSharedPtr<FGraphActionNode> InItem, const TSharedRef<STableViewBase>& OwnerTable)
{
	return MakeWidget(InItem, OwnerTable, false, true);
}

FText STG_GraphActionMenu::GetFilterText() const
{
...
void STG_GraphActionMenu::OnItemSelected(TSharedPtr< FGraphActionNode > InSelectedItem, ESelectInfo::Type SelectInfo)
{
	if (!bIgnoreUIUpdate)
	{
...
void STG_GraphActionMenu::OnTileItemSelected(TSharedPtr< FGraphActionNode > InSelectedItem, ESelectInfo::Type SelectInfo)
{
	if (!bIgnoreUIUpdate)
	{
...
TArray< TSharedPtr<FGraphActionNode> > STG_GraphActionMenu::GetSelectedItems() const
{
...
void STG_GraphActionMenu::OnItemDoubleClicked(TSharedPtr< FGraphActionNode > InClickedItem)
{
...
FReply STG_GraphActionMenu::OnItemDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
bool STG_GraphActionMenu::OnMouseButtonDownEvent(TWeakPtr<FEdGraphSchemaAction> InAction)
{
	bool bResult = false;
...
FReply STG_GraphActionMenu::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& KeyEvent)
{
...
void STG_GraphActionMenu::MarkActiveSuggestion()
{
...
void STG_GraphActionMenu::AddReferencedObjects(FReferenceCollector& Collector)
{
...
FString STG_GraphActionMenu::GetReferencerName() const
{
	return TEXT("STG_GraphActionMenu");
}

bool STG_GraphActionMenu::HandleSelection(TSharedPtr< FGraphActionNode >& InSelectedItem, ESelectInfo::Type InSelectionType)
{
	bool bResult = false;
...
void STG_GraphActionMenu::OnSetExpansionRecursive(TSharedPtr<FGraphActionNode> InTreeNode, bool bInIsItemExpanded)
{
...
STG_GraphActionMenu::FScoreResults STG_GraphActionMenu::ScoreAndAddActions(int32 StartingIndex)
{
...
void STG_GraphActionMenu::UpdateActiveSelection(STG_GraphActionMenu::FScoreResults ForResults)
{
