...
{
	UI_COMMAND(RunGraph, "Run", "Run the current graph.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(LogGraph, "Log", "Log the current graph.", EUserInterfaceActionType::Button, FInputChord());
	UI_COMMAND(ExportAsUAsset, "Export", "Export all the mix textures as UAssets", EUserInterfaceActionType::Button, FInputChord());
