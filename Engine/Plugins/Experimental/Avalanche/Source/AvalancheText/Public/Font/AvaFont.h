...
struct AVALANCHETEXT_API FAvaFont
{
	...
	/**
	 * Returns a formatted string representing a FAvalancheFont for the specified FontObjectPathName and FontName
	 * Useful e.g. when setting the value of multiple FAvalancheFonts using IPropertyHandle->SetPerObjectValues
	 * @param InFontName {The name of the font handled by UAvalancheFontObject}
	 * @param InFontObjectPathName {The path of the UAvalancheFontObject}
	 * @return {the formatted string}
	 */
	static FString GenerateAvalancheFontFormattedString(const FString& InFontName, const FString& InFontObjectPathName);

	/**
	 * Returns a formatted string representing a FAvalancheFont for the specified UAvalancheFontObject
	 * Useful e.g. when setting the value of multiple FAvalancheFonts using IPropertyHandle->SetPerObjectValues
	 * @param InFontObject {The UAvalancheFontObject used to generated the FAvalancheFont formatted string}
	 * @param OutFormattedString {will contain the formatted string}
	 * @return {true if successful}
	 */
	static bool GenerateAvalancheFontFormattedString(const UAvaFontObject* InFontObject, FString& OutFormattedString);
	...
	/** this FAvalancheFont will be initialized using the provided UAvalancheFontObject */
	...
	/** Returns true if this FAvalancheFont is marked as favorite by the user */
	bool IsFavorite() const;

	/**
	 * Checks if this FAvalancheFont is referencing the default font. This will be happening also in fallback state.
	...
	/**
	 * Checks if this FAvalancheFont is in fallback state.
	 * This happens when the font referenced by this FAvalancheFont is not available after de-serialization. This likely means that a UFont asset is missing.
	 * In fallback state, a FAvalancheFont will return the default font when GetFont() is called.
	 * @return true if this FAvalancheFont is in fallback state
	 */
	...
	/**
	 * Calling this function will null the now deprecated UFont CurrentFont reference, and use that UFont to initialize the currently used UAvalancheFontObject
	 */
	void EnsureUsingCurrentVersion();

	/**
	 * In case this FAvalancheFont is in fallback state, this call can return the name of the missing font asset, if available.
	...
	/**
	 * Returns the currently stored UAvalancheFontObject. This is useful when dealing with font load/import.
	 * If only the current UFont is needed, use GetFont().
	 * @return Current font object. 
	 */
	UAvaFontObject* GetFontObject() const { return AvalancheFontObject; }

	/**
	 * Directly sets the current UAvalancheFontObject, and then updates this FAvalancheFont accordingly.
	 * If this FAvalancheFont is from past versions, the internal deprecated CurrentFont field will be set to nullptr.
	 * @param InFontObject {The FontObject which will be assigned to this FAvalancheFont}
	 */
	...
	/**
	 * Called after serialization of FAvalancheFont.
	 * In case of missing font assets, the FAvalancheFont state will be marked as "FallbackFont"
	 */
...
private:
	...
	/**
	 * Deprecated - used to reference the font used by FAvalancheFont.
	 * Older assets still have the font stored in this field. Calling GetFont() will move the font to the currently used AvalancheFontObject property
	 */
	UPROPERTY(Transient, meta=(DeprecatedProperty, DeprecationMessage="AvalancheFontObject is the property currently handling the font."))
	TObjectPtr<UFont> CurrentFont_DEPRECATED;

	/** Stores the UAvalancheFontObject holding all the information about the font currently used by FAvalancheFont */
	UPROPERTY()
	TObjectPtr<UAvaFontObject> AvalancheFontObject;

	/** Updated when AvalancheFontObject is set. Mainly used when loading FAvaFont, to help looking for a font in case the current font is null */
	UPROPERTY()
	FString FontName;

	/** The current font asset state, used internally to track the current state of FAvalancheFont with respect to its referenced font asset */
	EFontAssetState FontAssetState;

	/** FAvalancheFonts can be marked as favorites by the user */
	bool bIsFavorite;
...
/** We need to intercept FAvalancheFont serialization to detect if there's a missing font when loading */
template<> struct TStructOpsTypeTraits<FAvaFont> : public TStructOpsTypeTraitsBase2<FAvaFont>
{
	enum
	{
		/** void PostSerialize(const FArchive& Ar) in FAvalancheFont */
