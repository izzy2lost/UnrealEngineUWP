...
class AVALANCHEMODIFIERS_API UAvaGridArrangeModifier : public UAvaArrangeBaseModifier
{
	GENERATED_BODY()
...
public:
	...
	FIntPoint GetCount() const { return Count; }

	/** Sets the width and height of the grid. */
	UFUNCTION(BlueprintCallable, Category = "Motion Design|Modifiers|GridArrange")
	void SetCount(const FIntPoint& NewCount);

	/** Returns the distance between each horizontal and vertical child. */
	UFUNCTION(BlueprintPure, Category = "Motion Design|Modifiers|GridArrange")
	FVector2D GetSpread() const { return Spread; }

	/** Sets the distance between each horizontal and vertical child. */
	UFUNCTION(BlueprintCallable, Category = "Motion Design|Modifiers|GridArrange")
	void SetSpread(const FVector2D& NewSpread);

	UFUNCTION(BlueprintCallable, Category = "Motion Design|Modifiers|GridArrange")
	void SetStartCorner(EAvaCorner2D InCorner);
	
	UFUNCTION(BlueprintPure, Category = "Motion Design|Modifiers|GridArrange")
	EAvaCorner2D GetStartCorner() const { return StartCorner; }

	UFUNCTION(BlueprintCallable, Category = "Motion Design|Modifiers|GridArrange")
	void SetStartDirection(EAvaGridArrangeDirection InDirection);
	
	UFUNCTION(BlueprintPure, Category = "Motion Design|Modifiers|GridArrange")
	EAvaGridArrangeDirection GetStartDirection() const { return StartDirection; }
