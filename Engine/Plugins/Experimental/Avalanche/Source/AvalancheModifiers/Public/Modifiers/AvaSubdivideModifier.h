...
class AVALANCHEMODIFIERS_API UAvaSubdivideModifier : public UAvaGeometryBaseModifier
{
	GENERATED_BODY()
...
public:
	...
	int32 GetCuts() const { return Cuts; }
	
	UFUNCTION()
	void SetRecomputeNormals(bool bInRecomputeNormals);
	bool GetRecomputeNormals() const { return bRecomputeNormals; }

	UFUNCTION()
	void SetType(EAvaSubdivisionType InType);
	EAvaSubdivisionType GetType() const { return Type; }
