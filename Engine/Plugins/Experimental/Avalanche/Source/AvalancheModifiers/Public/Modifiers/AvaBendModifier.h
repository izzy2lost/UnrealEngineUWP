...
class AVALANCHEMODIFIERS_API UAvaBendModifier : public UAvaGeometryBaseModifier
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION()
	void SetBendPosition(const FVector& InBendPosition);
	const FVector& GetBendPosition() const { return BendPosition; }

	UFUNCTION()
	void SetBendRotation(const FRotator& InBendRotation);
	const FRotator& GetBendRotation() const { return BendRotation; }

	UFUNCTION()
	void SetSymmetricExtents(bool bInSymmetricExtents);
	bool GetSymmetricExtents() const { return bSymmetricExtents; }

	UFUNCTION()
	void SetBidirectional(bool bInBidirectional);
	bool GetBidirectional() const { return bBidirectional; }
