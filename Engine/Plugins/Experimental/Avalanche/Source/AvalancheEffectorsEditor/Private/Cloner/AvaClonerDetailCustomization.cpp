...
#define LOCTEXT_NAMESPACE "AvaClonerDetailCustomization"

void FAvaClonerDetailCustomization::CustomizeDetails(IDetailLayoutBuilder& InDetailBuilder)
{
	...
	TArray<TWeakObjectPtr<AAvaClonerActor>> SelectedClonersWeak = InDetailBuilder.GetSelectedObjectsOfType<AAvaClonerActor>();
	...
	for (const TWeakObjectPtr<AAvaClonerActor>& ClonerWeak : SelectedClonersWeak)
	{
		const AAvaClonerActor* Cloner = ClonerWeak.Get();
		if (!Cloner)
		{
			continue;
		}
		...
		UAvaClonerLayoutBase* ActiveLayout = Cloner->GetActiveLayout();
		if (!ActiveLayout)
		{
			continue;
		}
	...
	if (!LayoutFunctionNames.IsEmpty())
	{
		...
		for (const TPair<FName, TMap<TWeakObjectPtr<UObject>, TWeakObjectPtr<UFunction>>>& LayoutFunctionNamesPair : LayoutFunctionNames)
		{
			...
			[
				SNew(SButton)
				.Text(ButtonLabel)
				.OnClicked(this, &FAvaClonerDetailCustomization::OnExecuteFunction, LayoutFunctionNamesPair.Key)
			];
		}
	...
	const AAvaClonerActor* Cloner = SelectedClonersWeak[0].Get();
...
FReply FAvaClonerDetailCustomization::OnExecuteFunction(FName InFunctionName)
{
...
void FAvaClonerDetailCustomization::RegisterCustomSections() const
{
	static const FName PropertyEditor("PropertyEditor");
	FPropertyEditorModule& PropertyModule = FModuleManager::GetModuleChecked<FPropertyEditorModule>(PropertyEditor);

	static const FName ClassName = AAvaClonerActor::StaticClass()->GetFName();
