...
#include "AvaClonerLayoutBase.generated.h"
 ...
 */
UCLASS(Abstract, BlueprintType, Within=AvaClonerActor, AutoExpandCategories=("Layout"))
class AVALANCHEEFFECTORS_API UAvaClonerLayoutBase : public UObject
{
	GENERATED_BODY()

public:
	UAvaClonerLayoutBase()
		: UAvaClonerLayoutBase(NAME_None, FString())
	{}
	
	UAvaClonerLayoutBase(const FName& InLayoutName, const FString& InLayoutAssetPath)
	...
	const FAvaClonerEffectorDataInterfaces& GetDataInterfaces() const
	{
	...
	bool CopyTo(UAvaClonerLayoutBase* InOtherLayout) const;
...
protected:
	...
	virtual void OnLayoutParametersChanged(UAvaClonerComponent* InComponent) {}
...
private:
	...
	UPROPERTY(Transient)
	FAvaClonerEffectorDataInterfaces DataInterfaces;
};