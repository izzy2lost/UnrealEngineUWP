...
DEFINE_LOG_CATEGORY_STATIC(LogAvaClonerComponent, Log, All);

#define LOCTEXT_NAMESPACE "AvaEffectorActorVisualizer"

UAvaClonerComponent::UAvaClonerComponent()
	: UNiagaraComponent()
{
	CastShadow = true;
...
void UAvaClonerComponent::PostLoad()
{
	SetAsset(nullptr);
...
void UAvaClonerComponent::UpdateClonerRenderState()
{
	/**
	...
	if (!GetAsset()
		...
		|| !bClonerMeshesDirty
		|| ClonerTree.Status != EAvaClonerAttachmentStatus::Updated)
	{
		return;
	}
...
void UAvaClonerComponent::RefreshUserParameters() const
{
...
void UAvaClonerComponent::OnRenderStateDirty(UActorComponent& InComponent)
{
	const AActor* OwningActor = InComponent.GetOwner();
	const AAvaClonerActor* ClonerActor = GetOuterAAvaClonerActor();
	...
	{
		UE_LOG(LogAvaClonerComponent, Log, TEXT("%s : Detected attachment change for %s"), *ClonerActor->GetActorNameOrLabel(), *OwningActor->GetActorNameOrLabel());
...
void UAvaClonerComponent::UpdateClonerAttachmentTree(bool bInReset)
{
	if (ClonerTree.Status == EAvaClonerAttachmentStatus::Updated)
	{
		ClonerTree.Status = EAvaClonerAttachmentStatus::Outdated;
	}
...
void UAvaClonerComponent::UpdateAttachmentTree()
{
	if (ClonerTree.Status != EAvaClonerAttachmentStatus::Outdated)
	{
		return;
	}
	ClonerTree.Status = EAvaClonerAttachmentStatus::Updating;

	// Invalidate all, to see what is outdated and what is still invalid
	for (TPair<TWeakObjectPtr<AActor>, FAvaClonerAttachmentItem>& AttachmentPair : ClonerTree.ItemAttachmentMap)
	{
		AttachmentPair.Value.Status = EAvaClonerAttachmentStatus::Invalid;
	}
	...
	{
		FAvaClonerAttachmentItem* ClonedItem = ClonerTree.ItemAttachmentMap.Find(ClonedActor);
		...
		if (ClonedItem->Status == EAvaClonerAttachmentStatus::Invalid)
		{
		...
		else if (ClonedItem->Status == EAvaClonerAttachmentStatus::Outdated)
		{
			if (ClonedItem->MeshStatus == EAvaClonerAttachmentStatus::Outdated)
			{
				DirtyItemAttachments.Add(ClonedItem);
				InvalidateBakedStaticMesh(ClonedActor.Get());
			}
			bClonerMeshesDirty = true;
			ClonedItem->Status = EAvaClonerAttachmentStatus::Updated;
		}
	}
	...
	{
		if (const AAvaClonerActor* ClonerActor = GetOuterAAvaClonerActor())
		{
...
void UAvaClonerComponent::UpdateActorAttachment(AActor* InActor, AActor* InParent)
{
	if (!InActor)
	{
		return;
	...
	FAvaClonerAttachmentItem* AttachmentItem = ClonerTree.ItemAttachmentMap.Find(InActor);
	if (AttachmentItem)
	{
		AttachmentItem->Status = EAvaClonerAttachmentStatus::Updated;
		...
		if (AttachmentItem->bRootItem != bIsRoot)
		{
			...
			AttachmentItem->Status = EAvaClonerAttachmentStatus::Outdated;
		}
	...
	{
		AttachmentItem = &ClonerTree.ItemAttachmentMap.Add(InActor, FAvaClonerAttachmentItem());
		...
		AttachmentItem->MeshStatus = EAvaClonerAttachmentStatus::Outdated;
...
void UAvaClonerComponent::BindActorDelegates(AActor* InActor)
{
	if (!InActor)
	{
		return;
	...
	InActor->OnDestroyed.AddUniqueDynamic(this, &UAvaClonerComponent::OnActorDestroyed);
	...
	for (UStaticMeshComponent* StaticMeshComponent : StaticMeshComponents)
	{
		...
		{
			StaticMeshComponent->OnStaticMeshChanged().AddUObject(this, &UAvaClonerComponent::OnMeshChanged, InActor);
		}
	}
#endif
	...
	for (UDynamicMeshComponent* DynamicMeshComponent : DynamicMeshComponents)
	{
		if (!DynamicMeshComponent->OnMeshChanged.IsBoundToObject(this))
		{
			...
			DynamicMeshComponent->OnMeshChanged.AddUObject(this, &UAvaClonerComponent::OnMeshChanged, NullComponent, InActor);
		}
	}
}

void UAvaClonerComponent::UnbindActorDelegates(AActor* InActor) const
{
	if (!InActor)
	{
		return;
...
void UAvaClonerComponent::OnActorDestroyed(AActor* InDestroyedActor)
{
...
#if WITH_EDITOR
void UAvaClonerComponent::OnActorPropertyChanged(UObject* InObject, FPropertyChangedEvent& InPropertyChangedEvent)
{
	OnMaterialChanged(InObject);
}
#endif

void UAvaClonerComponent::OnMaterialChanged(UObject* InObject)
{
	const AActor* ClonerActor = GetOwner();
	...
	{
		if (FAvaClonerAttachmentItem* Item = ClonerTree.ItemAttachmentMap.Find(ActorChanged))
		{
			int32 MatIdx = 0;
			...
			if (bMaterialChanged)
			{
				UE_LOG(LogAvaClonerComponent, Log, TEXT("%s : Detected material change for %s"), *ClonerActor->GetActorNameOrLabel(), *ActorChanged->GetActorNameOrLabel());
				...
				{
					Item->MeshStatus = EAvaClonerAttachmentStatus::Outdated;
				}
...
void UAvaClonerComponent::OnTransformUpdated(USceneComponent* InUpdatedComponent, EUpdateTransformFlags InUpdateTransformFlags, ETeleportType InTeleport)
{
	InvalidateBakedStaticMesh(InUpdatedComponent->GetOwner());
}

void UAvaClonerComponent::OnMeshChanged(UStaticMeshComponent*, AActor* InActor)
{
	if (!InActor)
	{
		return;
	...
	if (FAvaClonerAttachmentItem* Item = ClonerTree.ItemAttachmentMap.Find(InActor))
	{
		UE_LOG(LogAvaClonerComponent, Log, TEXT("%s : Detected mesh change for %s"), *ClonerActor->GetActorNameOrLabel(), *InActor->GetActorNameOrLabel());
...
void UAvaClonerComponent::GetOrderedRootActors(TArray<AActor*>& OutActors) const
{
	const AActor* ClonerActor = GetOwner();
	if (!ClonerActor)
	{
	...
	UWorld* ClonerWorld = ClonerActor->GetTypedOuter<UWorld>();
	if (!ClonerWorld)
	{
		return;
	}
		...
		if (AvaOutliner.IsValid())
		{
			...
			if (OutlinerClonerItem.IsValid())
			{
				for (FAvaOutlinerItemPtr OutlinerChild : OutlinerClonerItem->GetChildren())
				{
					...
					{
						OutActors.Add(OutlinerActor->GetActor());
					}
				}
			}
			return;
	...
	if (const IAvaSceneInterface* SceneInterface = FAvaActorUtils::GetSceneInterfaceFromActor(ClonerActor))
	{
		TArray<AActor*> AttachedActors;
		ClonerActor->GetAttachedActors(AttachedActors, true, false);
		OutActors.Reserve(AttachedActors.Num());
		...
		for (AActor* ChildActor : AttachedActors)
		{
			...
			{
				OutActors.Insert(ChildActor, ChildNode->GetLocalIndex());
			}
		}
	}
	else
	{
		// Fallback outside Avalanche
		ClonerActor->GetAttachedActors(OutActors, true, false);
	}
}

AActor* UAvaClonerComponent::GetRootActor(AActor* InActor) const
{
	if (InActor == nullptr)
	{
		return nullptr;
	}
	if (const FAvaClonerAttachmentItem* Item = ClonerTree.ItemAttachmentMap.Find(InActor))
	{
		if (Item->bRootItem)
		{
			return InActor;
...
void UAvaClonerComponent::InvalidateBakedStaticMesh(AActor* InActor)
{
	if (!InActor)
	{
		return;
	...
	if (const FAvaClonerAttachmentItem* FoundItem = ClonerTree.ItemAttachmentMap.Find(InActor))
	{
...
void UAvaClonerComponent::UpdateDirtyMeshesAsync()
{
	...
	TSet<FAvaClonerAttachmentItem*> UpdateItems = DirtyItemAttachments;
	DirtyItemAttachments.Empty();

	// Update baked dynamic meshes on other thread
	TWeakObjectPtr<UAvaClonerComponent> ThisWeak(this);
	Async(EAsyncExecution::TaskGraph, [ThisWeak, UpdateItems]()->bool
	{
		...
		for (const FAvaClonerAttachmentItem* Item : UpdateItems)
		{
...
void UAvaClonerComponent::OnDirtyMeshesUpdated()
{
...
void UAvaClonerComponent::UpdateActorBakedDynamicMesh(AActor* InActor)
{
	if (!InActor)
	{
		return;
	...
	if (!AttachmentItem || AttachmentItem->MeshStatus != EAvaClonerAttachmentStatus::Outdated)
	{
		return;
	}
	...
	AttachmentItem->MeshStatus = EAvaClonerAttachmentStatus::Updating;

	UE_LOG(LogAvaClonerComponent, Log, TEXT("%s : Updating baked actor mesh %s"), *ClonerActor->GetActorNameOrLabel(), *InActor->GetActorNameOrLabel());
	...
	AttachmentItem->MeshStatus = AttachmentItem->MeshStatus == EAvaClonerAttachmentStatus::Outdated ? EAvaClonerAttachmentStatus::Outdated : EAvaClonerAttachmentStatus::Updated;
	InvalidateBakedStaticMesh(AttachmentItem->ItemActor.Get());
}

void UAvaClonerComponent::UpdateRootActorBakedStaticMesh(AActor* InRootActor)
{
	if (!InRootActor)
	{
		return;
	...
	const FAvaClonerAttachmentItem* RootAttachmentItem = ClonerTree.ItemAttachmentMap.Find(InRootActor);
	...
	UE_LOG(LogAvaClonerComponent, Log, TEXT("%s : Updating root merged baked mesh %s"), *ClonerActor->GetActorNameOrLabel(), *InRootActor->GetActorNameOrLabel());
	...
	TArray<FAvaClonerAttachmentItem*> AttachmentItems;
		...
		](FDynamicMesh3& InMergedMesh)
	{
		...
		{
			FAvaClonerAttachmentItem* AttachmentItem = AttachmentItems[MeshIdx];
			if (!AttachmentItem)
			{
				continue;
			}
...
void UAvaClonerComponent::GetActorAttachmentItems(AActor* InActor, TArray<FAvaClonerAttachmentItem*>& OutAttachmentItems)
{
	if (!InActor)
	{
		return;
...
bool UAvaClonerComponent::IsAllMergedMeshesValid() const
{
...
bool UAvaClonerComponent::IsMaterialDirtyable(const UMaterialInterface* InMaterial) const
{
...
void UAvaClonerComponent::UpdateClonerMeshes()
{
	const AActor* ClonerActor = GetOwner();
	if (!ClonerActor)
	{
	...
	if (!MeshRenderer)
	{
		UE_LOG(LogAvaClonerComponent, Warning, TEXT("%s : Invalid mesh renderer for cloner system"), *ClonerActor->GetActorNameOrLabel());
		return;
	}
	...
	for (int32 Idx = 0; Idx < ClonerTree.MergedBakedMeshes.Num(); Idx++)
	{
		...
		{
			if (const FAvaClonerAttachmentItem* Item = ClonerTree.ItemAttachmentMap.Find(ClonerTree.RootActors[Idx]))
			{
	...
	UE_LOG(LogAvaClonerComponent, Log, TEXT("%s : Cloner mesh updated %i"), *ClonerActor->GetActorNameOrLabel(), ClonerTree.MergedBakedMeshes.Num());

	OnClonerMeshUpdated.Broadcast();
}

bool UAvaClonerComponent::SetClonerActiveLayout(UAvaClonerLayoutBase* InLayout)
{
	if (!InLayout)
	{
		return false;
	...
	if (!InLayout->IsLayoutLoaded())
	{
		...
		{
			UE_LOG(LogAvaClonerComponent, Warning, TEXT("%s : Cloner layout system failed to load %s - %s"), *GetOwner()->GetActorNameOrLabel(), *InLayout->GetLayoutName().ToString(), *InLayout->GetLayoutAssetPath());
			return false;
		}

		UE_LOG(LogAvaClonerComponent, Log, TEXT("%s : Cloner layout system loaded %s - %s"), *GetOwner()->GetActorNameOrLabel(), *InLayout->GetLayoutName().ToString(), *InLayout->GetLayoutAssetPath());
	...
	UE_LOG(LogAvaClonerComponent, Log, TEXT("%s : Cloner layout system changed %s - %s"), *ClonerActor->GetActorNameOrLabel(), *InLayout->GetLayoutName().ToString(), *InLayout->GetLayoutAssetPath());
