...
void FAvaClonerEffectorChannelData::Write(UNiagaraDataChannelWriter* InWriter) const
{
	if (!InWriter)
	{
		return;
...
void FAvaClonerEffectorChannelData::Read(const UNiagaraDataChannelReader* InReader)
{
	if (!InReader)
	{
		return;
	...
	/** Effector */
	bool bIsValid;
	Easing = static_cast<EAvaClonerEasing>(InReader->ReadInt(EasingName, Identifier, bIsValid));
	Mode = static_cast<EAvaClonerEffectorMode>(InReader->ReadInt(ModeName, Identifier, bIsValid));
	Type = static_cast<EAvaClonerEffectorType>(InReader->ReadInt(TypeName, Identifier, bIsValid));
...
FAvaClonerEffectorDataInterfaces::FAvaClonerEffectorDataInterfaces(const UNiagaraSystem* InSystem)
{
	check(InSystem);
	const FNiagaraUserRedirectionParameterStore& UserParameterStore = InSystem->GetExposedParameters();

	static const FNiagaraVariable EffectorIndexDIVar(FNiagaraTypeDefinition(UNiagaraDataInterfaceArrayInt32::StaticClass()), FAvaClonerEffectorDataInterfaces::IndexName);
...
void FAvaClonerEffectorDataInterfaces::CopyTo(FAvaClonerEffectorDataInterfaces& InOther) const
{
...
void FAvaClonerEffectorDataInterfaces::Resize(int32 InSize) const
{
...
void FAvaClonerEffectorDataInterfaces::Remove(int32 InIndex) const
{
...
bool FAvaClonerEffectorDataInterfaces::IsValid() const
{
	for (const TPair<FName, TObjectPtr<UNiagaraDataInterface>>& DataInterfacePair : DataInterfaces)
	{
...
int32 FAvaClonerEffectorDataInterfaces::Num() const
{
...
UNiagaraDataInterfaceArrayInt32* FAvaClonerEffectorDataInterfaces::GetIndexArray() const
{
	if (const TObjectPtr<UNiagaraDataInterface>* Array = DataInterfaces.Find(FAvaClonerEffectorDataInterfaces::IndexName))
	{
