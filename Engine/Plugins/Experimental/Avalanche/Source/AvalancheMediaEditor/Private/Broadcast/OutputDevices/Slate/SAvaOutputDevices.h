...
class FAvaOutputRootItem;
class ITableRow;
class STableViewBase;
template<typename ItemType> class STreeView;

class SAvaOutputDevices : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SAvaOutputDevices){}
	SLATE_END_ARGS()
	
	void Construct(const FArguments& InArgs, const TSharedPtr<FAvaBroadcastEditor>& InBroadcastEditor);
	virtual ~SAvaOutputDevices() override;
...
protected:
	...
	TSharedPtr<FAvaOutputRootItem> RootItem;
};
