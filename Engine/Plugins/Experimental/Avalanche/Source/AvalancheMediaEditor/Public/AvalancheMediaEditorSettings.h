...
#include "AvalancheMediaEditorSettings.generated.h"

UCLASS(config=EditorPerProjectUserSettings, meta=(DisplayName="Playback & Broadcast"))
class AVALANCHEMEDIAEDITOR_API UAvalancheMediaEditorSettings : public UDeveloperSettings
{
	GENERATED_BODY()

public:
	UAvalancheMediaEditorSettings();
	
	static const UAvalancheMediaEditorSettings& Get() {return *GetSingletonInstance();}
	static UAvalancheMediaEditorSettings& GetMutable() {return *GetSingletonInstance();}
...
private:
	static UAvalancheMediaEditorSettings* GetSingletonInstance();
};
