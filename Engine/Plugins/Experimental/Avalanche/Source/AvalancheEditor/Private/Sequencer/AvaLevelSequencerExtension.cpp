...
{
	// If one of these sequences is null (i.e. a ULevelSequence opening, or opening for first time where old is null), re-open the Asset Editor
	if (!InOldSequence || !InNewSequence)
	{
		OpenSequencerWithViewedSequence();	
	}
}
