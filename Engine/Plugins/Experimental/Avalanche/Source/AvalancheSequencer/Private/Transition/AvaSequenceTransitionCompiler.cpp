...
namespace UE::AvaSequencer::Private
{
	...
	void SetInitState(FStateTreeEditorPropertyBindings* InBindings, UStateTreeState& InInitState, const FPlayState& InPlayState)
	{
		InInitState.Modify();
		...
		for (const FStateTreeEditorNode* PlayTaskNode : InPlayState.PlayTaskNodes)
		{
			if (PlayTaskNode)
			{
				...
				TStateTreeEditorNode<FAvaTransitionInitializeSequence>& InitTaskNode = InInitState.AddTask<FAvaTransitionInitializeSequence>();

				FAvaTransitionInitializeSequence::FInstanceDataType& InitTask = InitTaskNode.GetInstanceData();
				...
				if (InBindings)
				{
					InBindings->CopyBindings(PlayTaskNode->ID, InitTaskNode.ID);
				}
			}
		}
	}
...
void FAvaSequenceTransitionCompiler::CompileStatesRecursive(UAvaTransitionTreeEditorData& InEditorData, TArray<TObjectPtr<UStateTreeState>>& InStates)
{
	...
	FStateTreeEditorPropertyBindings* const Bindings = InEditorData.GetPropertyEditorBindings();
	...
	for (int32 StateIndex = 0; StateIndex < WaitStateIndex; ++StateIndex)
	{
		...
		Private::SetInitState(Bindings, State, PlayStates.Pop());
	...
	while (!PlayStates.IsEmpty())
	{
		...
		Private::SetInitState(Bindings, NewState, PlayStates.Pop());
	}
