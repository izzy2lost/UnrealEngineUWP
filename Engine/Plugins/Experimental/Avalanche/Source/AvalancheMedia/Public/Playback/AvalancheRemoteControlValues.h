...
#include "AvalancheRemoteControlValues.generated.h"
 ...
 */
UENUM()
enum class EAvaRemoteControlChanges : uint8
{
	None = 0,
	EntityValues		= 1 << 0,
...
ENUM_CLASS_FLAGS(EAvaRemoteControlChanges);

USTRUCT()
struct AVALANCHEMEDIA_API FAvalancheRemoteControlValue
{
	GENERATED_BODY()

	FAvalancheRemoteControlValue() = default;

	FAvalancheRemoteControlValue(const FString& InValue, bool bInIsDefault = false)
		: Value(InValue), bIsDefault(bInIsDefault) {}

	void SetValueFrom(const FAvalancheRemoteControlValue& InOther)
	{
		Value = InOther.Value;
	}

	/** Returns true if the given value is the same. Ignores the default flag. */
	bool IsSameValueAs(const FAvalancheRemoteControlValue& InOther) const
	{
...
template<>
struct TStructOpsTypeTraits<FAvalancheRemoteControlValue> : public TStructOpsTypeTraitsBase2<FAvalancheRemoteControlValue>
{
	enum
	{
		WithSerializer = true,
...
USTRUCT()
struct AVALANCHEMEDIA_API FAvalancheRemoteControlValues
{
	GENERATED_BODY()

	FAvalancheRemoteControlValues() = default;
	
	/** Value as a binary array of the Remote Control Entity. */
	UPROPERTY()
	TMap<FGuid, FAvalancheRemoteControlValue> EntityValues;

	/** Controller values. */
	UPROPERTY()
	TMap<FGuid, FAvalancheRemoteControlValue> ControllerValues;
	 ...
	 */
	bool HasSameEntityValues(const FAvalancheRemoteControlValues& InOther) const;
	 ...
	 */
	bool HasSameControllerValues(const FAvalancheRemoteControlValues& InOther) const;

	/**
	 *	Removes the extra values compared to the given reference values.
	 * @return flags indicating what changed.
	 */
	EAvaRemoteControlChanges PruneRemoteControlValues(const FAvalancheRemoteControlValues& InReferenceValues);
	 ...
	 * @return flags indicating what changed.
	 */
	EAvaRemoteControlChanges UpdateRemoteControlValues(const FAvalancheRemoteControlValues& InReferenceValues, bool bInUpdateDefaults);
	...
	const FAvalancheRemoteControlValue* GetEntityValue(const FGuid& InId) const { return EntityValues.Find(InId); }
	void SetEntityValue(const FGuid& InId, const FAvalancheRemoteControlValue& InValue) {EntityValues.Add(InId, InValue);}
	...
	const FAvalancheRemoteControlValue* GetControllerValue(const FGuid& InId) const { return ControllerValues.Find(InId); }
	void SetControllerValue(const FGuid& InId, const FAvalancheRemoteControlValue& InValue) {ControllerValues.Add(InId, InValue);}
	 ...
	 */
	bool HasIdCollisions(const FAvalancheRemoteControlValues& InOtherValues) const;
	 ...
	 */
	bool Merge(const FAvalancheRemoteControlValues& InOtherValues);
	
	/** Returns true if the given maps have id collisions. */
	static bool HasIdCollisions(const TMap<FGuid, FAvalancheRemoteControlValue>& InValues, const TMap<FGuid, FAvalancheRemoteControlValue>& InOtherValues);
	
	static const FAvalancheRemoteControlValues& GetDefaultEmpty();
};
