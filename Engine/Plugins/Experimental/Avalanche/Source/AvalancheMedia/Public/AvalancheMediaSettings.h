...
USTRUCT()
struct FLocalPlaybackServerSettings
{
	GENERATED_BODY()
...
class AVALANCHEMEDIA_API UAvalancheMediaSettings : public UDeveloperSettings
{
	GENERATED_BODY()

public:
	UAvalancheMediaSettings();
	
	static const UAvalancheMediaSettings& Get() { return *GetSingletonInstance();}
	static UAvalancheMediaSettings& GetMutable() {return *GetSingletonInstance();}
	...
	UPROPERTY(config, EditAnywhere, Category = "Playback Server")
	FLocalPlaybackServerSettings LocalPlaybackServerSettings;
...
private:
	static UAvalancheMediaSettings* GetSingletonInstance();
};
