...
#include "AvaMediaOutputInfo.h"
#include "CoreMinimal.h"
#include "Engine/TextureRenderTarget2D.h"
#include "Slate/SAvaPlaceholderWidget.h"
#include "Viewport/AvaViewportQualitySettings.h"

#include "AvaOutputChannel.generated.h"
 ...
 */
USTRUCT()
struct AVALANCHEMEDIA_API FAvaMediaCapture
{
	GENERATED_BODY()
...
USTRUCT()
struct AVALANCHEMEDIA_API FAvaOutputChannel
{
	GENERATED_BODY()

public:
	FAvaOutputChannel() = default;
	explicit FAvaOutputChannel(FAvaBroadcastProfile* InProfile) : Profile(InProfile) {}
	~FAvaOutputChannel();
	...
	static FAvaOutputChannel NullChannel;
	
	static void DuplicateChannel(const FAvaOutputChannel& InSourceChannel, FAvaOutputChannel& OutTargetChannel);
	...
	EAvaChannelState RefreshState();
	
	EAvaChannelState GetState() const { return ChannelState; }
	EAvaMediaIssueSeverity GetIssueSeverity() const { return ChannelIssueSeverity; }

	EAvaMediaIssueSeverity GetMediaOutputIssueSeverity(EAvaMediaOutputState InOutputState, const UMediaOutput* InMediaOutput) const;
	const TArray<FString>& GetMediaOutputIssueMessages(const UMediaOutput* InMediaOutput) const;
	EAvaMediaOutputState GetMediaOutputState(const UMediaOutput* InMediaOutput) const;
	const FAvaMediaOutputInfo& GetMediaOutputInfo(const UMediaOutput* InMediaOutput) const;
	FAvaMediaOutputInfo* GetMediaOutputInfoMutable(const UMediaOutput* InMediaOutput);
	
	bool IsEditingEnabled() const;
	
protected:
	void SetState(EAvaChannelState InState, EAvaMediaIssueSeverity InIssueSeverity);
...
public:
	...
	UMediaOutput* AddMediaOutput(const UClass* InMediaOutputClass, const FAvaMediaOutputInfo& InOutputInfo);
	void AddMediaOutput(UMediaOutput* InMediaOutput, const FAvaMediaOutputInfo& InOutputInfo);
	...
	void UpdateRenderTarget(UAvaMediaPlayableGroup* InPlayableGroup, UTextureRenderTarget2D* InRenderTarget);
	void UpdateAudioDevice(const FAudioDeviceHandle& InAudioDeviceHandle);
	UAvaMediaPlayableGroup* GetLastActivePlayableGroup() const;
	...
	DECLARE_MULTICAST_DELEGATE_TwoParams(FOnAvaChannelChanged, const FAvaOutputChannel&, EAvaChannelChange);
	 ...
	 */
	DECLARE_MULTICAST_DELEGATE_TwoParams(FMediaOutputStateChanged, const FAvaOutputChannel& InChannel, const UMediaOutput* InMediaOutput);
...
protected:
	...
	UPROPERTY()
	TArray<FAvaMediaOutputInfo> MediaOutputInfos;
	 ...
	 */
	UPROPERTY(Transient)
	TMap<TObjectPtr<UMediaOutput>, FAvaMediaCapture> MediaCaptures;
	...
	*/
	 ...
	 */
	TWeakObjectPtr<UAvaMediaPlayableGroup> LastActivePlayableGroup;
	...
	TSharedPtr<SAvaPlaceholderWidget> PlaceholderWidget;
	...
	struct FLocalMediaOutputStatus
	{
		...
		EAvaMediaIssueSeverity Severity = EAvaMediaIssueSeverity::None;
	};
	mutable TMap<const UMediaOutput*, FLocalMediaOutputStatus> LocalMediaOutputStatuses;

	FString& EmplaceMessageForOutput(const UMediaOutput* InMediaOutput, EAvaMediaIssueSeverity InSeverity) const;
	EAvaMediaIssueSeverity GetLocalSeverityForOutput(const UMediaOutput* InMediaOutput) const;
