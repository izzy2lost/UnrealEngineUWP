...
/**
 * Cache for the managed Avalanche Blueprint instances.
 *
 * This is LRU cache. 
 */
class AVALANCHEMEDIA_API FAvalancheManagedInstanceCache
{
public:
	FAvalancheManagedInstanceCache();
	virtual ~FAvalancheManagedInstanceCache();
	 ...
	 */
	TSharedPtr<FAvalancheManagedInstance> GetOrLoadAvalancheInstance(const FSoftObjectPath& InAssetPath);
...
private:
	...
	void RemoveEntries(TFunctionRef<bool(const FSoftObjectPath&, const TSharedPtr<FAvalancheManagedInstance>&)> InRemovePredicate, bool bInNotify);
...
private:
	TMap<FSoftObjectPath, TSharedPtr<FAvalancheManagedInstance>> AvalancheInstances;
