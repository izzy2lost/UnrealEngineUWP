...
#include "AvalancheBroadcast.generated.h"

DECLARE_MULTICAST_DELEGATE_OneParam(FOnAvaBroadcastChanged, EAvaBroadcastChange)
DECLARE_MULTICAST_DELEGATE_OneParam(FOnChannelsListChanged, const FAvaBroadcastProfile& /*InProfile*/);
...
class AVALANCHEMEDIA_API UAvalancheBroadcast : public UObject
{
	GENERATED_BODY()
	
public:

	static UAvalancheBroadcast& Get();

	virtual void BeginDestroy() override;
	
	UFUNCTION(BlueprintCallable, Category = "Motion Design Broadcast")
	static UAvalancheBroadcast* GetAvalancheBroadcast();
...
public:
	...
	FOnChannelsListChanged& GetOnChannelsListChanged() { return OnChannelsListChanged; }

protected:

	// Todo: Proposal - make the events static and pass in the UAvalancheBroadcast that caused the event.
	// Reason: Attempt at gradually getting rid of the global UAvalancheBroadcast object. Channel events are also global.
	FOnAvaBroadcastChanged OnBroadcastChanged;
	FOnChannelsListChanged OnChannelsListChanged;
