 ...
 * implemented by TL itself, but it is not atm. It is thus a hard coded rule by the playlist
 ...
 *
 * Note: for now, FAvalanchePageTransitionBuilder is not used for stopping pages. This may change
...
{
public:
	FAvalanchePageTransitionBuilder(UAvalanchePlaylist* InPlaylist) : Playlist(InPlaylist) {}

	~FAvalanchePageTransitionBuilder();
	 ...
	 */
	UAvalanchePageTransition* FindTransition(const UAvalanchePagePlayer* InPlayer) const;
	 ...
	 */
	UAvalanchePageTransition* FindOrAddTransition(const UAvalanchePagePlayer* InPlayer);

private:
	UAvalanchePlaylist* Playlist;

	/** There will be a transition for each channel. */
	TArray<UAvalanchePageTransition*> PageTransitions;
};