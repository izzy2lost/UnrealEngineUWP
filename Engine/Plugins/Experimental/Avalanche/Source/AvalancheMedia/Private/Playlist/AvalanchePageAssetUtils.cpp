...
namespace UE::AvalanchePageAssetUtils::Private
{
	template<typename InInterfaceType>
	const InInterfaceType* GetInterfaceFromAvaAsset(const UObject* InLoadedAsset)
	{
		if (const InInterfaceType* const Interface = Cast<InInterfaceType>(InLoadedAsset))
		{
			return Interface;
		}
		if (const UWorld* const AvalancheWorld = Cast<UWorld>(InLoadedAsset))
		{
			if (const AAvaScene* AvaScene = AAvaScene::GetScene(AvalancheWorld->PersistentLevel, false))
			{
...
const IAvaSceneInterface* FAvalanchePageAssetUtils::GetSceneInterface(const UObject* InLoadedAsset)
{
	using namespace UE::AvalanchePageAssetUtils::Private;
	return GetInterfaceFromAvaAsset<IAvaSceneInterface>(InLoadedAsset);
}

const UAvaTransitionTree* FAvalanchePageAssetUtils::FindTransitionTree(const IAvaSceneInterface* InSceneInterface)
{
	if (!InSceneInterface)
	{
		return nullptr;
...
FAvaTagHandle FAvalanchePageAssetUtils::GetTransitionLayerTag(const UAvaTransitionTree* InTransitionTree)
{
