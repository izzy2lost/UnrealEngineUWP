...
UPackage* FAvalancheManagedInstanceUtils::MakeManagedInstancePackage(const FSoftObjectPath& InAssetPath)
{
	...
	UPackage* ManagedAvalancheBlueprintPackage = nullptr;
	...
	ManagedAvalancheBlueprintPackage = CreatePackage(*InstancePackageName);
	if (ManagedAvalancheBlueprintPackage)
	{
		ManagedAvalancheBlueprintPackage->SetFlags(RF_Transient | RF_Public);
	}
	else
	{
...
void FAvalancheManagedInstanceUtils::PreventWorldFromBeingSeenAsLeakingByLevelEditor(UWorld* InWorld)
{
