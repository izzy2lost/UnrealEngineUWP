...
namespace UE::AvalancheBroadcast::Private
{
	...
	static FString GetXmlSaveFilepath()
	{
		...
		if (FParse::Value(FCommandLine::Get(),TEXT("AvalancheBroadcastConfig="), BroadcastConfigName))
		{
		...
		else
		{
			...
			BroadcastConfigName = bIsServerRunning ? TEXT("AvalancheServerBroadcastConfig.xml") : TEXT("AvalancheBroadcastConfig.xml");
		}
...
UAvalancheBroadcast& UAvalancheBroadcast::Get()
{
	static UAvalancheBroadcast* Broadcast = nullptr;
	
	if (!IsValid(Broadcast))
	{
		static const TCHAR* PackageName = TEXT("/Temp/AvalancheMedia/AvalancheBroadcast");
		...
		Broadcast = NewObject<UAvalancheBroadcast>(BroadcastPackage
			, TEXT("AvalancheBroadcast")
...
void UAvalancheBroadcast::BeginDestroy()
{
...
UAvalancheBroadcast* UAvalancheBroadcast::GetAvalancheBroadcast()
{
	return &UAvalancheBroadcast::Get();
}

void UAvalancheBroadcast::StartBroadcast()
{
...
void UAvalancheBroadcast::StopBroadcast()
{
	GetCurrentProfile().StopChannelBroadcast();
}

bool UAvalancheBroadcast::IsBroadcastingAnyChannel() const
{
	return GetCurrentProfile().IsBroadcastingAnyChannel();
}

bool UAvalancheBroadcast::IsBroadcastingAllChannels() const
{
	return GetCurrentProfile().IsBroadcastingAllChannels();
}

bool UAvalancheBroadcast::ConditionalStartBroadcastChannel(const FName& InChannelName)
{
	FAvaOutputChannel& Channel = GetCurrentProfile().GetChannelMutable(InChannelName);
	...
	if (Channel.GetState() == EAvaChannelState::Idle)
	{
...
TArray<FName> UAvalancheBroadcast::GetProfileNames() const
{
...
const TMap<FName, FAvaBroadcastProfile>& UAvalancheBroadcast::GetProfiles() const
{
	return Profiles;
}

FName UAvalancheBroadcast::CreateProfile(FName InProfileName, bool bMakeCurrentProfile)
{
...
bool UAvalancheBroadcast::DuplicateProfile(FName InNewProfile, FName InTemplateProfile, bool bMakeCurrentProfile)
{
...
bool UAvalancheBroadcast::DuplicateCurrentProfile(FName InProfileName, bool bMakeCurrentProfile)
{
	return DuplicateProfile(InProfileName, CurrentProfile, bMakeCurrentProfile);
}

bool UAvalancheBroadcast::RemoveProfile(FName InProfileName)
{
...
bool UAvalancheBroadcast::CanRenameProfile(FName InProfileName, FName InNewProfileName, FText* OutErrorMessage) const
{
...
bool UAvalancheBroadcast::RenameProfile(FName InProfileName, FName InNewProfileName)
{
...
bool UAvalancheBroadcast::SetCurrentProfile(FName InProfileName)
{
	const bool bIsBroadcasting = IsBroadcastingAnyChannel();
...
FAvaBroadcastProfile& UAvalancheBroadcast::GetProfile(FName InProfileName)
{
...
const FAvaBroadcastProfile& UAvalancheBroadcast::GetProfile(FName InProfileName) const
{
	if (Profiles.Contains(InProfileName))
	{
		return Profiles[InProfileName];
...
#if WITH_EDITOR
void UAvalancheBroadcast::LoadBroadcast()
{
	using namespace UE::AvalancheBroadcast::Private;
	...
	if (FileReader.IsValid())
	{
		...
		UE::AvaSerializationUtils::SerializeObject(InputFormatter, this);
		FileReader->Close();
	}
...
void UAvalancheBroadcast::SaveBroadcast()
{
	...
	if (FileWriter.IsValid())	
	{
		...
		UE::AvaSerializationUtils::SerializeObject(XmlOutput, this);
...
FString UAvalancheBroadcast::GetBroadcastSaveFilepath() const
{
	return UE::AvalancheBroadcast::Private::GetXmlSaveFilepath();
}
#endif

void UAvalancheBroadcast::QueueNotifyChange(EAvaBroadcastChange InChange)
{
	...
	{		
		...
		if (bCreateAsyncTask)
		{
			TWeakObjectPtr<UAvalancheBroadcast> ThisWeak(this);
...
FDelegateHandle UAvalancheBroadcast::AddChangeListener(FOnAvaBroadcastChanged::FDelegate&& InDelegate)
{
	return OnBroadcastChanged.Add(MoveTemp(InDelegate));
}

void UAvalancheBroadcast::RemoveChangeListener(FDelegateHandle InDelegateHandle)
{
	OnBroadcastChanged.Remove(InDelegateHandle);
}

void UAvalancheBroadcast::RemoveChangeListener(const void* InUserObject)
{
	OnBroadcastChanged.RemoveAll(InUserObject);
}

int32 UAvalancheBroadcast::GetChannelNameCount() const
{
	return ChannelNames.Num();
}

int32 UAvalancheBroadcast::GetChannelIndex(FName InChannelName) const
{
	return ChannelNames.Find(InChannelName);
}

FName UAvalancheBroadcast::GetChannelName(int32 ChannelIndex) const
{
...
FName UAvalancheBroadcast::GetOrAddChannelName(int32 ChannelIndex)
{
	if (ChannelIndex < 0)
	{
		return NAME_None;
...
int32 UAvalancheBroadcast::AddChannelName(FName InChannelName)
{
	return ChannelNames.AddUnique(InChannelName);
}

TArray<int32> UAvalancheBroadcast::BuildChannelIndices() const
{
	...
	{
		for (const FAvaOutputChannel& Channel : Pair.Value.Channels)
		{
...
void UAvalancheBroadcast::UpdateChannelNames()
{
	...
	for (TPair<FName, FAvaBroadcastProfile>& Pair : Profiles)
	{
		for (FAvaOutputChannel& Channel : Pair.Value.Channels)
		{
...
bool UAvalancheBroadcast::CanRenameChannel(FName InChannelName, FName InNewChannelName) const
{
...
bool UAvalancheBroadcast::RenameChannel(FName InChannelName, FName InNewChannelName)
{
...
void UAvalancheBroadcast::SetChannelType(FName InChannelName, EAvaBroadcastChannelType InChannelType)
{
	ChannelTypes.Add(InChannelName, InChannelType);
}

EAvaBroadcastChannelType UAvalancheBroadcast::GetChannelType(FName InChannelName) const
{
...
void UAvalancheBroadcast::PinChannel(FName InChannelName, FName InProfileName)
{
	PinnedChannels.Add(InChannelName, InProfileName);
}

void UAvalancheBroadcast::UnpinChannel(FName InChannelName)
{
	PinnedChannels.Remove(InChannelName);
}

FName UAvalancheBroadcast::GetPinnedChannelProfileName(FName InChannelName) const
{
	const FName* ProfileName = PinnedChannels.Find(InChannelName);
	return ProfileName ? *ProfileName : NAME_None;
}

void UAvalancheBroadcast::RebuildProfiles()
{
	for (TPair<FName, FAvaBroadcastProfile>& Profile : Profiles)
	{
		const bool bIsProfileActive = Profile.Key == CurrentProfile;
...
#if WITH_EDITOR
void UAvalancheBroadcast::PostEditUndo()
{
...
FAvaBroadcastProfile& UAvalancheBroadcast::CreateProfileInternal(FName InProfileName)
{	
...
void UAvalancheBroadcast::EnsureValidCurrentProfile()
{
...
void UAvalancheBroadcast::UpdateProfileNames()
{
	for (TPair<FName, FAvaBroadcastProfile>& Pair : Profiles)
	{
