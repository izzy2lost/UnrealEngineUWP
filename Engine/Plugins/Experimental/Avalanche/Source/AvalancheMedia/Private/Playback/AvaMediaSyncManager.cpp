...
FAvaMediaSyncManager::FAvaMediaSyncManager(const FString& InRemoteName)
...
FAvaMediaSyncManager::~FAvaMediaSyncManager()
{
	UnregisterEventHandlers();
}

void FAvaMediaSyncManager::SetEnable(bool bInEnabled)
{
...
void FAvaMediaSyncManager::Tick()
{
...
TOptional<bool> FAvaMediaSyncManager::GetAssetSyncStatus(const FSoftObjectPath& InAssetPath, bool bInForceRefresh)
{
...
void FAvaMediaSyncManager::EnumerateAllTrackedPackages(TFunctionRef<void(const FName& /*InPackageName*/, bool /*bInNeedsSync*/)> InFunction) const
{
...
TArray<FName> FAvaMediaSyncManager::GetPendingRequests() const
{
...
void FAvaMediaSyncManager::RegisterEventHandlers()
{
	UPackage::PackageSavedWithContextEvent.AddRaw(this, &FAvaMediaSyncManager::HandlePackageSaved);
	IAvaMediaModule::Get().GetOnAvaMediaSyncPackageModified().AddRaw(this, &FAvaMediaSyncManager::HandleAvaSyncPackageModified);
	if (IAssetRegistry* AssetRegistry = IAssetRegistry::Get())
	{
		AssetRegistry->OnAssetRemoved().AddRaw(this, &FAvaMediaSyncManager::HandleAssetRemoved);
	}
}

void FAvaMediaSyncManager::UnregisterEventHandlers()
{
...
void FAvaMediaSyncManager::RequestPackageSyncStatus(const FName& InPackageName)
{
	...
	const FDateTime ExpirationTime = CurrentTime + FTimespan::FromSeconds(UAvalancheMediaSettings::Get().ServerPendingStatusRequestTimeout);
	CompareRequests.Add(InPackageName, ExpirationTime);

	TWeakPtr<FAvaMediaSyncManager> LocalWeakThis = SharedThis(this);
	AvaMediaSyncProvider->CompareWithRemote(RemoteName,{InPackageName},
		FOnAvaSyncCompareResponse::CreateLambda([LocalWeakThis, InPackageName](const TSharedPtr<FAvaSyncCompareResponse>& Response)
		{				
			if (const TSharedPtr<FAvaMediaSyncManager> LocalThis = LocalWeakThis.Pin())
			{
...
void FAvaMediaSyncManager::HandleExpiredRequest(const FName& InPackageName)
{
...
void FAvaMediaSyncManager::HandleFailedRequest(const FName& InPackageName, const FString& InErrorMessage)
{
...
void FAvaMediaSyncManager::HandleSyncStatusReceived(const FName& InPackageName, bool bInNeedsSynchronization)
{
	...
	if (const IAssetRegistry* AssetRegistry = IAssetRegistry::Get())
	{
		...
		if (AssetRegistry->GetAssetsByPackageName(InPackageName, RegistryData.Data))
		{
			...
			{
				if (FAvaMediaPlaybackManager::IsPlaybackAsset(Asset))
				{
...
void FAvaMediaSyncManager::HandleAvaSyncPackageModified(IAvaMediaSyncProvider* InAvaMediaSyncProvider, const FName& InPackageName)
{
	if (IAssetRegistry* AssetRegistry = IAssetRegistry::Get())
	{
...
void FAvaMediaSyncManager::HandlePackageSaved(const FString& InPackageFileName, UPackage* InPackage, FObjectPostSaveContext InObjectSaveContext)
{
...
void FAvaMediaSyncManager::HandleAssetRemoved(const FAssetData& InAssetData)
{
