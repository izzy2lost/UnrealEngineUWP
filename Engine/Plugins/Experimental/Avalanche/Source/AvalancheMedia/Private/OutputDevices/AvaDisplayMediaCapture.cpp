...
DEFINE_LOG_CATEGORY(LogAvaDisplayMedia);

class UAvaDisplayMediaCapture::FCaptureInstance
{
public:
	FCaptureInstance(UAvaDisplayMediaOutput* InMediaOutput, UTextureRenderTarget2D* InRenderTarget)
	{
		if (!InRenderTarget)
		{
			return;
...
UAvaDisplayMediaCapture::~UAvaDisplayMediaCapture()
{
	delete CaptureInstance;
}

void UAvaDisplayMediaCapture::OnRHIResourceCaptured_RenderingThread(const FCaptureBaseData& InBaseData, TSharedPtr<FMediaCaptureUserData, ESPMode::ThreadSafe> InUserData, FTextureRHIRef InTexture)
{
	...
	if (CaptureInstance)
	{
		...
		if (Target.IsValid())
		{
			...
			UE::AvaRenderTargetMediaUtils::CopyTexture(RHICmdList, InTexture, Target);
		}
	}
}

bool UAvaDisplayMediaCapture::InitializeCapture()
{
	return true;
}

bool UAvaDisplayMediaCapture::PostInitializeCaptureViewport(TSharedPtr<FSceneViewport>& InSceneViewport)
{
	bool bSuccess = false;
	...
	{
		UE_LOG(LogAvaDisplayMedia, Error, TEXT("Can't start capture of a scene viewport with no back buffer."));
	}
	return bSuccess;
}

bool UAvaDisplayMediaCapture::PostInitializeCaptureRenderTarget(UTextureRenderTarget2D* InRenderTarget)
{
	bool bSuccess = StartNewCapture(FIntPoint(InRenderTarget->SizeX, InRenderTarget->SizeY), InRenderTarget->GetFormat());
	return bSuccess;
}

void UAvaDisplayMediaCapture::StopCaptureImpl(bool /*bAllowPendingFrameToBeProcess*/)
{
	TRACE_BOOKMARK(TEXT("UAvaDisplayMediaCapture::StopCapture"));
...
bool UAvaDisplayMediaCapture::StartNewCapture(const FIntPoint& InSourceTargetSize, EPixelFormat InSourceTargetFormat)
{
	TRACE_BOOKMARK(TEXT("UAvaDisplayMediaCapture::StartNewCapture"));
	{
		FScopeLock ScopeLock(&CaptureInstanceCriticalSection);
		...
		UAvaDisplayMediaOutput* AvaDisplayMediaOutput = CastChecked<UAvaDisplayMediaOutput>(MediaOutput);
		if (AvaDisplayMediaOutput)
		{
			static const FName RenderTargetBaseName = TEXT("AvaDisplayCapture_RenderTarget");
			RenderTarget = AvaRenderTargetUtils::CreateDefaultRenderTarget(RenderTargetBaseName);
			check(RenderTarget);
		...
		{
			UE_LOG(LogAvaDisplayMedia, Error, TEXT("Internal Error: Media Capture's associated Media Output is not of type \"UAvaDisplayMediaOutput\"."));
		}
	}
