...
class FAvaDeviceProviderProxyManager;

/**
 * Implements a proxy for the device provider of another system.
 */
class FAvaDeviceProviderProxy: public IMediaIOCoreDeviceProvider
{
public:
	FAvaDeviceProviderProxy(FAvaDeviceProviderProxyManager* InManager, const FAvaDeviceProviderData& InData);
	virtual ~FAvaDeviceProviderProxy() = default;
...
private:
	FAvaDeviceProviderProxyManager* Manager;
	FAvaDeviceProviderData Data;
};

/**
 *	Implements a wrapper of multiple other providers of the same device type from different systems.
 */
class FAvaDeviceProviderWrapper : public IMediaIOCoreDeviceProvider
{
public:
	/**
	 ...
	 */
	FAvaDeviceProviderWrapper(FName InName, FAvaDeviceProviderProxyManager* InManager);
	virtual ~FAvaDeviceProviderWrapper() = default;
	...
	void AddProvider(const FString& InServerName, TUniquePtr<FAvaDeviceProviderProxy>&& InProvider);
	 ...
	 * server name. If the provider is local, the server name is FAvaDeviceProviderProxyManager::LocalServerName.
	 */
...
private:
	FAvaDeviceProviderProxyManager* Manager = nullptr;
	...
	FAvaDeviceProviderProxy DummyProxy;
	
	struct FProviderInfo
	{
		FString ServerName;
		IMediaIOCoreDeviceProvider* Provider;
		TUniquePtr<FAvaDeviceProviderProxy> Proxy;
	};
	TArray<FProviderInfo> Providers;
};

class FAvaDeviceProviderProxyManager : public IAvaDeviceProviderProxyManager
{
public:
	virtual ~FAvaDeviceProviderProxyManager() {}

	//~ Begin IAvaDeviceProviderProxyManager
	const FAvaDeviceProviderWrapper* GetDeviceProviderWrapper(FName InDeviceProviderName) const override;
	...
	virtual void Install(const FString& InServerName, const FAvaDeviceProviderDataList& InDeviceProviderDataList) override;
	virtual void Uninstall(const FString& InServerName) override;

	virtual TSet<FString> GetServerNames() const override;
	virtual const TSharedPtr<const FAvaDeviceProviderDataList> GetDeviceProviderDataListForServer(const FString& InServerName) const override;
	virtual FString GetLocalServerName() const override { return LocalServerName; }
	//~ End IAvaDeviceProviderProxyManager
...
private:
	TMap<FName, TUniquePtr<FAvaDeviceProviderWrapper>> DeviceProviderWrappers;

	/** Keep a copy of the device provider data list last received for each server. */
	TMap<FString, TSharedPtr<FAvaDeviceProviderDataList>> DeviceProviderDataListByServer;
};
