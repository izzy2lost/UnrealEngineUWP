...
//-----------------------------------------------------------------------------
//----- FAvaDeviceProviderProxy -----------------------------------------------
//-----------------------------------------------------------------------------

FAvaDeviceProviderProxy::FAvaDeviceProviderProxy(FAvaDeviceProviderProxyManager* InManager, const FAvaDeviceProviderData& InData)
: Manager(InManager)
, Data(InData)
{
...
TArray<FMediaIOConfiguration> FAvaDeviceProviderProxy::GetConfigurations(bool bAllowInput, bool bAllowOutput) const
{
...
TArray<FMediaIOMode> FAvaDeviceProviderProxy::GetModes(const FMediaIODevice& InDevice, bool bInOutput) const
{
		return Data.Modes; // TODO	
}

//-----------------------------------------------------------------------------
//----- FAvaDeviceProviderWrapper ---------------------------------------------
//-----------------------------------------------------------------------------

FAvaDeviceProviderWrapper::FAvaDeviceProviderWrapper(FName InName, FAvaDeviceProviderProxyManager* InManager)
	: Manager(InManager)
	, Name(InName)
	, DummyProxy(InManager, FAvaDeviceProviderData())
{ }

void FAvaDeviceProviderWrapper::AddProvider(const FString& InServerName, IMediaIOCoreDeviceProvider* InProvider, bool bInIsLocal)
{
	if (bInIsLocal)
	{
...
void FAvaDeviceProviderWrapper::AddProvider(const FString& InServerName, TUniquePtr<FAvaDeviceProviderProxy>&& InProviderProxy)
{
	for (FProviderInfo& Info : Providers)
	{
		if (Info.ServerName == InServerName)
...
void FAvaDeviceProviderWrapper::RemoveProvider(IMediaIOCoreDeviceProvider* InProvider)
{
...
void FAvaDeviceProviderWrapper::RemoveProvider(const FString& InServerName)
{
	for (int32 i = 0; i < Providers.Num(); ++i)
	{
...
const IMediaIOCoreDeviceProvider* FAvaDeviceProviderWrapper::GetProviderForDeviceName(const FName& InDeviceName) const
{
	...
	for (const FProviderInfo& ProviderInfo : Providers)
	{
		...
		// See FAvaDeviceProviderData::ApplyServerName.
...
const IMediaIOCoreDeviceProvider* FAvaDeviceProviderWrapper::GetProviderForServer(const FString& InServerName) const
{
	for (const FProviderInfo& ProviderInfo : Providers)
	{
		// For device from another server, the device name starts with the "ServerName:"
...
FString FAvaDeviceProviderWrapper::GetServerNameForDevice(const FName& InDeviceName) const
{
	// Search which provider proxy has the given device.
	for (const FProviderInfo& ProviderInfo : Providers)
	{
		if (FAvaDeviceProviderProxyManager::IsDevicePresent(ProviderInfo.Provider, InDeviceName))
		{
...
bool FAvaDeviceProviderWrapper::ShouldRemove() const
{
...
TArray<FMediaIOConnection> FAvaDeviceProviderWrapper::GetConnections() const
{
	if (Providers.Num() == 1)
	{
...
TArray<FMediaIOConfiguration> FAvaDeviceProviderWrapper::GetConfigurations() const
{
	if (Providers.Num() == 1)
	{
...
TArray<FMediaIOConfiguration> FAvaDeviceProviderWrapper::GetConfigurations(bool bInAllowInput, bool bInAllowOutput) const
{
	if (Providers.Num() == 1)
	{
...
TArray<FMediaIODevice> FAvaDeviceProviderWrapper::GetDevices() const
{
	if (Providers.Num() == 1)
	{
...
TArray<FMediaIOMode> FAvaDeviceProviderWrapper::GetModes(const FMediaIODevice& InDevice, bool bInOutput) const
{
...
TArray<FMediaIOInputConfiguration> FAvaDeviceProviderWrapper::GetInputConfigurations() const
{
	if (Providers.Num() == 1)
	{
...
TArray<FMediaIOOutputConfiguration> FAvaDeviceProviderWrapper::GetOutputConfigurations() const
{
	if (Providers.Num() == 1)
	{
...
TArray<FMediaIOVideoTimecodeConfiguration> FAvaDeviceProviderWrapper::GetTimecodeConfigurations() const
{
	if (Providers.Num() == 1)
	{
...
//-----------------------------------------------------------------------------
//----- FAvaDeviceProviderProxyManager ----------------------------------------
//-----------------------------------------------------------------------------
FString FAvaDeviceProviderProxyManager::LocalServerName(TEXT("Local (This Process)"));

const FAvaDeviceProviderWrapper* FAvaDeviceProviderProxyManager::GetDeviceProviderWrapper(FName InDeviceProviderName) const
{
	const TUniquePtr<FAvaDeviceProviderWrapper>* DeviceProviderWrapper = DeviceProviderWrappers.Find(InDeviceProviderName);
	return DeviceProviderWrapper ? DeviceProviderWrapper->Get() : nullptr;
}

TArray<const IMediaIOCoreDeviceProvider*> FAvaDeviceProviderProxyManager::GetDeviceProvidersForServer(const FString& InServerName) const
{
	TArray<const IMediaIOCoreDeviceProvider*> Providers;
	for (const TPair<FName, TUniquePtr<FAvaDeviceProviderWrapper>>& WrapperEntry : DeviceProviderWrappers)
	{
...
FString FAvaDeviceProviderProxyManager::FindServerNameForDevice(const FName& InDeviceProviderName, const FName& InDeviceName) const
{
	const FAvaDeviceProviderWrapper* DeviceProviderWrapper = GetDeviceProviderWrapper(InDeviceProviderName);
...
bool FAvaDeviceProviderProxyManager::IsLocalDevice(const FName& InDeviceProviderName, const FName& InDeviceName) const
{
	const FAvaDeviceProviderWrapper* DeviceProviderWrapper = GetDeviceProviderWrapper(InDeviceProviderName);

	if (DeviceProviderWrapper && DeviceProviderWrapper->HasLocalProvider())
	{
		return FAvaDeviceProviderProxyManager::IsDevicePresent(DeviceProviderWrapper->GetLocalProvider(), InDeviceName);
	}
	else if (const IMediaIOCoreDeviceProvider* LocalProvider = GetLocalDeviceProvider(InDeviceProviderName))
	{
		return FAvaDeviceProviderProxyManager::IsDevicePresent(LocalProvider, InDeviceName);
	}
	return false;
}

void FAvaDeviceProviderProxyManager::Install(const FString& InServerName, const FAvaDeviceProviderDataList& InDeviceProviderDataList)
{
	...
	DeviceProviderDataListByServer.Add(InServerName, MakeShared<FAvaDeviceProviderDataList>(InDeviceProviderDataList));
	
	for (const FAvaDeviceProviderData& ProviderData : InDeviceProviderDataList.DeviceProviders)
	{
		FAvaDeviceProviderWrapper* Wrapper = nullptr;
		...
		else
		{
			...
			TUniquePtr<FAvaDeviceProviderWrapper> DeviceProviderWrapper = MakeUnique<FAvaDeviceProviderWrapper>(ProviderData.Name, this);
		...
		if (Wrapper)
		{
			...
			TUniquePtr<FAvaDeviceProviderProxy> DeviceProviderProxy = MakeUnique<FAvaDeviceProviderProxy>(this, ProviderData);
	...
	UAvalancheBroadcast::Get().QueueNotifyChange(EAvaBroadcastChange::OutputDevices);
}

void FAvaDeviceProviderProxyManager::Uninstall(const FString& InServerName)
{
	if (!IMediaIOCoreModule::IsAvailable())
	{
		return;
	...
	for (TPair<FName, TUniquePtr<FAvaDeviceProviderWrapper>>& Entry : DeviceProviderWrappers )
	{
...
void FAvaDeviceProviderProxyManager::ListAllProviders()
{
	if (!IMediaIOCoreModule::IsAvailable())
	{
	...
	for (const UClass* Class : TObjectRange<UClass>())
	{
		if (Class->IsChildOf(UMediaOutput::StaticClass()))
		{
			...
			if (FName DeviceProviderName = UE::AvaMediaOutputUtils::GetDeviceProviderName(Class); !DeviceProviderName.IsNone())
			{
...
void FAvaDeviceProviderProxyManager::TestInstall()
{
...
void FAvaDeviceProviderProxyManager::TestUninstall()
{
	Uninstall(TEXT("RemoteTest"));
}

bool FAvaDeviceProviderProxyManager::IsDevicePresent(const IMediaIOCoreDeviceProvider* InProvider, const FName& InDeviceName)
{
...
void FAvaDeviceProviderProxyManager::ReorderWrappedLocalProviders()
{
	if (IMediaIOCoreModule::IsAvailable())
	{
		...
		for (TPair<FName, TUniquePtr<FAvaDeviceProviderWrapper>>& WrapperEntry : DeviceProviderWrappers)
		{
...
const IMediaIOCoreDeviceProvider* FAvaDeviceProviderProxyManager::GetLocalDeviceProvider(const FName& InDeviceProviderName)
{
	if (IMediaIOCoreModule::IsAvailable())
	{
...
TSet<FString> FAvaDeviceProviderProxyManager::GetServerNames() const
{
...
const TSharedPtr<const FAvaDeviceProviderDataList> FAvaDeviceProviderProxyManager::GetDeviceProviderDataListForServer(const FString& InServerName) const
{
	const TSharedPtr<FAvaDeviceProviderDataList>* FoundDeviceProviderDataList = DeviceProviderDataListByServer.Find(InServerName);
