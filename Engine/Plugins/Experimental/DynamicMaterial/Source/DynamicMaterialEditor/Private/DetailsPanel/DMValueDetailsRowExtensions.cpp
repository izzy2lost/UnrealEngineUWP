...
namespace UE::DynamicMaterialEditor::Private
{
	...
	const FName RowExtensionName = "DMValueRowExtensionContextSection";
	const FName MaterialDesignerMenuName = "MaterialDesigner";
	const FName PropertyEditorModuleName = "PropertyEditor";
	const FText SectionName = LOCTEXT("MaterialDesigner", "Material Designer");
	const FText MenuEntry = LOCTEXT("ParameterName", "Parameter Name");
	const FText MenuEntryX = LOCTEXT("ParameterNameX", "Parameter Name X");
	const FText MenuEntryY = LOCTEXT("ParameterNameY", "Parameter Name Y");
	const FText ToolTip = LOCTEXT("SetParameterNameToolTip", "Set the name of the parameter this property is exposed as within the generated material.");
	const FText InvalidCharacterError = LOCTEXT("InvalidCharacterError", "Valid characters are A-Z, A-z, _ and -");
	const FText TooShortError = LOCTEXT("TooShortError", "Min 3 characters.");
	const FText TooLongError = LOCTEXT("TooLongError", "Max 50 characters.");
	const TArray<TPair<TCHAR, TCHAR>> ValidCharacterRanges = {
		{'A', 'Z'},
		{'a', 'z'},
		{'0', '9'},
		{'-', '-'},
...
void FDMValueDetailsRowExtensions::FillPropertyRightClickMenu(UToolMenu* InToolMenu)
{
	...
	FToolMenuSection& Section = InToolMenu->AddSection(MaterialDesignerMenuName, SectionName);
...
void FDMValueDetailsRowExtensions::FillPropertyRightClickMenu_TextureUV(FToolMenuSection& InSection, UDMTextureUV* InTextureUV, FName InPropertyName)
{
	...
	{
		CreateMenuEntry(MenuEntry, ComponentX);
	}
	...
	{
		CreateMenuEntry(MenuEntryX, ComponentX);
		CreateMenuEntry(MenuEntryY, ComponentY);
	}
}
