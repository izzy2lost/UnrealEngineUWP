...
UDMMaterialStage* UDMMaterialStageBlend::CreateStage(TSubclassOf<UDMMaterialStageBlend> InMaterialStageBlendClass, UDMMaterialLayerObject* InLayer)
{
	...
	UDMMaterialStageBlend* SourceBlend = NewObject<UDMMaterialStageBlend>(NewStage, InMaterialStageBlendClass.Get(), NAME_None, RF_Transactional);
	check(SourceBlend);
...
void UDMMaterialStageBlend::GetMaskAlphaBlendNode(const TSharedRef<FDMMaterialBuildState>& InBuildState, UMaterialExpression*& OutExpression, int32& OutOutputIndex, int32& OutOutputChannel) const
{
...
void UDMMaterialStageBlend::AddDefaultInput(int32 InInputIndex) const
{
	...
	switch (InInputIndex)
	{
		case InputAlpha:
		{
			UDMMaterialStageInputValue* InputValue = Cast<UDMMaterialStageInputValue>(
...
int32 UDMMaterialStageBlend::ResolveInput(const TSharedRef<FDMMaterialBuildState>& InBuildState, int32 InputIndex, FDMMaterialStageConnectorChannel& OutChannel, TArray<UMaterialExpression*>& OutExpressions) const
{
	...
	if (InputIndex == InputB)
	{
		if (UDMMaterialStage* Stage = GetStage())
		{
			...
			{
				Layer->ApplyEffects(InBuildState, Stage, OutExpressions, OutChannel.OutputChannel, NodeOutputIndex);
			}
		}
	}
...
FDMExpressionInput UDMMaterialStageBlend::GetLayerMaskLinkTextureUVInputExpressions(const TSharedRef<FDMMaterialBuildState>& InBuildState) const
{
	...
	for (int32 InputIdx = InputA; InputIdx <= InputB; ++InputIdx)
	{
		...
		ExpressionInput.OutputIndex = ResolveLayerMaskTextureUVLinkInputImpl(InBuildState, StageInputs[StageInputIdx], Channel, ExpressionInput.OutputExpressions);
