...
namespace UE::NNERuntimeIREECpu::Private
{
	bool ResolveEnvironmentVariables(FString& String)
	{
		...
		while (StartIndex != INDEX_NONE)
		{
			...
			if (EndIndex > StartIndex)
			{
				...
				FString EnvironmentVariableValue = FNNERuntimeIREECpuCompiler::GetEnvVar(EnvironmentVariableName);
	...
	FNNERuntimeIREECpuCompiler::FNNERuntimeIREECpuCompiler(const FString& InCompilerCommand, const FString& InLinkerCommand, const FString& InSharedLibExt, TConstArrayView<FIREEBuildTarget> InTargets) : CompilerCommand(InCompilerCommand), LinkerCommand(InLinkerCommand), SharedLibExt(InSharedLibExt), Targets(InTargets)
	{

	}

	TUniquePtr<FNNERuntimeIREECpuCompiler> FNNERuntimeIREECpuCompiler::Make(const FString& InTargetPlatformName)
	{
		...
		for (FString BuildConfigFilePath : BuildConfigFilePaths)
		{
			if (PlatformFile.FileExists(*BuildConfigFilePath))
			{
				...
				if (FFileHelper::LoadFileToString(BuildConfigFileString, *BuildConfigFilePath))
				{
					...
					FIREEBuildConfig BuildConfig;
					if (FJsonSerializer::Deserialize(JsonReader, JsonObject) && JsonObject.IsValid() && BuildConfig.FromJson(JsonObject))
					{
						if (BuildConfig.Targets.IsEmpty())
						{
							UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu could not find targets in %s"), *BuildConfigFilePath);
							continue;
						}
						...
						for (int32 i = 0; i < BuildConfig.CompilerCommand.Num(); i++)
						{
							...
							{
								UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu could not replace environment variables in %s"), *BuildConfig.CompilerCommand[i]);
							}
						}
						if (TmpCompilerCommand.IsEmpty())
						{
							UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu could not find the compiler executable in %s"), *BuildConfigFilePath);
							continue;
						}
						...
						for (int32 i = 0; i < BuildConfig.LinkerCommand.Num(); i++)
						{
							...
							{
								UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu could not replace environment variables in %s"), *BuildConfig.LinkerCommand[i]);
							}
						}
						if (TmpLinkerCommand.IsEmpty())
						{
							UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu could not find the linker executable in %s"), *BuildConfigFilePath);
							continue;
						}
					...
					{
						UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu could not parse build config file %s"), *BuildConfigFilePath);
					}
				}
				else
				{
		...
		if (CompilerCommand.IsEmpty() || LinkerCommand.IsEmpty() || Targets.IsEmpty())
		{
			return TUniquePtr<FNNERuntimeIREECpuCompiler>();
		}
	...
	bool FNNERuntimeIREECpuCompiler::CompileMlir(TConstArrayView<uint8> InFileData, const FString& InModelName, const FString& InIntermediateDir, const FString& InStagingDir, TArray<FIREECompilerResult>& OutCompilerResults)
	{
		...
		for (int32 i = 0; i < Targets.Num(); i++)
		{
			...
			FString CompilerArguments = Targets[i].CompilerArguments;
			if (!ResolveEnvironmentVariables(CompilerArguments))
			{
				UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu could not replace environment variables in %s"), *Targets[i].CompilerArguments);
				continue;
			}
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu failed to compile the model \"%s\" using the command:"), *InputFilePath);
				UE_LOG(LogTemp, Warning, TEXT("\"%s\" %s"), *CompilerCommand, *CompilerArguments);
				bResult = false;
				continue;
			}

			FString LinkerArguments = Targets[i].LinkerArguments;
			if (!ResolveEnvironmentVariables(LinkerArguments))
			{
				UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu could not replace environment variables in %s"), *Targets[i].LinkerArguments);
				bResult = false;
				continue;
			}
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu failed to link the model \"%s\" using the command:"), *InputFilePath);
				UE_LOG(LogTemp, Warning, TEXT("\"%s\" %s"), *LinkerCommand, *LinkerArguments);
				bResult = false;
				continue;
			}

			FString StagingFilePathNoExt = FPaths::Combine(InStagingDir, Targets[i].Architecture, InModelName);
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu failed to copy \"%s\" to \"%s\""), *SharedLibFilePath, *StagedSharedLibFilePath);
				bResult = false;
			}
			if (IFileManager::Get().Copy(*StagedVmfbFilePath, *VmfbFilePath) != COPY_OK)
			{
				UE_LOG(LogTemp, Warning, TEXT("UNNERuntimeIREECpu failed to copy \"%s\" to \"%s\""), *VmfbFilePath, *StagedVmfbFilePath);
				bResult = false;
			}
			...
			FIREECompilerResult Result;
			Result.Architecture = Targets[i].Architecture;
			Result.RelativeDirPath = Targets[i].Architecture;
