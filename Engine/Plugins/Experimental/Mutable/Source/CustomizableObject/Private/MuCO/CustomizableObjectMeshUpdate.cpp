...
FCustomizableObjectMeshStreamIn::FCustomizableObjectMeshStreamIn(const UCustomizableObjectSkeletalMesh* InMesh, bool bInHighPrio, bool bInRenderThread)
...
namespace impl
{
	void Task_Mutable_UpdateMesh(const TSharedPtr<FMutableMeshOperationData> OperationData)
	{
		...
		for (int32 Index = OperationData->PendingFirstLODIdx; Index < OperationData->CurrentFirstLODIdx; ++Index)
		{
			...
			OperationData->Meshes[Index] = System->GetMesh(InstanceID, MeshID);
		}
