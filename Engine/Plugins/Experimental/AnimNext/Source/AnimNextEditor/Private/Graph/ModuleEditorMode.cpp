...
FModuleEditorMode::FModuleEditorMode(TSharedRef<FWorkflowCentricApplication> InHostingApp)
	: FApplicationMode(GraphModes::GraphEditor)
	, HostingAppPtr(InHostingApp)
{
	HostingAppPtr = InHostingApp;

	TSharedRef<FModuleEditor> ModuleEditor = StaticCastSharedRef<FModuleEditor>(InHostingApp);
	
	TabFactories.RegisterFactory(MakeShared<FGraphDetailsTabSummoner>(ModuleEditor, FOnDetailsViewCreated::CreateSP(&ModuleEditor.Get(), &FModuleEditor::HandleDetailsViewCreated)));
	...
	ModuleEditor->RegisterModeToolbarIfUnregistered(GetModeName());
}

void FModuleEditorMode::RegisterTabFactories(TSharedPtr<FTabManager> InTabManager)
{
...
void FModuleEditorMode::AddTabFactory(FCreateWorkflowTabFactory FactoryCreator)
{
...
void FModuleEditorMode::RemoveTabFactory(FName TabFactoryID)
{
