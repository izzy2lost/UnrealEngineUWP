	...
	: Class(InClass)
{
	...
	// Add properties after functions, checking for duplicate names
	for(TFieldIterator<FProperty> It(InClass, EFieldIterationFlags::IncludeSuper | EFieldIterationFlags::IncludeInterfaces); It; ++It)
	{
		...
		if(Property->HasAnyPropertyFlags(CPF_Edit | CPF_BlueprintVisible))
		{
			...
			if(TypeHandle.IsValid())
			{
				...
				{
					UE_LOGFMT(LogAnimation, Warning, "Could not add parameter '{ParameterName}' for class '{ClassName}' as it already exists", ParameterName, InClass->GetFName());
				}
			}
		}
	}
	...
	// Add any additional extension libraries that extend this class
			...
			for(TFieldIterator<UFunction> It(ProxyClass); It; ++It)
			{
				...
				if(CheckSignature(Function, ExtendedClass))
				{
					...
					if(TypeHandle.IsValid())
					{
						...
						{
							UE_LOGFMT(LogAnimation, Warning, "Could not add parameter '{ParameterName}' for class proxy '{ClassName}' as it already exists", ParameterName, InClass->GetFName());
						}
					}
				}
			}
