...
namespace UE::AnimNext
{
	...
	struct ANIMNEXT_API IDiscreteBlend : IDecoratorInterface
	{
		...
		virtual float GetBlendWeight(FExecutionContext& Context, const TDecoratorBinding<IDiscreteBlend>& Binding, int32 ChildIndex) const;
		...
		virtual const FAlphaBlend* GetBlendState(FExecutionContext& Context, const TDecoratorBinding<IDiscreteBlend>& Binding, int32 ChildIndex) const;

		// Returns the blend destination child index (aka the active child index)
		// Returns INDEX_NONE if no child is active
		virtual int32 GetBlendDestinationChildIndex(FExecutionContext& Context, const TDecoratorBinding<IDiscreteBlend>& Binding) const;

		// Called when a blend transition between children occurs
		virtual void OnBlendTransition(FExecutionContext& Context, const TDecoratorBinding<IDiscreteBlend>& Binding, int32 OldChildIndex, int32 NewChildIndex) const;

		// Called when the blend for specified child is initiated
		virtual void OnBlendInitiated(FExecutionContext& Context, const TDecoratorBinding<IDiscreteBlend>& Binding, int32 ChildIndex) const;

		// Called when the blend for specified child terminates
		virtual void OnBlendTerminated(FExecutionContext& Context, const TDecoratorBinding<IDiscreteBlend>& Binding, int32 ChildIndex) const;
	};
