...
namespace Dataflow
{
	...
	struct DATAFLOWSIMULATION_API  FDataflowSimulationData
	{
...
public:
	...
	void AdvanceSimulationData(const float DeltaTime, const float SimulationTime);

	/** Write all the data to the simulation proxies (to be send from GT->PT) */
	void WriteSimulationData(const float DeltaTime);

	/** Read all the data from the simulation proxies (received from PT->GT) */
	void ReadSimulationData(const float DeltaTime);
	...
	TSharedPtr<Dataflow::FDataflowSimulationContext> GetSimulationContext(const TObjectPtr<UDataflow>& DataflowAsset) const;
...
private :
	...
	TMap<TObjectPtr<UDataflow>, Dataflow::FDataflowSimulationData> SimulationData;
