...
struct FMeshPart
{
	...
	int32 PrecomputeMeshesIndex = INDEX_NONE;
	...
	FMeshPartsAssembly& AssemblyOut)
{
	...
	for ( int32 Index = 0; Index < NumStaticMeshInstances; ++Index)
	{
		...
		if (FoundPart == nullptr)
		{
			...
			NewPart->PrecomputeMeshesIndex = SourceMeshInstance.PrecomputeMeshesIndex;
...
void BuildCombinedMeshFromPrecomputedMeshes(
	TConstArrayView<IGeometryProcessing_CombineMeshInstances::FSinglePartMeshSet> CachedPartMeshes,
	const FMeshPartsAssembly& Assembly,
	...
	TArray<FCombinedSubAssembly>& CombinedResults)
{
	...
	for (int32 SetIndex = 0; SetIndex < NumParts; ++SetIndex)
	{
		const IGeometryProcessing_CombineMeshInstances::FSinglePartMeshSet& PartMeshes = CachedPartMeshes[Assembly.Parts[SetIndex]->PrecomputeMeshesIndex];
	...
	auto SourceIndexToMeshCategory = [&CachedPartMeshes, &Assembly](int32 PartIdx, int32 SourceIdx, int32& WithinCategoryIdx, bool& bCanPromoteWithinCategory, bool& bCanPromoteAtAll, double& PromotedAvgError)
		{
			const IGeometryProcessing_CombineMeshInstances::FSinglePartMeshSet& PartMeshes = CachedPartMeshes[Assembly.Parts[PartIdx]->PrecomputeMeshesIndex];
	...
	auto SourceIndexToMesh = [&CachedPartMeshes, &Assembly](int32 PartIdx, int32 SourceIdx)
		{
			const IGeometryProcessing_CombineMeshInstances::FSinglePartMeshSet& PartMeshes = CachedPartMeshes[Assembly.Parts[PartIdx]->PrecomputeMeshesIndex];
			if (SourceIdx < PartMeshes.Source.Num())
			{
	...
	auto GetPrecomputedMesh = [&PartSourcesPerLOD, &SourceIndexToMesh, &CachedPartMeshes, &Assembly, &PerLODOptions, NumParts](int32 LODLevel, int32 PartIdx, int32 InstIdx) -> const FDynamicMesh3*
		{
			...
			{
				const IGeometryProcessing_CombineMeshInstances::FSinglePartMeshSet& PartMeshes = CachedPartMeshes[Assembly.Parts[PartIdx]->PrecomputeMeshesIndex];
				PartIdx = FMath::Min(PartIdx, PartMeshes.Source.Num() + PartMeshes.Simplified.Num() - 1);
			}
			const FDynamicMesh3* Mesh = SourceIndexToMesh(PartIdx, SourceIdx);
			return Mesh;
		};
	auto GetLODTriCount = [&PartSourcesPerLOD, &GetPrecomputedMesh, &CachedPartMeshes, &Assembly, &PerLODOptions, NumParts, &SourceIndexToMesh](int32 LODLevel)
		{
			int32 TriCount = 0;
...
)
{
	...
	TMap<UStaticMesh*, int32> StaticMeshToPartMap;
	TMap<const IGeometryProcessing_CombineMeshInstances::FMeshLODSet*, int32> MeshLODSetToPartMap;
	...
	if (!bKeepExistingPartMeshes)
	{
		...
		for (int32 Index = 0; Index < NumStaticMeshInstances; ++Index)
		{
			...
			SourceMeshInstance.PrecomputeMeshesIndex = -1;
		}
		for (int32 Index = 0; Index < NumMeshLODSetInstances; ++Index)
		{
	...
	else
	{
		...
		{
			const IGeometryProcessing_CombineMeshInstances::FStaticMeshInstance& SourceMeshInstance = SourceInstanceList.StaticMeshInstances[Index];
			if (SourceMeshInstance.PrecomputeMeshesIndex != -1)
			{
				UStaticMesh* StaticMesh = SourceMeshInstance.SourceMesh;
				StaticMeshToPartMap.Add(StaticMesh, SourceMeshInstance.PrecomputeMeshesIndex);
			}
		}
		for (int32 Index = 0; Index < NumMeshLODSetInstances; ++Index)
		{
			...
			if (SourceMeshInstance.PrecomputeMeshesIndex != -1)
			{
				...
				const IGeometryProcessing_CombineMeshInstances::FMeshLODSet* MeshLODSet = &SourceInstanceList.MeshLODSets[MeshSetIndex];
				MeshLODSetToPartMap.Add(MeshLODSet, SourceMeshInstance.PrecomputeMeshesIndex);
			}
		}
	}
	...
	for (int32 Index = 0; Index < NumStaticMeshInstances; ++Index)
	{
		...
		UStaticMesh* StaticMesh = SourceMeshInstance.SourceMesh;
		int32* FoundIdx = StaticMeshToPartMap.Find(StaticMesh);
		int32 LocalIdx = -1;
		if (FoundIdx == nullptr)
		{
			LocalIdx = ResultMeshSets.Emplace();
			...
			ComputeSinglePartMeshSet(StaticMesh, Options, PartOptions, ResultMeshSets[LocalIdx]);
			FoundIdx = &LocalIdx;
			StaticMeshToPartMap.Add(StaticMesh, LocalIdx);
		}
		SourceMeshInstance.PrecomputeMeshesIndex = *FoundIdx;
	}
	...
	for (int32 Index = 0; Index < NumMeshLODSetInstances; ++Index)
	{
		...
		int32* FoundIdx = MeshLODSetToPartMap.Find(MeshLODSet);
		int32 LocalIdx = -1;
		if (FoundIdx == nullptr)
		{
			LocalIdx = ResultMeshSets.Emplace();
			...
			ComputeSinglePartMeshSet(MeshLODSet->ReferencedMeshLODs, Options, PartOptions, ResultMeshSets[LocalIdx]);
			FoundIdx = &LocalIdx;
			MeshLODSetToPartMap.Add(MeshLODSet, LocalIdx);
		}
		SourceMeshInstance.PrecomputeMeshesIndex = *FoundIdx;
	}
}
...
)
{
...

	{
		...
		{
			if (Part->PrecomputeMeshesIndex == INDEX_NONE)
			{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(CombineMeshInst_BuildMeshes);
		BuildCombinedMeshFromPrecomputedMeshes(CachedPartMeshes, PartAssembly, AllLODOptions, PerLODOptions, CombinedResults);
		for (FCombinedSubAssembly& Assembly : CombinedResults)
		{
			SubsetIDsOrdering.Add(Assembly.SubAssemblyID);
		}
