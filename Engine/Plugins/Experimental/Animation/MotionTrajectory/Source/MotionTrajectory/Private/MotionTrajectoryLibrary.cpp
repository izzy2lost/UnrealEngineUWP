...
void FMotionTrajectoryLibrary::UpdatePrediction_SimulateCharacterMovement(FPoseSearchQueryTrajectory& Trajectory, FCharacterTrajectoryData& CharacterTrajectoryData,
	const FTrajectorySamplingData& SamplingData, float DeltaSeconds)
{
	...
	const FRotator ControllerRotationRate = FMotionTrajectoryLibrary::CalculateControllerRotationRate(DeltaSeconds, CharacterTrajectoryData);
	FQuat ControllerRotationPerStep = (ControllerRotationRate * SamplingData.SecondsPerPredictionSample).Quaternion();
...
FRotator FMotionTrajectoryLibrary::CalculateControllerRotationRate(float DeltaSeconds, FCharacterTrajectoryData& CharacterTrajectoryData)
{
	...
	const AController* Controller = CharacterTrajectoryData.Character->Controller;
	if (!Controller)
	{
	...
	if (CharacterTrajectoryData.CharacterMovementComponent->ShouldRemainVertical())
	{
	...
	const FRotator DesiredRotationDelta = DesiredControllerRotation - CharacterTrajectoryData.DesiredControllerRotationLastUpdate;
	CharacterTrajectoryData.DesiredControllerRotationLastUpdate = DesiredControllerRotation;

	FRotator ControllerRotationRate = DesiredRotationDelta.GetNormalized() * (1.f / DeltaSeconds);
	if (CharacterTrajectoryData.MaxControllerRotationRate >= 0.f)
	{
		ControllerRotationRate.Pitch = FMath::Sign(ControllerRotationRate.Pitch) * FMath::Min(FMath::Abs(ControllerRotationRate.Pitch), CharacterTrajectoryData.MaxControllerRotationRate);
		ControllerRotationRate.Yaw = FMath::Sign(ControllerRotationRate.Yaw) * FMath::Min(FMath::Abs(ControllerRotationRate.Yaw), CharacterTrajectoryData.MaxControllerRotationRate);
		ControllerRotationRate.Roll = FMath::Sign(ControllerRotationRate.Roll) * FMath::Min(FMath::Abs(ControllerRotationRate.Roll), CharacterTrajectoryData.MaxControllerRotationRate);
	}
