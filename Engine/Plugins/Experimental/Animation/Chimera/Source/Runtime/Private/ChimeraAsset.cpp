...
bool UChimeraAsset::IsLooping() const
{
	...
	for (const FChimeraAssetItem& Item : Items)
	{
...
bool UChimeraAsset::HasRootMotion() const
{
...
float UChimeraAsset::GetPlayLength() const
{
...
#if WITH_EDITOR
int32 UChimeraAsset::GetFrameAtTime(float Time) const
{
...
FQuat UChimeraAsset::FindReferenceOrientation(const TArrayView<const FTransform> Transforms, const TArrayView<int32> SortedByWarpingWeightRotationItemIndex) const
{
...
FVector UChimeraAsset::FindReferencePosition(const TArrayView<const FTransform> Transforms, const TArrayView<float> NormalizedWarpingWeightTranslation) const
{
	const int32 ItemsNum = Items.Num();
...
UAnimationAsset* UChimeraAsset::GetAnimationAsset(const UE::PoseSearch::FRole& Role) const
{
	for (const FChimeraAssetItem& Item : Items)
	{
		if (Item.Role == Role)
...
FTransform UChimeraAsset::GetOrigin(const UE::PoseSearch::FRole& Role) const
{
	for (const FChimeraAssetItem& Item : Items)
	{
		if (Item.Role == Role)
...
#if WITH_EDITOR
FTransform UChimeraAsset::GetDebugWarpOrigin(const UE::PoseSearch::FRole& Role, bool bComposeWithDebugWarpOffset) const
{
	for (int32 ItemIndex = 0; ItemIndex < Items.Num(); ++ItemIndex)
	{
		const FChimeraAssetItem& Item = Items[ItemIndex];
		if (Item.Role == Role)
		{
...
void UChimeraAsset::CalculateWarpTransforms(float Time, const TArrayView<const FTransform> ActorRootBoneTransforms, TArrayView<FTransform> FullAlignedActorRootBoneTransforms, const UWorld* DebugDrawWorld) const
{
