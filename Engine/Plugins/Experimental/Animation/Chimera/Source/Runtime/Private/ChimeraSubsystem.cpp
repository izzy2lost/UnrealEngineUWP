...
{
	FString FChimeraAvailabilityEx::GetPoseHistoryName() const
	{
		if (HistoryCollector)
		{
	...
	const FAnimNode_PoseSearchHistoryCollector_Base* FChimeraAvailabilityEx::GetHistoryCollector(const UAnimInstance* AnimInstance) const
	{
		if (HistoryCollector)
		{
	...
	struct FSearchContextGroup
	{
		bool Contains(const FSearchContext& SearchContext) const
		{
		...
		void Add(const FSearchContext& SearchContext, int32 SearchContextIndex)
		{
			for (const TWeakObjectPtr<UAnimInstance>& AnimInstance : SearchContext.AnimInstances)
			{
				if (AnimInstance.Get())
		...
		void Merge(const FSearchContextGroup& SearchContextGroup)
		{
	...
	static void ProcessAvailabilityRequestsMap(const FAvailabilityRequestsMap& AvailabilityRequestsMap, UWorld* SubsystemWorld, FDatabaseToRoledAnimInstances& DatabaseToRoledAnimInstances)
	{
		for (FAvailabilityRequestsMap::TConstIterator Iter = AvailabilityRequestsMap.CreateConstIterator(); Iter; ++Iter)
		{
			...
			if (!AnimInstance)
			{
				UE_LOG(LogChimera, Log, TEXT("ProcessAvailabilityRequestsMap null anim instance. The associated character got removed from the world"));
				continue;
			}

			if (!AnimInstance->GetWorld())
			{
				UE_LOG(LogChimera, Error, TEXT("ProcessAvailabilityRequestsMap AnimInstance %s is not in a world!"), *AnimInstance->GetName());
				continue;
			}

			if (SubsystemWorld != AnimInstance->GetWorld())
			{
				UE_LOG(LogChimera, Error, TEXT("ProcessAvailabilityRequestsMap AnimInstance %s is from World %s, and supposed to be in World @s!"), *AnimInstance->GetName(), *AnimInstance->GetWorld()->GetName(), *SubsystemWorld->GetName());
				continue;
			}

			for (const FChimeraAvailabilityEx& AvailabilityRequest : Iter->Value)
			{
				if (!AvailabilityRequest.Database)
				{
					UE_LOG(LogChimera, Log, TEXT("ProcessAvailabilityRequestsMap null AvailabilityRequest.Database"));
					continue;
				}

				if (!AvailabilityRequest.Database->Schema)
				{
					UE_LOG(LogChimera, Error, TEXT("ProcessAvailabilityRequestsMap null Schema for Database %s"), *AvailabilityRequest.Database->GetName());
					continue;
				}
				...
				if (!HistoryCollector)
				{
					UE_LOG(LogChimera, Error, TEXT("ProcessAvailabilityRequestsMap couldn't find PoseHistory %s for AnimInstance %s"), *AvailabilityRequest.GetPoseHistoryName(), *AnimInstance->GetName());
					continue;
				}
				...
				else
				{
					for (const UE::PoseSearch::FRole& Role : AvailabilityRequest.RolesFilter)
					{
						...
						{
							UE_LOG(LogChimera, Warning, TEXT("ProcessAvailabilityRequestsMap unsupported Role %s for Database %s"), *Role.ToString(), *AvailabilityRequest.Database->GetName());
						}
					}
				}
			}
...
UChimeraSubsystem* UChimeraSubsystem::GetSubsystem_AnyThread(UObject* AnimInstance)
{
	if (AnimInstance)
	{
		if (UWorld* World = AnimInstance->GetWorld())
		{
			...
			return UWorld::GetSubsystem<UChimeraSubsystem>(World);
		}
	}
	return nullptr;
}

void UChimeraSubsystem::Deinitialize()
{
	DestroyAllIslands();
...
void UChimeraSubsystem::Tick(float DeltaSeconds)
{
	...
	// FScopeLock Lock(&AvailabilityRequestsMapMutex); is not necessary since UChimeraSubsystem gets ticked outside the parallel animation jobs
	...
	TArray<FSearchContext, TInlineAllocator<PreallocatedSearchesNum, TMemStackAllocator<>>> SearchContexts;
	for (const FDatabaseToRoledAnimInstancesPair& DatabaseToRoledAnimInstancesPair : DatabaseToRoledAnimInstances)
	{
		...
		GeneratePermutations(DatabaseToRoledAnimInstancesPair.Value.GetDataView(), CombinationCardinality, 
			[&AnimInstances, &CoveredRoles, Database, &RoledSkeletons, &SearchContexts](const TArrayView<const FRoledAnimInstance> RoledAnimInstances, const TArrayView<int32> Combination)
			{
				...
				FSearchContext& SearchContext = SearchContexts.AddDefaulted_GetRef();
	...
	TArray<FSearchContextGroup, TInlineAllocator<PreallocatedSearchesNum, TMemStackAllocator<>>> SearchContextGroups;
	for (int32 SearchContextIndex = 0; SearchContextIndex < SearchContexts.Num(); ++SearchContextIndex)
	{
		const FSearchContext& SearchContext = SearchContexts[SearchContextIndex];
...
TStatId UChimeraSubsystem::GetStatId() const
{
	...
	FName PoseHistoryName, const FAnimNode_PoseSearchHistoryCollector_Base* HistoryCollector, bool bValidateResultAgainstAvailabilities)
{
	...
	Result = FChimeraBlueprintResult();
	...
	if (AnimInstance)
	{
		...
		if (UChimeraIslandComponent* Island = FindIsland(AnimInstance))
		{
			...
			if (bValidateResultAgainstAvailabilities && Result.SelectedAnimation)
			{
				...
				for (const FChimeraAvailability& Availability : Availabilities)
				{
		...
		if (!Availabilities.IsEmpty())
		{
			...
			TArray<FChimeraAvailabilityEx>& AvailabilityRequests = AvailabilityRequestsMap.FindOrAdd(AnimInstance);
			for (const FChimeraAvailability& Availability : Availabilities)
			{
...
void UChimeraSubsystem::DebugDraw() const
{
	using namespace UE::Chimera;
	...
	for (FAvailabilityRequestsMap::TConstIterator Iter = AvailabilityRequestsMap.CreateConstIterator(); Iter; ++Iter)
	{
		...
		if (UAnimInstance* AnimInstance = Cast<UAnimInstance>(Iter->Key.Get()))
		{
			...
			UE_VLOG_SEGMENT(AnimInstance, "Chimera", Display, ActorLocation, ActorLocation + StringOffset, FColor::Transparent, TEXT("%s"), StringBuilder.ToString());
		}
	}
