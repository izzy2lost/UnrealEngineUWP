...
// the ChimeraQuery_Pure will ultimately setup a motion matching query using looking for the pose history "PoseHistoryName" 
...
USTRUCT(BlueprintType, Category = "Animation|Chimera")
struct CHIMERA_API FChimeraAvailability
{
	GENERATED_BODY()
	...
	// the associated character to this FChimeraAvailability will partecipate in an interaction only if all the necessary roles gest assigned to character within BroadPhaseRadius centimeters
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category=Settings)
	float BroadPhaseRadius = 500.f;

	// the associated character to this FChimeraAvailability will not partecipate in an interaction if the motion matching search cost result is higher than MaxCost
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category=Settings)
	float MaxCost = 1000.f;
};

USTRUCT(BlueprintType, Category="Animation|Chimera")
struct CHIMERA_API FChimeraBlueprintResult
{
	GENERATED_BODY()
public:
...
UCLASS()
class UChimeraLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:

	// function publishing this character (via its AnimInstance) FChimeraAvailability to the UChimeraSubsystem,
	// FChimeraAvailability represents the character availability to partecipate in an interaction with other characters for the next frame.
	// that means there will always be one frame delay between publiching availabilities and getting a result back from ChimeraQuery_Pure!
	// 
