...
struct CHIMERA_API FChimeraAssetItem
{
	...
	// associated aniamtion for this FChimeraAssetItem
	UPROPERTY(EditAnywhere, Category = "Settings")
	TObjectPtr<UAnimationAsset> Animation;

	// associated role for this FChimeraAssetItem
	UPROPERTY(EditAnywhere, Category = "Settings")
	FName Role;

	// relative weight to the other FChimeraAssetItem::WarpingWeightRotation(s) defining which character will be rotated while warping
	// 0 - the associated character to this item will move fully to compensate the warping errors
	// > 0 && all the other FChimeraAssetItem::WarpingWeightTranslation as zero, and the associated character will not move
	UPROPERTY(EditAnywhere, Category = "Warping", meta = (ClampMin = "0", ClampMax = "1"))
	float WarpingWeightRotation = 0.5f;

	// relative weight to the other FChimeraAssetItem::WarpingWeightTranslation(s) defining which character will be translated while warping
	// 0 - the associated character to this item will move fully to compensate the warping errors
	// > 0 && all the other FChimeraAssetItem::WarpingWeightTranslation as zero, and the associated character will not move
	UPROPERTY(EditAnywhere, Category = "Warping", meta = (ClampMin = "0", ClampMax = "1"))
...
class CHIMERA_API UChimeraAsset : public UMultiAnimAsset
{
	...
	UPROPERTY(EditAnywhere, Category = "Settings")
	TArray<FChimeraAssetItem> Items;
