...
void UNiagaraScriptSource::PostLoadFromOwner(FVersionedNiagaraScriptData& OwnerData)
{
	PostLoadTransferScriptHierarchyData(OwnerData);
}
...
void UNiagaraScriptSource::PostLoadTransferScriptHierarchyData(FVersionedNiagaraScriptData& OwnerData)
{
	UNiagaraGraph* Graph = Cast<UNiagaraScriptSource>(OwnerData.GetSource())->NodeGraph;

	if(Graph == nullptr)
	{
		UE_LOG(LogNiagaraEditor, Verbose, TEXT("Failed post loading script hierarchy of versioned script data. No graph found."));
		return;
	}
	
	const int32 CustomVersion = Graph->GetLinkerCustomVersion(FNiagaraCustomVersion::GUID);

	if(CustomVersion < FNiagaraCustomVersion::HierarchyEditorScriptSupport)
	{
		UNiagaraHierarchyRoot* ParameterRoot = Graph->GetScriptParameterHierarchyRoot();
		...
		TMap<FNiagaraVariable, TObjectPtr<UNiagaraScriptVariable>> ScriptVariablesMap = Graph->GetAllMetaData();
		...
		for(const auto& ScriptVariablePair : ScriptVariablesMap)
		{
			...
			if(ScriptVariable->Metadata.GetParentAttribute_DEPRECATED().IsNone())
			{
				...
				ParentChildInputMap.Add(ScriptVariable, Graph->GetChildScriptVariablesForInput_Deprecated(ScriptVariable->Metadata.GetVariableGuid()));
			}
		}
