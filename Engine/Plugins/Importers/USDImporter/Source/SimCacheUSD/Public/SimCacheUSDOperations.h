...
namespace UE::SimCacheUSD
{
	...
	SIMCACHEUSD_API bool NewStage(const FString& StageName, UE::FUsdStage& Stage);
	/** Opens an existing stage, resident in the USD stage cache. */
	SIMCACHEUSD_API bool OpenStage(const FString& StageName, UE::FUsdStage& Stage);

	/** Save the stage and update the frame range (if not -DBL_MAX). */
	SIMCACHEUSD_API bool SaveStage(UE::FUsdStage& Stage, const double FirstFrame, const double LastFrame);

	/** Close and remove a stage from the USD stage cache. */
	SIMCACHEUSD_API bool CloseStage(const UE::FUsdStage& Stage);
	SIMCACHEUSD_API bool CloseStage(const FString& StageName);
	 ...
	 */
	SIMCACHEUSD_API bool WriteTetMesh(UE::FUsdStage& Stage, const FString& PrimPath, const FManagedArrayCollection& Collection, const int32 StructureIndex = INDEX_NONE);
	 ...
	 */
	SIMCACHEUSD_API bool WritePoints(UE::FUsdStage& Stage, const FString& PrimPath, const double Time, const FManagedArrayCollection& Collection, const int32 StructureIndex = INDEX_NONE);
	/** Write points and velocities to \c UsdGeomPointBased at \c PrimPath. */
	SIMCACHEUSD_API bool WritePoints(UE::FUsdStage& Stage, const FString& PrimPath, const double Time, pxr::VtArray<pxr::GfVec3f>& VtPoints, pxr::VtArray<pxr::GfVec3f>& VtVels);

	/** Get time samples for an attribute. */
	SIMCACHEUSD_API bool ReadTimeSamples(const UE::FUsdStage& Stage, const FString& PrimPath, const FString& AttrName, TArray<double>& TimeSamples);
	/** Get time samples for the points attribute. */
	SIMCACHEUSD_API bool ReadTimeSamples(const UE::FUsdStage& Stage, const FString& PrimPath, TArray<double>& TimeSamples);

	SIMCACHEUSD_API FString GetPointsAttrName();
	SIMCACHEUSD_API FString GetVelocityAttrName();
	SIMCACHEUSD_API bool GetBracketingTimeSamples(const UE::FUsdStage& Stage, const FString& PrimPath, const FString& AttrName, const double TargetTime, double* Lower, double* Upper);

	/** Get points from an attribute. Default time is used if \p Time is \c -DBL_MAX. */
	SIMCACHEUSD_API bool ReadPoints(const UE::FUsdStage& Stage, const FString& PrimPath, const FString& AttrPath, const double Time, pxr::VtArray<pxr::GfVec3f>& Points);
	/** Get points from the points attribute. Default time is used if \p Time is \c -DBL_MAX. */
	SIMCACHEUSD_API bool ReadPoints(const UE::FUsdStage& Stage, const FString& PrimPath, const double Time, pxr::VtArray<pxr::GfVec3f>& Points, pxr::VtArray<pxr::GfVec3f>& VtVels);
