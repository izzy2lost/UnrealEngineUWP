...
#include "UEUsdGeomTetMesh.h"
#include "SimCacheUSDUtil.h"
...
bool 
UE::SimCacheUSD::NewStage(const FString& StageName, UE::FUsdStage& UsdStage)
{
...
bool 
UE::SimCacheUSD::OpenStage(const FString& StageName, UE::FUsdStage& UsdStage)
{
	if (!FPaths::FileExists(StageName))
	{
...
bool 
UE::SimCacheUSD::SaveStage(UE::FUsdStage& Stage, const double FirstFrame, const double LastFrame)
{
...
bool
UE::SimCacheUSD::CloseStage(const UE::FUsdStage& Stage)
{
...
bool 
UE::SimCacheUSD::CloseStage(const FString& StageName)
{
	...
	for (UE::FUsdStage& Stage : CachedStages)
	{
		...
		{
			return SimCacheUSD::CloseStage(Stage);
		}
	}
	return true;
}
...
void
UE::SimCacheUSD::GenerateValueClipStageNames(
...
FString
UE::SimCacheUSD::GenerateValueClipTimeVaryingStageName(
...
bool
UE::SimCacheUSD::NewValueClipsStages(
	const FString& ParentStageName,
	const FString& TopologyStageName,
	UE::FUsdStage& ParentStage,
	UE::FUsdStage& TopologyStage)
{
	if (!SimCacheUSD::NewStage(ParentStageName, ParentStage)) return false;
	if (!SimCacheUSD::NewStage(TopologyStageName, TopologyStage)) return false;
	return true;
}

bool
UE::SimCacheUSD::NewValueClipsFrameStage(
...
{
	FrameStageName =
		SimCacheUSD::GenerateValueClipTimeVaryingStageName(TimeVaryingStageTemplate, Time);
	return SimCacheUSD::NewStage(FrameStageName, FrameStage);
}

bool 
UE::SimCacheUSD::InitValueClipsTemplate(
	UE::FUsdStage& ParentStage,
	UE::FUsdStage& TopologyStage,
	const FString& ParentStageName,
	const FString& TopologyStageName,
...
{
	UE::FUsdStage ParentStage;
	if (!SimCacheUSD::OpenStage(ParentStageName, ParentStage)) return false;
	UE::FUsdStage TopologyStage;
	if (!SimCacheUSD::OpenStage(TopologyStageName, TopologyStage)) return false;

	return SimCacheUSD::InitValueClipsTemplate(
		ParentStage,
		TopologyStage,
		ParentStageName,
		TopologyStageName,
	...
	const int32 StructureIndex)
{
	...
	if (!UE::SimCacheUSD::WritePoints(Stage, PrimPath, -TNumericLimits<double>::Max(), Collection, StructureIndex))
	{
	...
	UE::SimCacheUSD::SaveStage(Stage, -TNumericLimits<double>::Max(), -TNumericLimits<double>::Max());
	return true;
}

bool
UE::SimCacheUSD::WritePoints(
	UE::FUsdStage& Stage,
	const FString& PrimPath,
	const double Time,
	...
	pxr::VtArray<pxr::GfVec3f>& VtVels)
{
	...
	if (!Prim)
	{
		...
		UE::SimCacheUSDUtil::Private::DefineAncestorTransforms(Stage, Path);

		return UE::SimCacheUSDUtil::Private::SetPointsExtentAndPrimXform(
...
bool 
UE::SimCacheUSD::ReadTimeSamples(
...
UE::SimCacheUSD::ReadTimeSamples(
	...
	TArray<double>& TimeSamples)
{
	return SimCacheUSD::ReadTimeSamples(Stage, PrimPath, FString(pxr::UsdGeomTokens->points.GetText()), TimeSamples);
}

FString
UE::SimCacheUSD::GetPointsAttrName()
{
	FScopedUnrealAllocs UEAllocs; // Use UE memory allocator
...
FString
UE::SimCacheUSD::GetVelocityAttrName()
{
	FScopedUnrealAllocs UEAllocs; // Use UE memory allocator
...
UE::SimCacheUSD::ReadPoints(
	...
	pxr::VtArray<pxr::GfVec3f>& VtVels)
{
	SimCacheUSD::ReadPoints(Stage, PrimPath, FString(pxr::UsdGeomTokens->velocities.GetText()), Time, VtVels);
	return SimCacheUSD::ReadPoints(Stage, PrimPath, FString(pxr::UsdGeomTokens->points.GetText()), Time, VtPoints);
}
