...
public:
	...
	FTestPrerequisitesImpl()
		: FTestPrerequisites()
	{
		SocketSubsystem = reinterpret_cast<FOnlineBeaconUnitTestSocketSubsystem*>(ISocketSubsystem::Get(SocketSubsystemName));
	...
	FOnlineBeaconUnitTestSocketSubsystem* SocketSubsystem = nullptr;
...
bool ConfigureBeaconNetDriver(const FTestPrerequisites& Prerequisites, AOnlineBeacon* OnlineBeacon, TSharedPtr<FNetworkStats>* OutStats)
{
	...
	const BeaconUnitTest::FTestConfig TestConfig = Prerequisites.GetConfig();
	NetDriver->bNoTimeouts = TestConfig.NetDriver.bNoTimeouts;
