...
namespace BeaconUnitTest
{
	...
	public:
		...
		static ONLINESUBSYSTEMUTILSTEST_API TSharedPtr<FTestPrerequisites> TryCreate();
		static ONLINESUBSYSTEMUTILSTEST_API TSharedPtr<FTestPrerequisites> Get();

		static ONLINESUBSYSTEMUTILSTEST_API FTestStats* GetActiveTestStats();
		static ONLINESUBSYSTEMUTILSTEST_API const FTestConfig* GetActiveTestConfig();
	 ...
	 */
	ONLINESUBSYSTEMUTILSTEST_API bool SetSocketFlags(AOnlineBeacon* OnlineBeacon, ESocketFlags Flags);

	ONLINESUBSYSTEMUTILSTEST_API bool ConfigureBeacon(const FTestPrerequisites& Prerequisites, AOnlineBeacon* OnlineBeacon);

	ONLINESUBSYSTEMUTILSTEST_API bool ConfigureBeaconNetDriver(const FTestPrerequisites& Prerequisites, AOnlineBeacon* OnlineBeacon, TSharedPtr<FNetworkStats>* OutStats);

	/**
	 * Helper to initialize the connection for a beacon client and overriding the default UserId.
	 */
	ONLINESUBSYSTEMUTILSTEST_API bool InitClientForUser(const FTestPrerequisites& Prerequisites, AOnlineBeaconClient* OnlineBeaconClient, const FUniqueNetIdRef& User);

	/**
	 * Helper to get the hosts version of the client beacon actor for the specified user.
	 */
	ONLINESUBSYSTEMUTILSTEST_API AOnlineBeaconClient* GetBeaconClientForUser(AOnlineBeaconHostObject* HostObject, const FUniqueNetIdRef& User);


	/**
	 * Tick the unit test world one time to progress network state.
	 */
	ONLINESUBSYSTEMUTILSTEST_API bool TickOnce(const FTestPrerequisites& Prerequisites, ETickFlags Flags = ETickFlags::None);

	/**
	 * Tick the unit test world until the beacon client connection has completed the packet handler handshake.
	 */
	ONLINESUBSYSTEMUTILSTEST_API bool TickUntilConnectionInitialized(const FTestPrerequisites& Prerequisites, AOnlineBeaconClient* OnlineBeaconClient, ETickFlags Flags = ETickFlags::None);

	/**
	 * Tick the unit test world until the beacon client has changed its status to 'Closed' or 'Invalid'.
	 */
	ONLINESUBSYSTEMUTILSTEST_API bool TickUntilControlMessageReceived(const FTestPrerequisites& Prerequisites, const FNetworkStats& Stats, ETickFlags Flags = ETickFlags::None);

	/**
	 * Tick the unit test world until the beacon client has changed its status to 'Open'.
	 */
	ONLINESUBSYSTEMUTILSTEST_API bool TickUntilConnected(const FTestPrerequisites& Prerequisites, AOnlineBeaconClient* OnlineBeaconClient, ETickFlags Flags = ETickFlags::None);

	/**
	 * Tick the unit test world until the beacon client has changed its status to 'Closed' or 'Invalid'.
	 */
	ONLINESUBSYSTEMUTILSTEST_API bool TickUntilDisconnected(const FTestPrerequisites& Prerequisites, AOnlineBeaconClient* OnlineBeaconClient, ETickFlags Flags = ETickFlags::None);

	/**
	 * Tick the unit test world until the network timeout has elapsed.
	 */
	ONLINESUBSYSTEMUTILSTEST_API bool TickUntilTimeoutElapsed(const FTestPrerequisites& Prerequisite, ETickFlags Flags = ETickFlags::None);
