...
TArray<UFootageCaptureData::FPathAssociation> UFootageCaptureData::CheckImageSequencePaths() const
{
	...
	for (const FFootageCaptureView& View : Views)
	{
		const UImgMediaSource* ImageSequence = View.ImageSequence;
...
{
	CHECK_AND_PRINT_RET(!Views.IsEmpty(), TEXT("Capture Data doesn't contain views"));
	...
	for (const FFootageCaptureView& View : Views)
	{
		...
		CHECK_AND_PRINT_RET(FImageSequenceUtils::GetImageSequenceInfo(View.ImageSequence, ImageDimensions, NumImageFrames), TEXT("Image Sequence asset is invalid"));

		int32 NumDepthFrames = 0;
		FIntVector2 DepthDimensions;

		CHECK_AND_PRINT_RET(FImageSequenceUtils::GetImageSequenceInfo(View.DepthSequence, DepthDimensions, NumImageFrames), TEXT("Depth Sequence asset is invalid"));

		CHECK_AND_PRINT_RET(View.ImageSequence->FrameRateOverride.IsValid(), TEXT("Image Sequence asset contains invalid frame rate"));
		CHECK_AND_PRINT_RET(View.DepthSequence->FrameRateOverride.IsValid(), TEXT("Depth Sequence asset contains invalid frame rate"));
		CHECK_AND_PRINT_RET(View.ImageSequence->FrameRateOverride == View.DepthSequence->FrameRateOverride, TEXT("Image Sequence asset and Depth Sequence asset contain different frame rates"));
	}
...
void UFootageCaptureData::GetFrameRanges(const FFrameRate& InTargetRate, ETimecodeAlignment InTimecodeAlignment, bool bInIncludeAudio, TMap<TWeakObjectPtr<UObject>, TRange<FFrameNumber>>& OutMediaFrameRanges, TRange<FFrameNumber>& OutProcessingFrameRange, TRange<FFrameNumber>& OutMaximumFrameRange) const
{
	...
	{
		OutMediaFrameRanges.Add(Views[Index].ImageSequence, GetFrameRange(InTargetRate, Views[Index].ImageSequence, GetEffectiveImageTimecode(Index), GetEffectiveImageTimecodeRate(Index), InTimecodeAlignment == ETimecodeAlignment::None));
		OutMediaFrameRanges.Add(Views[Index].DepthSequence, GetFrameRange(InTargetRate, Views[Index].DepthSequence, GetEffectiveDepthTimecode(Index), GetEffectiveDepthTimecodeRate(Index), InTimecodeAlignment == ETimecodeAlignment::None));
	}
...
{
	check(InView < Views.Num());

	FTimecode Timecode;

	if (Views[InView].bImageTimecodePresent)
	{
		Timecode = Views[InView].ImageTimecode;
	}
	else if (Views[InView].bDepthTimecodePresent)
	{
		Timecode = Views[InView].DepthTimecode;
	}
	else
	{
...
FFrameRate UFootageCaptureData::GetEffectiveImageTimecodeRate(int32 InView) const
{
	...
	if (Views[InView].bImageTimecodePresent)
	{
		FrameRate = Views[InView].ImageTimecodeRate;
	}
	else if (Views[InView].bDepthTimecodePresent)
	{
		FrameRate = Views[InView].DepthTimecodeRate;
	}
	else
	{
		FTimecode Timecode;
...
FTimecode UFootageCaptureData::GetEffectiveDepthTimecode(int32 InView) const
{
	...
	if (Views[InView].bDepthTimecodePresent)
	{
		Timecode = Views[InView].DepthTimecode;
	}
	else if (Views[InView].bImageTimecodePresent)
	{
		Timecode = Views[InView].ImageTimecode;
	}
	else
...
void UFootageCaptureData::GetDefaultTimecodeInfo(FTimecode& OutTimecode, FFrameRate& OutFrameRate) const
{
	for (const FFootageCaptureView& View : Views)
	{
		...
		{
			OutTimecode = View.ImageTimecode;
			OutFrameRate = View.ImageTimecodeRate;
			return;
		}
	}
	...
	for (const FFootageCaptureView& View : Views)
	{
		...
		{
			OutTimecode = View.DepthTimecode;
			OutFrameRate = View.DepthTimecodeRate;
			return;
		}
	}
