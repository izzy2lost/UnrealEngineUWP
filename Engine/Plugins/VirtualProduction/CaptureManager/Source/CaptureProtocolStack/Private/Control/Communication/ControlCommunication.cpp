...
void FControlCommunication::OnProcessSentPacket(FControlPacket InMessage)
{
    ...
    {
        UE_LOG(LogCPSControlCommunication, Error, TEXT("Invalid message: %s"), *(SerializeResult.StealError().GetMessage()));
        return;
    }
}
...
uint32 FControlCommunication::FCommunicationRunnable::Run()
{
	while (bIsRunning)
	{
		...
		{
			FCaptureProtocolError Error = ReceiveHeaderResult.StealError();
		...
		{
			FCaptureProtocolError Error = ReceivePacketResult.StealError();
			if (Error.GetCode() == FTcpClient::DisconnectedError)
			{
				Stop();
			}
