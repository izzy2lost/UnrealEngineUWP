...
public:
	...
	void operator()(Args... InArgs)
	{
		if (Delegate.IsBound())
		{
			UE::CaptureManager::Private::ExecuteDelegate(Delegate, ExecutionThread, Forward<Args>(InArgs)...);
		}
	}
...
public:
	...
	void Add(DelegateType InDelegate, EDelegateExecutionThread InThread = EDelegateExecutionThread::GameThread)
	{
		...
		{
			UE::CaptureManager::Private::ExecuteDelegate(InDelegate, InThread, Forward<Args>(InArgs)...);
		});
	}
