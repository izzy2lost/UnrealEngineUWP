...
void SDMXPixelMappingOutputComponentText::Construct(const FArguments& InArgs, const TSharedRef<FDMXPixelMappingToolkit>& InToolkit, TWeakObjectPtr<UDMXPixelMappingOutputComponent> OutputComponent)
{
	...
	SetRenderTransform(TAttribute<TOptional<FSlateRenderTransform>>::CreateSP(this, &SDMXPixelMappingOutputComponentText::GetRenderTransform));
...
TOptional<FSlateRenderTransform> SDMXPixelMappingOutputComponentText::GetRenderTransform() const
{
	const FQuat2D Quaternion = Model->GetQuaternion();

	// The text always should face upwards to be readable. Hence,
	// if the rotation goes > 90° or < -90°, draw the widget inverted.
	if (Quaternion.GetVector().X < 0.0)
	{
		return FSlateRenderTransform(FQuat2D(UE_PI), FVector2D::ZeroVector);
	}
...
void SDMXPixelMappingOutputComponentText::OnPaintComponentName(const FPaintArgs& Args, const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FVector2f& Scale) const
{
	...
	const float ScaledFontSize = FontSize / Scale.Y;

	const float MaxFontSize = LocalSize.Y - ScaledFontSize - Margin * 2.f;
	const float DesiredFontSize = ScaledFontSize < MaxFontSize ? ScaledFontSize : MaxFontSize;
	const float ClampedFontSize = FMath::Max(UE_DELTA, DesiredFontSize);

	const float OffsetX = Margin;
	const float OffsetY = Model->ShouldDrawNameAbove() ? -ScaledFontSize - Margin : Margin;
...
void SDMXPixelMappingOutputComponentText::OnPaintPatchInfo(const FPaintArgs& Args, const FGeometry& AllottedGeometry, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FVector2f& Scale) const
{
	...
	// Find a font size that fits in the lower third of this section. Consider two lines of text are painted
