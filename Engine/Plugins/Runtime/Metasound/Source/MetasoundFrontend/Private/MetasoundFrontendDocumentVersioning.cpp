...
namespace Metasound::Frontend
{
	...
	bool FVersionDocument::Transform(FDocumentHandle InHandle) const
	{
		return false;
	}

	bool FVersionDocument::Transform(TScriptInterface<IMetaSoundDocumentInterface> DocumentInterface) const
	{
		bool bWasUpdated = false;

		const IMetaSoundDocumentInterface* ConstInterface = DocumentInterface.GetInterface();
		const FMetasoundFrontendDocument& Document = ConstInterface->GetConstDocument();
		...
		if (InitVersionNumber.Major == 1 && InitVersionNumber.Minor < 12)
		{
			...
			FMetasoundAssetBase* Asset = AssetManager.GetAsAsset(*DocObject);
			FDocumentHandle DocHandle = Asset->GetDocumentHandle();
		...
		// Builder Transforms
		{
			...
			UObject* DocObject = DocumentInterface.GetObject();
			check(DocObject);
			FMetaSoundFrontendDocumentBuilder& Builder = AssetManager.AttachDocumentBuilderChecked(*DocObject);
