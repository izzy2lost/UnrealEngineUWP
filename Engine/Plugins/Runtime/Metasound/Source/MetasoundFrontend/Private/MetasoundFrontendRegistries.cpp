	...
	namespace Frontend
	{
		...
		FNodeRegistryKey FRegistryContainerImpl::RegisterGraph(const FNodeClassInfo& InNodeClassInfo, const TScriptInterface<IMetaSoundDocumentInterface>& InDocumentInterface, bool bAsync)
		{
			...
			{	
				...
				ActiveRegistrationTasks.Add(RegistryKey, FActiveRegistrationTaskInfo{BuildAndRegisterTask, InDocumentInterface.GetObject()});
			}
			else
			{
...
#if WITH_ENGINE
		void FRegistryContainerImpl::AddReferencedObjects(FReferenceCollector& Collector)
		{
			// Inflight IMetaSoundDOucmentInterface objects are protected from GC while their
			// async tasks or being processed. 
			FScopeLock LockActiveReg(&ActiveRegistrationTasksCriticalSection);
			for (TPair<FNodeRegistryKey, FActiveRegistrationTaskInfo>& Pair : ActiveRegistrationTasks)
			{
				if (Pair.Value.OwningObject)
				{
					Collector.AddReferencedObject(Pair.Value.OwningObject);
				}
			}
		}

		FString FRegistryContainerImpl::GetReferencerName() const
		{
			return TEXT("Metasound::Frontend::FRegistryContainerImpl");
		}
