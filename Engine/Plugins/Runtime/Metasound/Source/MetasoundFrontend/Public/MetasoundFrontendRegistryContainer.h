...
public:
	...
	virtual FNodeRegistryKey RegisterGraph(const TScriptInterface<IMetaSoundDocumentInterface>& InDocument, bool bAsync = true) = 0;

	// Wait for async graph registration to complete for a specific graph
	virtual void WaitForAsyncGraphRegistration(const FNodeRegistryKey& InRegistryKey, const FTopLevelAssetPath& InAssetPath) const = 0;

	// Unregister an asset based graph from the frontend.
	virtual bool UnregisterGraph(const TScriptInterface<IMetaSoundDocumentInterface>& InDocument, bool bAsync = true) = 0;
	...
	virtual TSharedPtr<const Metasound::FGraph> GetGraph(const FNodeRegistryKey& InRegistryKey, const FTopLevelAssetPath& InAssetPath) const = 0;
	...
	virtual bool IsGraphRegistered(const FNodeRegistryKey& InKey, const FTopLevelAssetPath& InAssetPath) const = 0;
