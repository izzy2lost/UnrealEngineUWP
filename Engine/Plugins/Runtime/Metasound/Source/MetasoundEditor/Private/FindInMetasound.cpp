...
namespace Metasound::Editor
{
	...
	FString FFindInMetasoundResult::GetValueText()
	{
		...
		if (const UMetasoundEditorGraphMember* Member = GetMetaSoundGraphMember(MemberNode))
		{
			...
			if (Member->GetDataType() != GetMetasoundDataTypeName<FTrigger>())
			{
				...
				ValueText = DefaultLiteral.ToString();
				if (DefaultLiteral.IsArray())
				{
					ValueText = FString::Printf(TEXT("(%u element array)"), DefaultLiteral.GetArrayNum());
				}
		...
		else if (ResolvedPin)
		{
			...
			if (bHasCachedValue && bIsUnconnectedInputPin)
			{
				if (ResolvedPin->DefaultObject)
				{
					...
					ValueText = FString::Printf(TEXT("(%s)"), DefaultActor ? *DefaultActor->GetActorLabel() : *ResolvedPin->DefaultObject->GetName());
				}
				else if (!ResolvedPin->DefaultValue.IsEmpty())
				{
					...
					{
						ValueText = FString::Printf(TEXT("%s"), *ResolvedPin->DefaultValue);
					}
				}
				else if (!ResolvedPin->AutogeneratedDefaultValue.IsEmpty())
				{
					ValueText = FString::Printf(TEXT("%s"), *ResolvedPin->AutogeneratedDefaultValue);
				}
				else if (!ResolvedPin->DefaultTextValue.IsEmpty())
				{
					ValueText = FString::Printf(TEXT("%s"), *ResolvedPin->DefaultTextValue.ToString());
				}
		...
		{
			ValueText = FString::Printf(TEXT("(overridden) %s"), *ValueText);
		}
		return ValueText;
	}

	FString FFindInMetasoundResult::GetValueTooltipText()
	{
		...
		if (const UMetasoundEditorGraphMember* Member = GetMetaSoundGraphMember(MemberNode))
		{
			...
			const FMetasoundFrontendLiteral& DefaultLiteral = Member->GetLiteral()->GetDefault();
			return DefaultLiteral.ToString();
		}
			...
			[
				...
				+ SHorizontalBox::Slot()
					...
					[
						...
						.HighlightText(HighlightText)
						.ToolTipText(FText::FromString(InItem->GetValueTooltipText()))
					]
				+ SHorizontalBox::Slot()
					.AutoWidth()
					.HAlign(HAlign_Right)
