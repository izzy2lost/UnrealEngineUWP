...
// EmmettJnr: Updated for UE5 and added device detection thread
...
void pollIndividualLoop(JoyShock *jc) {
	...
	while (!jc->cancel_thread) {
		...
		{
			// disconnected!
			UE_LOG(LogJoyShockLib, Display, TEXT("Controller %d disconnected"), jc->intHandle);
		...
		if (res == 0)
		{
			...
			if (numTimeOuts >= 10)
			{
				UE_LOG(LogJoyShockLib, Display, TEXT("Controller timed out"), jc->intHandle);
			...
			else
			{
				...
				else
				{
					if (jc->is_usb)
					{
						UE_LOG(LogJoyShockLib, Display, TEXT("Attempting to re-initialise controller %d"), jc->intHandle);
						if (jc->init_usb())
						{
							numTimeOuts = 0;
						}
	...
	if (jc->cancel_thread)
	{
		UE_LOG(LogJoyShockLib, Display, TEXT("Ending cancelled thread"));
	}
	else
	{
		UE_LOG(LogJoyShockLib, Display, TEXT("Timing out thread"));
	}

	// remove
	if (jc->remove_on_finish)
	{
		UE_LOG(LogJoyShockLib, Display, TEXT("Removing device"));
		if (!lockedThread)
		{
			_connectedLock.lock();
		}
	...
	{
		UE_LOG(LogJoyShockLib, Display, TEXT("Deleting device"));
		delete jc;
	}
...
void JslConnectDevices()
{
	...
	while (cur_dev) {
		...
		if (handle != nullptr)
		{
			UE_LOG(LogJoyShockLib, Display, TEXT("Opened new handle"));
			if (isSameController)
			{
				UE_LOG(LogJoyShockLib, Display, TEXT("Reconnecting controller %d"), currentJc->intHandle);
			...
			{
				UE_LOG(LogJoyShockLib, Display, TEXT("New controller detected"));
			...
			{
				UE_LOG(LogJoyShockLib, Display, TEXT("Deinitialising old controller"));
				...
				UE_LOG(LogJoyShockLib, Display, TEXT("Ending old thread"));
				std::thread* thread = currentJc->thread;
	...
	for (std::pair<int, JoyShock*> pair : _joyshocks)
	{
		...
		{
			UE_LOG(LogJoyShockLib, Display, TEXT("Starting new thread"));
...
void JslSetLightColour(int deviceId, unsigned char colourR, unsigned char colourG, unsigned char colourB)
{
	std::shared_lock<std::shared_timed_mutex> lock(_connectedLock);
	JoyShock* jc = GetJoyShockFromHandle(deviceId);
