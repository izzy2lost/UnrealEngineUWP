...
#define MAX_NUM_JOYSHOCK_CONTROLLERS 16

/** Max number of controller buttons.  Must be < 256*/
#define MAX_NUM_CONTROLLER_BUTTONS 24

/**
 * Interface class for JoyShock devices (DS4, DS, Switch)
 */
class JoyShockInterface : public IInputDevice
{
public:
	static TSharedRef<JoyShockInterface> Create(const TSharedRef<FGenericApplicationMessageHandler>& InMessageHandler, bool bShouldBePrimaryDevice);
...
private:
	JoyShockInterface(const TSharedRef<FGenericApplicationMessageHandler>& InMessageHandler, bool bShouldBePrimaryDevice);
	void RefreshConnectedControllers();
	 ...
	 */
	void GetPlatformUserAndDevice(int32 InControllerId, EInputDeviceConnectionState InDeviceState, FPlatformUserId& OutPlatformUserId, FInputDeviceId& OutDeviceId) const;
	...
	uint8	JoyShockToXboxControllerMapping[MAX_NUM_CONTROLLER_BUTTONS];

	/** Controller states */
	FControllerState ControllerStates[MAX_NUM_JOYSHOCK_CONTROLLERS];
