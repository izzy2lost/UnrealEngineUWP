...
void JoyShock::init(hid_device_info* dev, hid_device* inHandle, int uniqueHandle, const std::string& inPath)
{
	this->path = inPath;
...
JoyShock::JoyShock(hid_device_info* dev, hid_device* inHandle, int uniqueHandle, const std::string& inPath) {
...
IMU_STATE JoyShock::get_transformed_imu_state(IMU_STATE& imu_state)
{
...
bool JoyShock::get_switch_controller_info() {
	...
	UE_LOG(LogJoyShockLib, Display, TEXT("Body: %#08x; Buttons: %#08x; Left Grip: %#08x; Right Grip: %#08x;"),
		body_colour,
		button_colour,
		left_grip_colour,
		right_grip_colour);
...
void JoyShock::enable_IMU(unsigned char* buf, int bufLength) {
	...
	// Enable IMU data
	UE_LOG(LogJoyShockLib, Display, TEXT("Enabling IMU data..."));
...
bool JoyShock::init_usb() {
	...
	//Get MAC Left
	UE_LOG(LogJoyShockLib, Display, TEXT("Getting MAC..."));
	...
	// Do handshaking
	UE_LOG(LogJoyShockLib, Display, TEXT("Doing handshake...")); 
	memset(buf, 0x00, 0x40);
	buf[0] = 0x80;
	buf[1] = 0x02;
	hid_exchange(this->handle, buf, 0x2);
	...
	UE_LOG(LogJoyShockLib, Display, TEXT("Switching baudrate..."));
	memset(buf, 0x00, 0x40);
	buf[0] = 0x80;
	buf[1] = 0x03;
	hid_exchange(this->handle, buf, 0x2);
	...
	UE_LOG(LogJoyShockLib, Display, TEXT("Doing handshake again..."));
	memset(buf, 0x00, 0x40);
	buf[0] = 0x80;
	buf[1] = 0x02;
	hid_exchange(this->handle, buf, 0x2);
	...
	UE_LOG(LogJoyShockLib, Display, TEXT("Only talk HID..."));
	memset(buf, 0x00, 0x40);
	buf[0] = 0x80;
	buf[1] = 0x04;
	hid_exchange(this->handle, buf, 0x2);
	...
	// Enable vibration
	UE_LOG(LogJoyShockLib, Display, TEXT("Enabling vibration..."));
	...
	UE_LOG(LogJoyShockLib, Display, TEXT("Getting calibration data..."));
	bool result = get_switch_controller_info();

	if (result)
	{
		UE_LOG(LogJoyShockLib, Display, TEXT("Successfully initialized %s!"), *FString(this->name.c_str()));
	}
	else
	{
		UE_LOG(LogJoyShockLib, Error, TEXT("Could not initialise %s! Will try again later."), *FString(this->name.c_str()));
	}
	return result;
}
...
bool JoyShock::init_bt() {
	...
	unsigned char buf[0x40];
	memset(buf, 0, 0x40);
	UE_LOG(LogJoyShockLib, Display, TEXT("Initialising Bluetooth connection..."));
	...
	for (int idx = 0; idx < 5; idx++)
	{
		if (hid_read_timeout(this->handle, buf, 0x40, 200) && buf[0] == 0x81)
		{
			...
			UE_LOG(LogJoyShockLib, Display, TEXT("Attempting USB connection"));
		...
		//printf("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
		//	buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7], buf[8], buf[9], buf[10]);
		UE_LOG(LogJoyShockLib, Display, TEXT("Not a USB response..."));
	}
	memset(buf, 0, 0x40);
	...
	UE_LOG(LogJoyShockLib, Display, TEXT("Set input report mode to 0x30..."));
	buf[0] = 0x30;
...
void JoyShock::init_ds4_bt() {
	UE_LOG(LogJoyShockLib, Display, TEXT("Initialising DS4 Bluetooth connection..."));
...
void JoyShock::set_ds5_rumble_light(unsigned char smallRumble, unsigned char bigRumble, unsigned char colourR, unsigned char colourG, unsigned char colourB, unsigned char playerlights) {
	if (!is_usb) {
...
void JoyShock::set_ds4_rumble_light(unsigned char smallRumble, unsigned char bigRumble, unsigned char colourR, unsigned char colourG, unsigned char colourB) {
	if (!is_usb) {
...
void JoyShock::set_ds4_rumble_light_usb(unsigned char smallRumble, unsigned char bigRumble, unsigned char colourR, unsigned char colourG, unsigned char colourB) {
...
void JoyShock::set_ds4_rumble_light_bt(unsigned char smallRumble, unsigned char bigRumble, unsigned char colourR, unsigned char colourG, unsigned char colourB) {
...
void JoyShock::set_ds5_rumble_light_usb(unsigned char smallRumble, unsigned char bigRumble, unsigned char colourR, unsigned char colourG, unsigned char colourB, unsigned char playerlights) { // DS5 actually has player lights.
	unsigned char buf[79];
	memset(buf, 0, 79);
...
void JoyShock::set_ds5_rumble_light_bt(unsigned char smallRumble, unsigned char bigRumble, unsigned char colourR, unsigned char colourG, unsigned char colourB, unsigned char playerlights) { // DS5 actually has player lights.
	unsigned char buf[79];
	memset(buf, 0, 79);
...
void JoyShock::CalcAnalogStick2(float& pOutX, float& pOutY, uint16_t x, uint16_t y, uint16_t x_calc[3], uint16_t y_calc[3])
{
...
int JoyShock::write_spi_data(uint32_t offset, const uint8_t write_len, uint8_t* test_buf) {
	int res;
	uint8_t buf[0x100];
	int error_writing = 0;
	while (1) {
