...
public:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "GeometryScript|ListUtils", meta=(ScriptMethod))
	static void ConvertTriangleListToArray(FGeometryScriptTriangleList TriangleList, TArray<FIntVector>& TriangleTupleArray);

	/**
	* Converts a Triangle Array of integer triplets to a Triangle List.
	*/
	UFUNCTION(BlueprintCallable, Category = "GeometryScript|ListUtils")
	static void ConvertArrayToTriangleList(const TArray<FIntVector>& TriangleTupleArray, FGeometryScriptTriangleList& TriangleList);
