...
TPair<UEdGraph*, UK2Node*> CreateGraph(UBlueprint* Blueprint, FName GraphName, const UFunction* Signature, const UFunction* FunctionToWrap, bool bIsConst, bool bTransient)
{
	...
	{
		UEdGraphPin* FunctionEntryThenPin = FunctionEntry->GetThenPin();
...
TPair<UEdGraph*, UK2Node*> CreateGraph(UBlueprint* Blueprint, FName GraphName, const UFunction* Signature, TSubclassOf<UK2Node> NodeType, bool bIsConst, bool bTransient, TFunctionRef<void(UK2Node*)> InitNodeCallback)
{
	...
	{
		FGraphNodeCreator<UK2Node> CallFunctionCreator(*FunctionGraph);
...
void SetPropertyPathForPin(const UBlueprint* Blueprint, const FMVVMBlueprintPropertyPath& Path, UEdGraphPin* PathPin)
{
	...
	// Link Then / Exec pin
	{
		...
		if (UEdGraphPin* CallFunctionExecPin = ConversionNode->FindPin(UEdGraphSchema_K2::PN_Execute))
		{
			ThenPin->MakeLinkTo(CallFunctionExecPin);
		}
		else
		{
			UEdGraphPin* FunctionResultExecPin = ConverionFunctionResult->GetExecPin();
