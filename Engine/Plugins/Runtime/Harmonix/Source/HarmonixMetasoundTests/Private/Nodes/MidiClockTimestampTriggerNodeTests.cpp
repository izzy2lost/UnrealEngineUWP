...
namespace HarmonixMetasoundTests::MidiClockTimestampTriggerNode
{
	...
	IMPLEMENT_SIMPLE_AUTOMATION_TEST(
		FMidiClockTimestampTriggerCreateNodeTest,
		"Harmonix.Metasound.Nodes.MidiClockTimestampTriggerNode.CreateNode",
		EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
		bool FMidiClockTimestampTriggerCreateNodeTest::RunTest(const FString&)
	{
		// Build the graph.
		constexpr int32 NumSamplesPerBlock = 256;
		const TUniquePtr<FMetasoundGenerator> Generator = GraphBuilder::MakeSingleNodeGraph(
			{ HarmonixMetasound::HarmonixNodeNamespace, "MidiClockTimestampTrigger", "" },
			0,
			48000,
			NumSamplesPerBlock);
