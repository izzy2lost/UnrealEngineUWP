...
struct FPCGWorldPartitionBuilder
{
	...
	static bool GenerateComponents(TArray<TWeakObjectPtr<UPCGComponent>>& Components, UWorld* InWorld, const FPCGWorldPartitionBuilderArgs& InArgs, TSet<FString>& InOutDeletedActorPackages, bool& bOutGenerationErrors);

	/** Generate a component. Applies correct editing mode if necessary. */
	static void GenerateComponent(UPCGComponent* InComponent, UWorld* InWorld);
...
bool UPCGWorldPartitionBuilder::RunEntireWorld(UWorld* World, FPackageSourceControlHelper& PackageHelper)
{
	...
	for (TArray<TWeakObjectPtr<UPCGComponent>>& GroupToGenerate : GroupsToGenerate)
	{
		...
		bGeneratedAnyComponent |= FPCGWorldPartitionBuilder::GenerateComponents(GroupToGenerate, World, Args, DeletedActorPackages, bErrorsOccurred);
...
bool UPCGWorldPartitionBuilder::RunPerCell(UWorld* InWorld, const FCellInfo& InCellInfo, FPackageSourceControlHelper& PackageHelper)
{
	...
	auto GenerateComponentFilter = [&InCellInfo, &ExpandedBounds, this](const UPCGComponent* InComponent)
	{
		...
		if (!InCellInfo.Bounds.IsInsideOrOnXY(ActorBounds.GetCenter()))
		{
			return false;
		}
	...
	if (ComponentsToGenerate.Num() > 0)
	{
		...
		for (TArray<TWeakObjectPtr<UPCGComponent>>& GroupToGenerate : GroupsToGenerate)
		{
			bCellHasChanges |= FPCGWorldPartitionBuilder::GenerateComponents(GroupToGenerate, InWorld, Args, CellDeletedPackageNames, bErrorsOccurred);
			bErrorOccurredWhileGenerating |= bErrorsOccurred;
		}
	}
	...
	bool& bOutGenerationErrors)
{
	...
	for (TWeakObjectPtr<UPCGComponent> ComponentWeakPtr : Components)
	{
		...
		FPCGWorldPartitionBuilder::GenerateComponent(Component, InWorld);
...
void FPCGWorldPartitionBuilder::GenerateComponent(UPCGComponent* InComponent, UWorld* InWorld)
{
...
bool FPCGWorldPartitionBuilder::CreatePartitionedActors(UWorld* InWorld, const FPCGWorldPartitionBuilderArgs& InArgs, TMap<TSoftObjectPtr<UPCGComponent>, FWorldPartitionReference>& InOutPartitionedComponents)
{
	...
	for (TWeakObjectPtr<UPCGComponent> ComponentPtr : PartitionedComponentsToProcess)
	{
		if (UPCGComponent* PCGComponent = ComponentPtr.Get())
		{
			...
			ensure(PCGHelpers::GetGenerationGridSizes(PCGComponent->GetGraph(), PCGSubsystem->GetPCGWorldActor(), GridSizes, bHasUnbounded));
