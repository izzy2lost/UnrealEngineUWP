...
protected:
	...
	virtual void CustomizeDetails(IDetailLayoutBuilder& InDetailBuilder) override
	{
		...
		// Hide the original assignments property (since it presents an asset picker by default) for both jobs and shots
		const TSharedRef<IPropertyHandle> JobAssignmentsProperty =
			InDetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UMoviePipelineExecutorJob, VariableAssignments));
		const TSharedRef<IPropertyHandle> ShotAssignmentsProperty =
			InDetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UMoviePipelineExecutorShot, VariableAssignments));
		const TSharedRef<IPropertyHandle> ShotPrimaryGraphAssignmentsProperty =
			InDetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UMoviePipelineExecutorShot, PrimaryGraphVariableAssignments));
		...
		if (bIsShot)
		{
			AddVariableAssignments(SelectedShots[0]->VariableAssignments, ShotGraphVariablesCategory);
			AddVariableAssignments(SelectedShots[0]->PrimaryGraphVariableAssignments, PrimaryGraphVariablesShotOverridesCategory);
		}
		else
		{
			AddVariableAssignments(SelectedJobs[0]->VariableAssignments, PrimaryGraphVariablesCategory);
		}
	}
	//~ End IDetailCustomization interface

private:
	void AddVariableAssignments(TArray<TObjectPtr<UMovieJobVariableAssignmentContainer>>& VariableAssignments, IDetailCategoryBuilder& InCategory)
	{
