...
public:
	...
	/**
	 * Overrides on the variables in graph presets associated with this job. A job can have multiple graphs associated with it if the job's assigned
	 * graph contains subgraphs. The job's graph and each subgraph will have an entry in this array.
	 */
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Instanced, Category = "Variable Assignments")
	TArray<TObjectPtr<UMovieJobVariableAssignmentContainer>> VariableAssignments;
