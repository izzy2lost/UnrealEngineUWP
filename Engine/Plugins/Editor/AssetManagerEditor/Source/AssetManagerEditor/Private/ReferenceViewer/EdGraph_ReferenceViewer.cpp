...
TArray<FReferencingPropertyDescription> UEdGraph_ReferenceViewer::RetrieveReferencingProperties(UObject* InReferencer, UObject* InReferencedAsset)
{
	...
	for (FReferenceChainSearch::FReferenceChain* ObjectRefChain : ReferenceChainSearch.GetReferenceChains())
	{
		for (int32 NodeIndex = 0; NodeIndex < ObjectRefChain->Num(); ++NodeIndex)
		{
			...
			// Let's skip the referenced asset itself
			if (LinkObject == InReferencedAsset || LinkObject->GetOuter() == InReferencedAsset)
			{
				continue;
			}

			bool bReferencerFound = false;

			UClass* LinkObjClass = LinkObject->GetClass();

			// Check Class or Class Generated by for BPs
			if (LinkObjClass == InReferencer->GetClass() ||
				LinkObjClass->ClassGeneratedBy == InReferencer)
			{
				bReferencerFound = true;
			}

			FString IntermediateObjectName;

			if (!bReferencerFound)
			{
				// Let's check outer as well, in case the referencing property is a field of an Object/Component
				if (const UObject* LinkObjectOuter = LinkObject->GetOuter())
				{
					// Check Class or Class Generated by for outer
					if (LinkObjectOuter->GetClass() == InReferencer->GetClass())
					{
						bReferencerFound = true;
					}
					else if (const UClass* LinkObjectOuterClass = LinkObjectOuter->GetClass())
					{
						if (LinkObjectOuterClass->ClassGeneratedBy == InReferencer)
						{
							bReferencerFound = true;
						}
					}
					...
					if (bReferencerFound)
					{
						// Store the Object name, so we can display clearer information
						// TODO: maybe improve the way this info is collected in FReferencingPropertyDescription
						IntermediateObjectName = LinkObject->GetName();
					}
				}
			}
			...
			if (UObject* ReferenceObject = ReferenceInfo->Object->ObjectInfo->TryResolveObject())
			{
				...
				{
					FString RefName = ReferenceInfo->ReferencerName.ToString();

					if (!IntermediateObjectName.IsEmpty())
					{
						// TODO: can be improved by better design.
						// This is a quick way to display useful info,
						// e.g. MyStaticMeshComponent:StaticMesh instead of just StaticMesh
						RefName = IntermediateObjectName + TEXT(":") + RefName;
					}
