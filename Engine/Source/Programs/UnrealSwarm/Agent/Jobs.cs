	...
	public class AgentJob
	{
		...
		public Int32 OpenJob( Connection NewOwner, AgentGuid NewJobGuid )
		{
			...
			if( NewOwner is LocalConnection )
			{
				...
				AgentApplication.UpdateMachineState(Unreal.MachineName, -1, EProgressionState.InstigatorConnected );
			}
			...
			try
			{
				...
				lock( PostJobStatsToDBLock )
				{
						...
						( Properties.Settings.Default.StatsDatabaseName.Length > 0 ) )
					{
						...
						SqlCommandString += String.Format( " @MachineName='{0}',", Unreal.MachineName.ToUpperInvariant() );
			...
			if( ExecutableNameMatches )
			{
				...
				if( AllTasksMatch )
				{
					...
					for( Int32 i = 0; i < LastSuccessfulJobRecord.WorkerAgentNames.Count; i++ )
					{
						...
						if( ( WorkerAgentName != Unreal.MachineName) &&
				...
				if( AllowJobToStart )
				{
					...
					if( ErrorCode >= 0 )
					{
						...
						if( OwnerIsInstigator )
						{
							...
							AgentApplication.UpdateMachineState(Unreal.MachineName, ( Int32 )TaskCount, EProgressionState.TaskTotal );
						}
			...
			lock( CurrentStateLock )
			{
				...
				if( CurrentState != JobState.AGENT_JOB_CLOSED )
				{
					...
					if( OwnerIsInstigator )
					{
						...
						AgentApplication.UpdateMachineState(Unreal.MachineName, -1, EProgressionState.InstigatorDisconnected );
			...
			if( RunningTasks.TryGetValue( UpdatedTaskState.TaskGuid, out RunningTask ) )
			{
				...
				switch( UpdatedTaskState.TaskState )
				{
					...
					case EJobTaskState.TASK_STATE_REJECTED:
						...
						{
							Manager.Log( EVerbosityLevel.Informative, ELogColour.Red, "[UpdateTaskState]: Task Rejected locally by " + Unreal.MachineName + ", counted as failure");
					...
					case EJobTaskState.TASK_STATE_KILLED:
						...
						{
							Manager.Log( EVerbosityLevel.Informative, ELogColour.Red, "[UpdateTaskState]: Task Killed locally by " + Unreal.MachineName + ", counted as failure" );
							UpdateTaskStateAsFailure( RunningTask );
						}
						break;
					...
					case EJobTaskState.TASK_STATE_COMPLETE_FAILURE:
						...
						{
							Manager.Log( EVerbosityLevel.Informative, ELogColour.Red, "[UpdateTaskState]: Task Failed on " + Unreal.MachineName);
						}
				...
				if( OwnerIsInstigator )
				{
					AgentApplication.UpdateMachineState(Unreal.MachineName, RetiredTasks.Count, EProgressionState.TasksCompleted );
					AgentApplication.UpdateMachineState(Unreal.MachineName, RunningTasks.Count, EProgressionState.TasksInProgress );
				}
			}
		}
