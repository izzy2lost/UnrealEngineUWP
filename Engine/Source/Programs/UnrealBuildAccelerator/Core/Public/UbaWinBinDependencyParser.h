...
namespace uba
{
	...
	inline bool FindImportsInMem(LPCWSTR fileName, void* mem, const Function<void(const tchar* import)>& func)
	{
		...
		__try
		{
			...
			while(importDesc->Name != NULL)
			{
				...
				wname.Append(name);
				if (!IsKnownSystemFile(wname.data))
					func(wname.data);
				++importDesc;
	...
	inline bool FindImports(LPCWSTR fileName, const Function<void(const tchar* import)>& func)
	{
