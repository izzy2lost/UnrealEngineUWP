...
namespace uba
{
	...
	bool ObjectFileElf::Parse(Logger& logger, const tchar* hint)
	{
		...
		for (u16 i=0; i!=header.e_shnum; ++i)
		{
			...
			if (strcmp(sectionName, ".shstrtab") == 0)
				printf("");
		...
		for (u16 i=0; i!=header.e_shnum; ++i)
		{
			...
			else if (section.sh_type == SHT_RELA)
			{
				...
				char* sectionName = sectionNamesTable + section.sh_name;
				(void)sectionName;
				auto& data = *(Elf64Rela*)(m_data + section.sh_offset);
				ELF64_R_SYM(data.r_info);
				ELF64_R_TYPE(data.r_info);
					(void)data;
				//printf("");
			}
			//else
	...
	*/
	...
	bool ObjectFileElf::CreateExtraFile(Logger& logger, const StringView& platform, MemoryBlock& memoryBlock, const UnorderedSymbols& allNeededImports, const UnorderedSymbols& allSharedImports, const UnorderedExports& allSharedExports, bool includeExportsInFile)
	{
		...
		header.e_ident[5] = 1;
		header.e_ident[6] = 1;
		header.e_ident[7] = platform.Equals(TC("PS4")) ? 9 : 0; // OS ABI identification
		header.e_type = 1;
