...
namespace UnrealBuildTool
{
	public class VCEnvironment
	{
		...
		private VCEnvironment(CPPTargetPlatform InPlatform, bool bSupportWindowsXP)
		{
			...
			// @ATG_CHANGE : BEGIN UWP support
			string CompilerVSToolPath = (Platform == CPPTargetPlatform.Win64 || Platform == CPPTargetPlatform.UWP64) ? VSToolPath64Bit : VSToolPath32Bit;
			// @ATG_CHANGE : END
		...
		private void CompareEnvironmentVariable(string VariableName, string RegistryValue, bool bSupportWindowsXP)
		{
			...
			Telemetry.SendEvent("CompareEnvironmentVariable",
				...
				// @ATG_CHANGE : BEGIN UWP support
				"UWPBuildForStore", UniversalWindowsPlatform.bBuildForStore.ToString(),
				// @ATG_CHANGE : END
		...
		/// </summary>
		// @ATG_CHANGE : BEGIN needed for UWP packaging
		public static string GetVSToolPath64Bit(string BaseVSToolPath)
		// @ATG_CHANGE : END
		{
		...
		static List<string> GetVisualCppLibraryPaths(string VisualCppDir, string UniversalCRTDir, string UniversalCRTVersion, string NetFXSDKDir, string WindowsSDKDir, string WindowsSDKLibVersion, CPPTargetPlatform Platform, bool bSupportWindowsXP)
		{
			...
			{
				// @ATG_CHANGE : BEGIN UWP support
				string StoreLibraryDir = Path.Combine(VisualCppDir, "LIB", "store");
				if (Platform == CPPTargetPlatform.UWP64)
				{
					StoreLibraryDir = Path.Combine(StoreLibraryDir, "amd64");
				}
				// @ATG_CHANGE : END
