...
#pragma warning restore CA2000 // Dispose objects before losing scope
		...
		private async Task<LoginResult> ProcessHttpRequestAsync(HttpListener http, AuthorizeState loginState, OidcClient oidcClient, CancellationToken cancellationToken)
		{
			LoginResult? loginResult = null;
			...
			else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
			{
				if (IsRunningWsl())
				{
					...
					catch (Win32Exception e)
					{
						...
						{
							throw new Exception("Unable to start wslview, make sure its installed by running 'sudo apt install wslu' .", e);
						}
