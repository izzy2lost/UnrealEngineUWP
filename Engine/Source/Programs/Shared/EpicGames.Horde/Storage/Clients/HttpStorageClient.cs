...
namespace EpicGames.Horde.Storage.Clients
{
	...
	public class HttpStorageClient : BundleStorageClient
	{
		...
		/// Constructor
		/// </summary>
		public HttpStorageClient(IHttpClientFactory httpClientFactory, Uri baseAddress, string? bearerToken, StorageCache cache, ILogger logger)
			...
			using (HttpClient httpClient = _createClient())
			{
				using (HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, $"nodes?alias={HttpUtility.UrlEncode(alias.ToString())}"))
				{
					using (HttpResponseMessage response = await httpClient.SendAsync(request, cancellationToken))
					{
						...
						{
							yield return new FlushedNodeHandle(BundleReader, new BundleNodeLocator(node.Hash, node.Blob, node.ExportIdx));
						}
					}
				}
			}
				...
				using (HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, $"refs/{name}"))
				{
					...
					using (HttpResponseMessage response = await httpClient.SendAsync(request, cancellationToken))
					{
						...
						else
						{
							...
							return new FlushedNodeHandle(BundleReader, new BundleNodeLocator(data.Hash, data!.Blob, data!.ExportIdx));
						}
					}
				}
			}
