...
namespace AutomationTool.Tasks
{
	...
	public class CreateArtifactTask : SpawnTaskBase
	{
		...
		public override async Task ExecuteAsync(JobContext Job, HashSet<FileReference> BuildProducts, Dictionary<string, HashSet<FileReference>> TagNameToFileSet)
		{
			...
			serviceCollection.AddHorde(new HordeOptions { AllowAuthPrompt = !Automation.IsBuildMachine });
			...
			using (IStorageClient client = httpStorageClientFactory.CreateClient(response.NamespaceId, response.Token))
			{
				await using (IStorageWriter writer = client.CreateWriter(response.RefName))
				{
					...
					HashedNodeRef<DirectoryNode> outputNodeRef = await writer.WriteFilesAsync(baseDir.ToDirectoryInfo(), files, progress: new CopyStatsLoggerWithTotals(totalCount, totalSize, Logger));
					await writer.FlushAsync();

					await client.WriteRefAsync(response.RefName, outputNodeRef.Handle);
				}
			}
