...
namespace Horde.Agent.Tests
{
	...
	public class JsonRpcLoggerTests
	{
		...
		class FakeJsonRpcLoggerBackend : JsonRpcAndStorageLogSink
		{
			...
			protected override Task UpdateLogAsync(BundleNodeHandle target, int lineCount, bool complete, CancellationToken cancellationToken)
			{
				Target = target;
		...
		{
			using BundleReaderCache cache = new BundleReaderCache();
