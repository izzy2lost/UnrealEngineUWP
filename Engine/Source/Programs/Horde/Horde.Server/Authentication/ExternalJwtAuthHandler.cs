	...
	public class ExternalJwtAuthHandler
	{
		...
		private async Task OnTokenValidatedAsync(TokenValidatedContext context)
		{
			...
			if (!_subToUser.TryGetValue(accessToken.Subject, out IUser? user))
			{
				...
				if (_userInfoUrl == null)
				{
					Uri discoveryUrl = new Uri(_settings.OidcAuthority.TrimEnd('/') + "/.well-known/openid-configuration");

					using HttpResponseMessage discoveryResponse = await httpClient.GetAsync(discoveryUrl, cancellationToken);
					if (!discoveryResponse.IsSuccessStatusCode)
					{
						ReportError(logger, context, "Bad status code from OpenID authority when fetching OIDC discovery document: " + discoveryResponse.StatusCode);
						return;
					}

					JsonObject? wellKnownUrls = await discoveryResponse.Content.ReadFromJsonAsync<JsonObject>(cancellationToken);
					if (wellKnownUrls == null || !wellKnownUrls.TryGetPropertyValue("userinfo_endpoint", out JsonNode? userInfoNode))
					{
						ReportError(logger, context, "Missing userinfo endpoint from OIDC discovery document");
						return;
					}

					string? userInfoNodeValue = (string?)userInfoNode;
					if (userInfoNodeValue == null)
					{
						ReportError(logger, context, "Missing userinfo value from OIDC discovery document");
						return;
					}

					_userInfoUrl = new Uri(userInfoNodeValue);
				}

				using HttpRequestMessage request = new(HttpMethod.Get, _userInfoUrl);
