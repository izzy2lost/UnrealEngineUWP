// Copyright Epic Games, Inc. All Rights Reserved.

using System;
using System.ComponentModel;
using System.Globalization;
using System.IO;
using System.Text.Json;
using System.Text.Json.Serialization;
using Blake3;
using EpicGames.Core;
using EpicGames.Serialization;

#pragma warning disable CS1591

namespace Horde.Server.Ddc
{
	[TypeConverter(typeof(ContentHashTypeConverter))]
    [JsonConverter(typeof(ContentHashJsonConverter))]
    [CbConverter(typeof(ContentHashCbConverter))]
    public class JupiterContentHash : IEquatable<JupiterContentHash>, IEquatable<byte[]>
    {
        protected byte[] Identifier { get; init; }
        public const int HashLength = 20;

        public byte[] HashData => Identifier;
        public JupiterContentHash(byte[] identifier)
        {
            Identifier = identifier;
            /*if (identifier.Length != HashLength)
            {
                throw new ArgumentException("Supplied identifier was not 20 bytes, this is not a valid identifier", nameof(identifier));
            }*/
        }

        [JsonConstructor]
        public JupiterContentHash(string identifier)
        {
            if (identifier == null)
            {
                throw new ArgumentNullException(nameof(identifier));
            }

            byte[] byteIdentifier = StringUtils.ParseHexString(identifier);
            Identifier = byteIdentifier;
            /*if (byteIdentifier.Length != HashLength)
            {
                throw new ArgumentException("Supplied identifier was not 20 bytes, this is not a valid identifier", nameof(identifier));
            }*/
        }

        public override int GetHashCode()
        {
			HashCode hashCode = new HashCode();
			hashCode.AddBytes(Identifier);
			return hashCode.ToHashCode();
        }

        public bool Equals(JupiterContentHash? other) => other != null && Identifier.AsSpan().SequenceEqual(other.Identifier);

		public bool Equals(byte[]? other) => other != null && Identifier.AsSpan().SequenceEqual(other);

        public override bool Equals(object? obj)
        {
            if (ReferenceEquals(null, obj))
            {
                return false;
            }

            if (ReferenceEquals(this, obj))
            {
                return true;
            }

            if (obj.GetType() != GetType())
            {
                return false;
            }

            return Equals((JupiterContentHash) obj);
        }

        public override string ToString()
        {
            return StringUtils.FormatHexString(Identifier);
        }

        public static JupiterContentHash FromBlob(byte[] blobMemory)
        {
            using Hasher hasher = Hasher.New();
            hasher.UpdateWithJoin(blobMemory);
            Hash blake3Hash = hasher.Finalize();

            // we only keep the first 20 bytes of the Blake3 hash
            Span<byte> hash = blake3Hash.AsSpanUnsafe().Slice(0, HashLength);

            return new JupiterContentHash(hash.ToArray());
        }
    }
    
    public class ContentHashCbConverter : CbConverterBase<JupiterContentHash>
    {
        public override JupiterContentHash Read(CbField field) => new JupiterContentHash(field.AsHash().ToByteArray());

        /// <inheritdoc/>
        public override void Write(CbWriter writer, JupiterContentHash value) => writer.WriteHashValue(new IoHash(value.HashData));

        /// <inheritdoc/>
        public override void WriteNamed(CbWriter writer, Utf8String name, JupiterContentHash value) => writer.WriteHash(name, new IoHash(value.HashData));
    }

    public class ContentHashJsonConverter : JsonConverter<JupiterContentHash>
    {
        public override JupiterContentHash Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            string? str = reader.GetString();
            if (str == null)
            {
                throw new InvalidDataException("Unable to parse content hash");
            }

            return new JupiterContentHash(str);
        }

        public override void Write(Utf8JsonWriter writer, JupiterContentHash value, JsonSerializerOptions options)
        {
            writer.WriteStringValue(value.ToString());
        }
    }

    public class ContentHashTypeConverter : TypeConverter
    {
        public override bool CanConvertFrom(ITypeDescriptorContext? context, Type sourceType)
        {  
            if (sourceType == typeof(string))  
            {  
                return true;
            }  
            return base.CanConvertFrom(context, sourceType);
        }  
  
        public override object? ConvertFrom(ITypeDescriptorContext? context, CultureInfo? culture, object value)  
        {
            if (value is string s)
            {
                return new JupiterContentHash(s);
            }

            return base.ConvertFrom(context, culture, value);  
        }

        public override bool CanConvertTo(ITypeDescriptorContext? context, Type? destinationType)
        {
            if (destinationType == typeof(string))
            {
                return true;
            }
            return base.CanConvertTo(context, destinationType);
        }

        public override object? ConvertTo(ITypeDescriptorContext? context, CultureInfo? culture, object? value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                JupiterContentHash? identifier = (JupiterContentHash?)value;
                return identifier?.ToString();
            }
            return base.ConvertTo(context, culture, value, destinationType);
        }
    }
}
