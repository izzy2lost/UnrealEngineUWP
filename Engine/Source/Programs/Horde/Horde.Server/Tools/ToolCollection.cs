...
namespace Horde.Server.Tools
{
	...
	public class ToolCollection : IToolCollection
	{
		...
		/// Constructor
		/// </summary>
		public ToolCollection(MongoService mongoService, RedisService redisService, StorageService storageService, StorageCache cache, IClock clock, ILogger<ToolCollection> logger)
		{
		...
		public async Task<BundleStorageClient> GetStorageClientAsync(ITool tool, CancellationToken cancellationToken)
		{
			...
			{
				return new FileStorageClient(DirectoryReference.Combine(Program.AppDir, bundledConfig.DataDir ?? $"tools/{tool.Id}"), _cache, _logger);
			}
			else
			{
