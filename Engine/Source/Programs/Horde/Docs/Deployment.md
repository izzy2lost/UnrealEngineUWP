...
describes the system architecture of Horde, the various components that make it up, and external services that it
interfaces with.
...
* **Horde Dashboard**
  * A web frontend for Horde, written in Typescript using React. The dashboard can be hosted by the regular Horde
    Server, or by a separate web server.
* [**Horde Server**](Deployment/Server.md)
  * The core server code, written in C# / ASP.NET. Can be deployed in isolation, or as a horizontally scaled service
...
* [**Horde Agent**](Deployment/Agent.md)
  * Software which runs on worker machines and executes tasks assigned to it by the Horde server. Written in C#
...
* **Redis** database (or compatible)
  * An in-memory database used to cache frequently accessed state between server instances, and provide pub/sub
...
* **Storage** 
  * Horde can utilize a local disk, network share or cloud-based object store (eg.
    ...
    for storing intermediate and output artifacts, log files, and cache data. Other backends can be implemented through
    the *IStorageBackend* interface. By default, Horde stores data locally on the server machine.
...
* **Perforce**
  * Horde supports Perforce for revision control in a CI setting, and also supports reading configuration data
    directly from it.
  * See: [Deployment > Integrations > Perforce](Deployment/Integrations/Perforce.md)
* **Jira**
  * Horde has its own system for tracking and triaging build health issues, but can interface with an external issue
    service if desired. Horde ships with support for [Jira](https://www.atlassian.com/software/jira), though other
    backends can be supported through the *IExternalIssueService* interface.

Support for services mentioned above are the result of what Epic happens to use internally. Support may change in
