...
to work, and they may not be for everyone. Being completely generic is a non-goal for Horde - we believe it is in
...
run and debug locally on all our supported desktop platforms with very few prerequisites installed. A database
will be created locally if you don't have one set up, and all the required services will automatically start and
stop with the lifetime of the server.

## Easy to Manage

Having control over the source for a project like Horde, while also using aggressively it in a high-velocity
...
configuration data can be stored in source control, and we provide detailing profiling and performance
tooling built-in.

## Open

There are other tools developed by large engineering companies which provide (some) similar services
...
Epic does not host any of your data with Horde, nor receive any telemetry from user deployments of it by default.
