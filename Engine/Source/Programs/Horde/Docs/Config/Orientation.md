...
Horde is almost exclusively configured through JSON config files. We strongly believe this to be a feature over
UI-driven configuration, despite the initial learning curve for some of the conventions and concepts involved. By
...
Each project and stream typically have their own configuration file. By convention (and for the schema server to work
...
Projects and streams need to be set up in order to use the CI, PerfMem Hub, and Test Hub aspects of Horde. For
...
There are a lot of settings in Horde config files, and it can take time to get used to them. In order to make editing
easier, Horde implements a JSON schema server which can allow IDEs to perform context highlighting, autocomplete, and
