...
Communication between agent and server is done through streaming gRPC calls initated by the agent. The ends of the
...
new leases added by the server, and the server has acknowledged completion of any removed leases by the agent.

Within the Horde server, leases are assigned to agents through classes implementing `ITaskSource`.

## Lease Types

Each lease type consists of three key classes; a message defining the lease itself (ie. what the agent needs to do),
a *task source* on the server that decides to assign a lease to an agent (`ITaskSource`), and a *lease handler* on
the agent which does the work (`LeaseHandler`).
...
To add a new lease type, each of the three classes above must be added.
