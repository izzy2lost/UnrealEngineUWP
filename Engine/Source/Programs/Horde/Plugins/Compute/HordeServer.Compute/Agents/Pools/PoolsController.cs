...
namespace HordeServer.Agents.Pools
{
	...
	public class PoolsController : HordeControllerBase
	{
		...
		/// Constructor
		/// </summary>
		public PoolsController(AgentService agentService, IPoolCollection poolCollection, IUtilizationDataCollection utilizationDataCollection, IClock clock, IOptionsSnapshot<ComputeConfig> computeConfig)
		{
		...
		{
			if (!_computeConfig.Value.Authorize(PoolAclAction.CreatePool, User))
			{
		...
		{
			if (!_computeConfig.Value.Authorize(PoolAclAction.ViewPool, User))
			{
		...
		{
			if (!_computeConfig.Value.Authorize(PoolAclAction.UpdatePool, User))
			{
		...
		{
			if (!_computeConfig.Value.Authorize(PoolAclAction.DeletePool, User))
			{
