...
namespace HordeServer.Agents.Leases
{
	...
	public class LeasesController : HordeControllerBase
	{
		...
		/// Constructor
		/// </summary>
		public LeasesController(AgentService agentService, IEnumerable<ITaskSource> taskSources, IOptionsSnapshot<ComputeConfig> computeConfig, Tracer tracer)
		{
			...
			CancellationToken cancellationToken = default)
		{
			...
			bool includeCosts = _computeConfig.Value.Authorize(ServerAclAction.ViewCosts, User);
		...
		public async Task<ActionResult<GetAgentLeaseResponse>> GetLeaseAsync(LeaseId leaseId, CancellationToken cancellationToken)
		{
			...
			double? agentRate = null;
			if (_computeConfig.Value.Authorize(ServerAclAction.ViewCosts, User) && agent != null)
			{
		...
		{
			if (!_computeConfig.Value.Authorize(LeaseAclAction.ViewLeaseTasks, User))
			{
		...
		{
			if (!_computeConfig.Value.Authorize(AdminAclAction.AdminWrite, User))
			{
