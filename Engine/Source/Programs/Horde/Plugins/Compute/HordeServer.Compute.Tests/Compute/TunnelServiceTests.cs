...
namespace HordeServer.Tests.Compute
{
	...
	public sealed class TunnelServiceTests
	{
		...
		private readonly int _echoPort = GetAvailablePort();
		private readonly int _tunnelPort = GetAvailablePort();
		...
		public async Task SendRecvTestAsync(int echoServerBufferSize)
		{
			...
			using TunnelService tunnelService = new(new OptionsWrapper<StaticComputeConfig>(serverSettings), ComputeTestSetup.CreateConsoleLogger<TunnelService>());
