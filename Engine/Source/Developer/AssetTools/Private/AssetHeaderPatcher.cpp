	...
	for (const TTuple<FString, FString>& Filename : PatchedFiles)
	{
		auto DoPatchFn = [this, 
			...
			OnError = InOnError]()
			{
				...
				if (NumPatched)
				{
					std::atomic_ref(*NumPatched).fetch_add(1, std::memory_order_relaxed);
				}
...
/*
	Enable when serialization is replaced with PackageReader. Thumbnail serialization seems to work but 
	we don't test it well enough to confidently enable. This data is regenerated when missing upon load 
	so not patching it isn't a huge deal. Patching it incorrectly may corrupt the whole file however.
...
TEST_CASE("AssetHeaderPatcher", "[AssetHeaderPatcher][EngineFilter]")
{
...
#define CHECK_NOT_EQUALS(What, X, Y) if(!FAutomationTestFramework::Get().GetCurrentTest()->TestNotEqual(What, X, Y)) { UE_DEBUG_BREAK(); };
	...
	{
		{
			FString Actual = LR"(/SrcMount/SomePath/SrcPackage)";
			const FString Expected = LR"(/DstMount/SomePath/DstPackage)";
			CHECK(Patcher.DoPatch(Actual));
			CHECK_EQUALS(TEXT("Patch string with direct match"), Actual, Expected);
		}

		{
			FString Actual = LR"(/SrcMount/SomePath/SrcPackage.SrcPackage)";
			const FString Expected = LR"(/DstMount/SomePath/DstPackage.DstPackage)";
			CHECK(Patcher.DoPatch(Actual));
			CHECK_EQUALS(TEXT("Generated Top-Level Asset mapping"), Actual, Expected);
		}

		{
			FString Actual = LR"(/SrcMount/SomePath/SrcPackage.SrcPackage_C)";
			const FString Expected = LR"(/DstMount/SomePath/DstPackage.DstPackage_C)";
			CHECK(Patcher.DoPatch(Actual));
			CHECK_EQUALS(TEXT("Generated Blueprint Generated Class mapping"), Actual, Expected);
		}

		{
			FString Actual = LR"(/SrcMount/SomePath/SrcPackage.Default__SrcPackage_C)";
			const FString Expected = LR"(/DstMount/SomePath/DstPackage.Default__DstPackage_C)";
			CHECK(Patcher.DoPatch(Actual));
			CHECK_EQUALS(TEXT("Generated Blueprint Generated Class Default Object mapping"), Actual, Expected);
		}

		{
			FString Actual = LR"(/SrcMount/SomePath/SrcPackage.SrcPackageEditorOnlyData)";
			const FString Expected = LR"(/DstMount/SomePath/DstPackage.DstPackageEditorOnlyData)";
			CHECK(Patcher.DoPatch(Actual));
		...
		SECTION("Verse Mountpoints")
		{
			for (const FString& VerseMount : Context.GetVerseMountPoints())
			{
				...
				{
					FString Actual = FString::Printf(LR"(/%s/SrcMount/SomePath/SrcPackage)", *VerseMount);
					const FString Expected = FString::Printf(LR"(/%s/DstMount/SomePath/DstPackage)", *VerseMount);
					CHECK(!Patcher.DoPatch(Actual));
					CHECK_NOT_EQUALS(TEXT("Patch string with direct match"), Actual, Expected);
				}

				{
					FString Actual = FString::Printf(LR"(/%s/SrcMount/SomePath/SrcPackage/SrcPackage)", *VerseMount);
					const FString Expected = FString::Printf(LR"(/%s/DstMount/SomePath/DstPackage/DstPackage)", *VerseMount);
					CHECK(Patcher.DoPatch(Actual));
					CHECK_EQUALS(TEXT("Patch string with direct match"), Actual, Expected);
				}
				...
				{
					FString Actual = FString::Printf(LR"(/%s/SrcMount/SomePath/SrcPackage/SrcPackage_C)", *VerseMount);
					const FString Expected = FString::Printf(LR"(/%s/DstMount/SomePath/DstPackage/DstPackage_C)", *VerseMount);
					CHECK(Patcher.DoPatch(Actual));
					CHECK_EQUALS(TEXT("Generated Blueprint Generated Class mapping"), Actual, Expected);
				}

				{
					FString Actual = FString::Printf(LR"(/%s/SrcMount/SomePath/SrcPackage/Default__SrcPackage_C)", *VerseMount);
					const FString Expected = FString::Printf(LR"(/%s/DstMount/SomePath/DstPackage/Default__DstPackage_C)", *VerseMount);
					CHECK(Patcher.DoPatch(Actual));
					CHECK_EQUALS(TEXT("Generated Blueprint Generated Class Default Object mapping"), Actual, Expected);
				}

				{
					FString Actual = FString::Printf(LR"(/%s/SrcMount/SomePath/SrcPackage/SrcPackageEditorOnlyData)", *VerseMount);
					const FString Expected = FString::Printf(LR"(/%s/DstMount/SomePath/DstPackage/DstPackageEditorOnlyData)", *VerseMount);
					CHECK(Patcher.DoPatch(Actual));
					CHECK_EQUALS(TEXT("Generated MaterialFunctionInterface Editor Only Data mapping"), Actual, Expected);
				}
			}
	...
	SECTION("DoPatch(FString)")
	{
		SECTION("Direct match")
		{
			...
			{
				FString Actual = LR"(/SrcMount/SomePath/SrcPackage2)";
		...
		{
			{
				FString Actual = LR"(/SrcMount/SomePath/SrcPackage.SrcPackage:AnOuter.To.A.SubObject)";
				const FString Expected = LR"(/DstMount/SomePath/DstPackage.DstPackage:AnOuter.To.A.SubObject)";
				CHECK(Patcher.DoPatch(Actual));
			...
			{
				FString Actual = LR"(/SrcMount/SomePath/SrcPackage.UnmappedObject:AnOuter.To.A.SubObject)";
				const FString Expected = LR"(/DstMount/SomePath/DstPackage.UnmappedObject:AnOuter.To.A.SubObject)";
				CHECK(!Patcher.DoPatch(Actual));
		...
		SECTION("Quoted match")
		{
			...
			SECTION("Substring match")
			{
				{
					FString Actual =
						LR"(((ReferenceNodePath="/SrcMount/SomePath/SrcPackage.SrcPackage:RigVMModel.Setup Arm",)"
						LR"(((Package="/SrcMount/SomePath/SrcPackage",)"
						LR"(HostObject="/SrcMount/SomePath/SrcPackage.SrcPackage_C")))";
					FString Expected =
						LR"(((ReferenceNodePath="/DstMount/SomePath/DstPackage.DstPackage:RigVMModel.Setup Arm",)"
						LR"(((Package="/DstMount/SomePath/DstPackage",)"
						LR"(HostObject="/DstMount/SomePath/DstPackage.DstPackage_C")))";
					CHECK(Patcher.DoPatch(Actual));
		...
		SECTION("Mountpoint match")
		{
			...
			*/
			/*
			{
				FString Actual = LR"(/SrcSpecialMount/SomePath/SomePackage)";
				const FString Expected = LR"(/DstSpecialMount/SomePath/SomePackage)";
				CHECK(Patcher.DoPatch(Actual));
				CHECK_EQUALS(TEXT("Patch package path replaces only mount"), Actual, Expected);
			}

			{
				FString Actual = LR"(/SrcSpecialMount/SomePath/SomePackage.SomePackage)";
				const FString Expected = LR"(/DstSpecialMount/SomePath/SomePackage.SomePackage)";
				CHECK(Patcher.DoPatch(Actual));
			...
			*/
			...
			{
				FString Actual = LR"(/SrcSpecialMount/SomePath/SomePackage.TopLevel:SubObject1.SubObject2)";
				const FString Expected = LR"(/DstSpecialMount/SomePath/SomePackage.TopLevel:SubObject1.SubObject2)";
				CHECK(Patcher.DoPatch(Actual));
			...
			{
				FString Actual = LR"(SomePrefix="/SrcSpecialMount/SomePath/SomePackage")";
				const FString Expected = LR"(SomePrefix="/DstSpecialMount/SomePath/SomePackage")";
				CHECK(Patcher.DoPatch(Actual));
