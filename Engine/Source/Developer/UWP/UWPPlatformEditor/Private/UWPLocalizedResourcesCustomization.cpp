	...
	, LocalizedResourceCollectionArray(InLocalizedResourceCollectionProperty->AsArray().ToSharedRef())
{
	...
	FSimpleDelegate ChangedDelegate = FSimpleDelegate::CreateRaw(this, &FUWPLocalizedResourcesNodeBuilder::OnNumElementsChanged);
	LocalizedResourceCollectionArray->SetOnNumElementsChanged(ChangedDelegate);
}

FUWPLocalizedResourcesNodeBuilder::~FUWPLocalizedResourcesNodeBuilder()
{
	FSimpleDelegate Empty;
	LocalizedResourceCollectionArray->SetOnNumElementsChanged(Empty);
}

void FUWPLocalizedResourcesNodeBuilder::AddLocalizedEntryForSelectedCulture()
{
...
void FUWPLocalizedResourcesNodeBuilder::AddLocalizedEntry(const FString& CultureId)
{
...
void FUWPLocalizedResourcesNodeBuilder::EmptyLocalizedEntries()
{
	uint32 NumElements;
	LocalizedResourceCollectionArray->GetNumElements(NumElements);
...
void FUWPLocalizedResourcesNodeBuilder::GenerateChildContent(IDetailChildrenBuilder& ChildrenBuilder)
{
	...
	if (AvailableUECultureNames.Num() > 0)
	{
		LocalizedGroup.AddWidgetRow()
			...
			[
				...
				.AutoWidth()
				[
					PropertyCustomizationHelpers::MakeAddButton(FSimpleDelegate::CreateSP(this, &FUWPLocalizedResourcesNodeBuilder::AddLocalizedEntryForSelectedCulture))
				]
			];
	}
	else
...
TSharedRef<IPropertyTypeCustomization> FUWPLocalizedResourcesCustomization::MakeInstance()
{
	return MakeShareable(new FUWPLocalizedResourcesCustomization);
}

void FUWPLocalizedResourcesCustomization::CustomizeHeader(TSharedRef<class IPropertyHandle> StructPropertyHandle, class FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
...
void FUWPLocalizedResourcesCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> InStructPropertyHandle, class IDetailChildrenBuilder& ChildBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
