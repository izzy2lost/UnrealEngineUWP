...
{
    int32_t idx;
#ifdef DEBUG_TZNAME && !PLATFORM_UWP
...
#if !defined (DEBUG_TZNAME) && (U_PLATFORM != U_PF_ORBIS) && !defined(PLATFORM_UWP)
    tzid = getenv("TZ");
...
u_getDataDirectory(void) {
    ...
    are not allowed.
    */
#   if !defined(ICU_NO_USER_DATA_OVERRIDE) && !UCONFIG_NO_FILE_IO && !PLATFORM_UWP
...
#   endif
    ...
    if(path==NULL || *path==0) {
# if defined(ICU_DATA_DIR_PREFIX_ENV_VAR) && !PLATFORM_UWP
...
static const char *uprv_getPOSIXIDForCategory(int category)
{
    ...
    if (category == LC_MESSAGES || category == LC_CTYPE) {
        ...
        */
        ...
        {
#if U_PLATFORM == U_PF_ORBIS || PLATFORM_UWP
