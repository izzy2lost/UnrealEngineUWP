...
class ElfSectionReader {
 public:
  ElfSectionReader(const char *name, const string &path, OSHandle fd,
    ...
    const size_t additional = header_.sh_offset % GetOSPageSize();
    ...
    int prot_flags = PAGE_READONLY;
    int flags = FILE_MAP_READ;
#else
  ...
  ~ElfSectionReader() {
    if (contents_aligned_ != NULL)
      CloseOSMapping(contents_aligned_, size_aligned_);
    else
      delete[] contents_;
  }
...
class ElfReaderImpl {
 public:
  explicit ElfReaderImpl(const string &path, OSHandle fd)
      : path_(path),
        fd_(fd),
  ...
  // in case of failure.
  static bool IsArchElfFile(OSHandle fd, string *error) {
    unsigned char header[EI_NIDENT];
    if (ReadOSHandle(fd, header, sizeof(header)) != sizeof(header)) {
...
*/
 ...
 private:
  ...
  bool ParseHeaders(OSHandle fd, const string &path) {
    // Read in the global ELF header.
    if (ReadOSHandle(fd, &header_, sizeof(header_)) != sizeof(header_)) {
      return false;
    }
    ...
    if (ReadOSHandle(fd, &first_section_header_, sizeof(first_section_header_), 1, header_.e_shoff) != sizeof(first_section_header_)) {
        return false;
      }
    }
    ...
    const size_t section_headers_size =
        GetNumSections() * sizeof(*section_headers_);
    section_headers_ = new typename ElfArch::Shdr[section_headers_size];
    if (ReadOSHandle(fd, section_headers_, section_headers_size, 1, header_.e_shoff) != section_headers_size) {
      return false;
    }
...
ElfReader::ElfReader(const string &path)
    : path_(path), fd_(OSHandleInvalid), impl32_(NULL), impl64_(NULL) {
...
static bool IsElfFile(const OSHandle fd, const string &path) {
...
template <typename ElfArch>
static bool IsNonStrippedELFBinaryImpl(const string &path, const OSHandle fd,
                                       bool debug_only) {
