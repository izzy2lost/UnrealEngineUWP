...
SDetailsDiff::FDiffControl SDetailsDiff::GenerateDetailsPanel(const TFunction<const UObject*(const UObject*)>& Redirector)
{
	...
	{
		const UObject* CenterObject = Redirector ? Redirector(OutputObject) : OutputObject;
		...
		{
			TSharedRef<IDetailsView> DetailsView = DiffControl->InsertObject(CenterObject, false, 1);
			...
			Splitter->AddSlot(
				SDetailsSplitter::Slot()
					.DetailsView(DiffControl->GetDetailsWidget(CenterObject))
					.Value(0.5f)
					.IsReadonly(false)
					.DifferencesWithRightPanel(DiffControl.ToSharedRef(), &FDetailsDiffControl::GetDifferencesWithRight, (const UObject*)CenterObject),
