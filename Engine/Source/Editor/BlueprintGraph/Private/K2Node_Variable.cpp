...
bool UK2Node_Variable::CreatePinForVariable(EEdGraphPinDirection Direction, FName InPinName/* = NAME_None */)
{
	...
	// This must run before GetPropertyForVariable to avoid guid collisions in pastebins
	// from causing the variable reference to be renamed by FMemberReference::ResolveMemberImpl
	// via FMemberReference::RefreshLocalVariableName:
	if (VariableReference.IsLocalScope())
	{
		if (FBPVariableDescription const* Desc = GetBlueprintVarDescription())
		{
			const FName PinName = InPinName.IsNone() ? GetVarName() : InPinName;
			UEdGraphPin* VariablePin = CreatePin(Direction, NAME_None, PinName);
			// search for local declaration:
			VariablePin->PinType = Desc->VarType;
...
FBPVariableDescription const* UK2Node_Variable::GetBlueprintVarDescription() const
{
	...
	bool const bIsLocalVariable = VariableReference.IsLocalScope();
	if (bIsLocalVariable)
	{
		if (VariableScope)
		{
			if (FBPVariableDescription const* Local = FBlueprintEditorUtils::FindLocalVariable(GetBlueprint(), VariableScope, VarName))
			{
				return Local;
			}
		}

		return FBlueprintEditorUtils::FindLocalVariable(GetBlueprint(), GetGraph(), VarName);
	}
...
void UK2Node_Variable::PostPasteNode()
{
	...
	if(!bInvalidateVariable && VariableReference.ResolveMember<FProperty>(Blueprint) == nullptr)
	{
		bInvalidateVariable = true;
	}
