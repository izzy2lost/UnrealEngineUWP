...
{
	for (const TPair<const UWorld*, TSet<AActor*>>& WorldActorsPair : ActorsPendingMovementUpdatePerWorld)
	{
		const UWorld* World = WorldActorsPair.Key;
		const TSet<AActor*>& ActorsPendingMovementUpdate = WorldActorsPair.Value;
		...
		{
			for (AActor* Actor : ActorsPendingMovementUpdate)
			{
				if (!IsValid(Actor))
				{
					continue;
