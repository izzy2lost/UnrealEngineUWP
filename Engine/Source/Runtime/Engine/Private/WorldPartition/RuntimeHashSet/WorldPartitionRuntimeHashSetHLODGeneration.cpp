		...
		for (auto& [RuntimePartition, CellDescInstances] : RuntimePartitionsStreamingDescs)
		{
			for (URuntimePartition::FCellDescInstance& CellDescInstance : CellDescInstances)
			{
				...
				for (auto& [HLODLayer, ActorInstances] : HLODLayerActorInstances)
				{
					...
					HLODCreationParams.RuntimeGrid = *FString::Printf(TEXT("%s:%s"), *FString::JoinBy(MainPartitionTokens, TEXT("."), [](const FName Token) { return Token.ToString(); }), *HLODLayer->GetName());
