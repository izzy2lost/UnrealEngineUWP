...
void UCharacterMovementComponent::Crouch(bool bClientSimulation)
{
	...
	if( !bClientSimulation )
	{
		...
		if (ClampedCrouchedHalfHeight > OldUnscaledHalfHeight)
		{
			...
			const bool bEncroached = GetWorld()->OverlapBlockingTestByChannel(UpdatedComponent->GetComponentLocation() - RotateGravityToWorld(FVector(0.f,0.f,ScaledHalfHeightAdjust)), GetWorldToGravityTransform(),
		...
		if (bCrouchMaintainsBaseLocation)
		{
			...
			UpdatedComponent->MoveComponent(RotateGravityToWorld(FVector(0.f, 0.f, -ScaledHalfHeightAdjust)), UpdatedComponent->GetComponentQuat(), true, nullptr, EMoveComponentFlags::MOVECOMP_NoFlags, ETeleportType::TeleportPhysics);
		}
	...
	if ((bClientSimulation && CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy) || (IsNetMode(NM_ListenServer) && CharacterOwner->GetRemoteRole() == ROLE_AutonomousProxy))
	{
		...
		if (ClientData)
		{
			ClientData->MeshTranslationOffset -= RotateGravityToWorld(FVector(0.f, 0.f, MeshAdjust));
...
void UCharacterMovementComponent::UnCrouch(bool bClientSimulation)
{
	...
	if( !bClientSimulation )
	{
			...
			if (bEncroached)
			{
				...
				if (ScaledHalfHeightAdjust > 0.f)
				{
					...
					else
					{
						...
						const FVector GravityRelativeAdjustment = FVector(0, 0, -DistanceToBase + StandingCapsuleShape.Capsule.HalfHeight + SweepInflation + MIN_FLOOR_DIST / 2.f);
						const FVector NewLoc = PawnLocation + RotateGravityToWorld(GravityRelativeAdjustment);
		...
		else
		{
			...
			FVector StandingLocation = PawnLocation + RotateGravityToWorld(FVector(0.f, 0.f, StandingCapsuleShape.GetCapsuleHalfHeight() - CurrentCrouchedHalfHeight));
			...
			if (bEncroached)
			{
				if (IsMovingOnGround())
				{
					...
					{
						StandingLocation -= RotateGravityToWorld(FVector(0, 0, CurrentFloor.FloorDist - MinFloorDist));
						bEncroached = MyWorld->OverlapBlockingTestByChannel(StandingLocation, GetWorldToGravityTransform(), CollisionChannel, StandingCapsuleShape, CapsuleParams, ResponseParam);
					}
				}				
			}
