...
bool UWPKeyState[UWPKeyCount];
bool UWPKeyLastState[UWPKeyCount];
bool UWPCharState[UWPKeyCount];
bool UWPCharLastState[UWPKeyCount];

void appUWPKeyEvent(uint32 InCode, bool bInWasPressed)
{
	if (InCode < UWPKeyCount)
	{
		UWPKeyState[InCode] = bInWasPressed;
	}
}

void appUWPCharEvent(uint32 InCode, bool bInWasPressed)
{
	if (InCode < UWPKeyCount)
	{
		UWPCharState[InCode] = bInWasPressed;
	}
}

TSharedRef< FUWPInputInterface > FUWPInputInterface::Create( const TSharedRef< FGenericApplicationMessageHandler >& InMessageHandler )
{
	return MakeShareable( new FUWPInputInterface( InMessageHandler ) );
}

FUWPInputInterface::FUWPInputInterface( const TSharedRef< FGenericApplicationMessageHandler >& InMessageHandler )
	: MessageHandler( InMessageHandler )
	, GamingInput( WindowsGamingInputInterface::Create(InMessageHandler) )
{
	FMemory::Memzero(UWPKeyState, UWPKeyCount);
	FMemory::Memzero(UWPKeyLastState, UWPKeyCount);
	FMemory::Memzero(UWPCharState, UWPKeyCount);
	FMemory::Memzero(UWPCharLastState, UWPKeyCount);

	// register focus handling events
	FCoreDelegates::ApplicationHasReactivatedDelegate.AddRaw( this, &FUWPInputInterface::OnFocusGain );
	FCoreDelegates::ApplicationWillDeactivateDelegate.AddRaw( this, &FUWPInputInterface::OnFocusLost );
}

FUWPInputInterface::~FUWPInputInterface()
{
...
void FUWPInputInterface::SetMessageHandler( const TSharedRef< FGenericApplicationMessageHandler >& InMessageHandler )
{
	MessageHandler = InMessageHandler;
...
void FUWPInputInterface::Tick( float DeltaTime )
{
	ConditionalScanForKeyboardChanges(DeltaTime);
}

void FUWPInputInterface::SendControllerEvents()
{
	GamingInput->UpdateGamepads();
}

void FUWPInputInterface::SetForceFeedbackChannelValue(int32 ControllerId, FForceFeedbackChannelType ChannelType, float Value)
{
...
void FUWPInputInterface::SetForceFeedbackChannelValues(int32 ControllerId, const FForceFeedbackValues &Values)
{
...
{
	FMemory::Memzero(UWPKeyState);
	FMemory::Memzero(UWPCharState);

	GamingInput->DisableUpdate();
	ConditionalScanForKeyboardChanges(0.0f);
}

void FUWPInputInterface::OnFocusGain()
{
...
void FUWPInputInterface::ConditionalScanForKeyboardChanges( float DeltaTime )
{
	bool bShiftIsDown = (UWPKeyState[VK_LSHIFT] == true) | (UWPKeyState[VK_RSHIFT] == true);
	FModifierKeysState ModifierKeyState(
		(UWPKeyState[VK_LSHIFT] == true) ? true : false,
		(UWPKeyState[VK_RSHIFT] == true) ? true : false,
		(UWPKeyState[VK_LCONTROL] == true) ? true : false,
		(UWPKeyState[VK_RCONTROL] == true) ? true : false,
		false,
		false,
		false,
		false,
	...
	for (uint32 KeyIndex = 0; KeyIndex < UWPKeyCount; KeyIndex++)
	{
		// Process key up/down
		{
			bool bKeyPressed = UWPKeyState[KeyIndex];
			bool bKeyReleased = ((UWPKeyState[KeyIndex] == false) && (UWPKeyLastState[KeyIndex] == true));

			if ((bKeyPressed == true) || (bKeyReleased == true))
			{
				bool bIsRepeat = ((bKeyPressed == true) && (UWPKeyLastState[KeyIndex] == true));
				...
				//@todo.UWP: Put in a delay for registering repeats??
				if (bIsRepeat == false)
				{
				...
				if (bKeyReleased == true)
				{
					...
					if (CharKeyIndex != 0)
					{
						UWPCharState[CharKeyIndex] = false;
						UWPCharLastState[CharKeyIndex] = false;
					}
				}
			}
		}
		...
		// Process char
		bool bCharPressed = UWPCharState[KeyIndex];
		if (bCharPressed == true)
		{
			bool bIsRepeat = UWPCharLastState[KeyIndex];
			//@todo.UWP: Put in a delay for registering repeats??
			if (bIsRepeat == false)
			{
		...
		UWPKeyLastState[KeyIndex] = UWPKeyState[KeyIndex];
		UWPCharLastState[KeyIndex] = UWPCharState[KeyIndex];
	}
}

uint32 FUWPInputInterface::MapVirtualKeyToCharacter(uint32 InVirtualKey, bool bShiftIsDown)
{
