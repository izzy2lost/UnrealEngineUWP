...
void FByteProperty::ExportText_Internal( FString& ValueStr, const void* PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope ) const
{
	...
	{
		// Put the byte returned by the getter into GetterResult
		// - this will be re-read by a call to GetSignedIntPropertyValue below.
		ValuePtr = &GetterResult;
		GetValue_InContainer(PropertyValueOrContainer, (uint8*)ValuePtr);
	}
	else
	{
	...
	int64 Value = GetSignedIntPropertyValue(ValuePtr);
