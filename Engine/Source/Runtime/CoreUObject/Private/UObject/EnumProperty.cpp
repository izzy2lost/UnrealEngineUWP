	...
	if (UnderlyingArchive.IsLoading())
	{
		...
		if (Enum)
		{
			...
			if (Enum->HasAnyEnumFlags(EEnumFlags::Flags))
			{
				...
				{
					NewEnumValue = Enum->GetValueOrBitfieldFromString(*EnumValueName.ToString());
				}
			}
			else
			{
...
void FEnumProperty::ExportText_Internal(FString& ValueStr, const void* PropertyValueOrContainer, EPropertyPointerType PropertyPointerType, const void* DefaultValue, UObject* Parent, int32 PortFlags, UObject* ExportRootScope) const
{
	...
	if (PropertyPointerType == EPropertyPointerType::Container && HasGetter())
	{
		...
		// - these bytes will be re-read by a call to LocalUnderlyingProp below.
		ValuePtr = &GetterResult;
		GetValue_InContainer(PropertyValueOrContainer, ValuePtr);
	}
	else
	{
		// Otherwise read directly from the property
		ValuePtr = PointerToValuePtr(PropertyValueOrContainer, PropertyPointerType);
	}

	if (PortFlags & PPF_ConsoleVariable)
	{
		LocalUnderlyingProp->ExportText_Internal(ValueStr, ValuePtr, EPropertyPointerType::Direct, DefaultValue, Parent, PortFlags, ExportRootScope);
		return;
	}

	int64 Value = LocalUnderlyingProp->GetSignedIntPropertyValue(ValuePtr);
