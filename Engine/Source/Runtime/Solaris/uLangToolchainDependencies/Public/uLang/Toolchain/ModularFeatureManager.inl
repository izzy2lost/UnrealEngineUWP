...
namespace Private
{
    ...
    public: // intended to be private, to limit access, but clang apparently doesn't follow templatized friend decls
        ...
        ULANGTOOLCHAINDEPENDENCIES_API static const TSRef<IModularFeatureRegistry>& GetRegistry();
        ULANGTOOLCHAINDEPENDENCIES_API static void SetRegistry(const TSRef<IModularFeatureRegistry>& NewRegistry);

        ULANGTOOLCHAINDEPENDENCIES_API static void Register(const TSRef<IModularFeature>& NewFeature, const RegistryId FeatureId);
        ULANGTOOLCHAINDEPENDENCIES_API static bool Unregister(const TSRef<IModularFeature>& Feature);

        ULANGTOOLCHAINDEPENDENCIES_API static RegistryId GetRegistryId(const char* FeatureName);
    };
...
namespace Private
{
    ULANGTOOLCHAINDEPENDENCIES_API int32_t GetModularFeatureCount(const RegistryId FeatureId);
    ULANGTOOLCHAINDEPENDENCIES_API TSPtr<IModularFeature> GetModularFeature(const RegistryId FeatureId, const int32_t Index);
