...
class ULANGTOOLCHAINDEPENDENCIES_API CInterface : public CDefinition, public CNominalType, public CLogicalScope
{
public:
...
class ULANGTOOLCHAINDEPENDENCIES_API CInstantiatedInterface : public CInstantiatedType
{
public:
...
ULANGTOOLCHAINDEPENDENCIES_API CInterface* InstantiateInterface(
    const CInterface&,
    ETypePolarity,
    const TArray<STypeVariableSubstitution>&);

ULANGTOOLCHAINDEPENDENCIES_API CInterface* InstantiatePositiveInterface(
    const CInterface&,
    const TArray<STypeVariableSubstitution>&);
...
ULANGTOOLCHAINDEPENDENCIES_API TArray<CInterface*> GetNegativeInterfaces(const TArray<CInterface*>& Interfaces);

ULANGTOOLCHAINDEPENDENCIES_API void InstantiatePositiveFunction(
    CLogicalScope& InstScope,
    const CNormalType& InstType,
    const CFunction&,
    const TArray<STypeVariableSubstitution>& Substitutions);

ULANGTOOLCHAINDEPENDENCIES_API void SetInstantiatedOverriddenDefinition(CDefinition& InstDefinition, const CNormalType& InstType, const CDefinition&);

ULANGTOOLCHAINDEPENDENCIES_API TSRef<CFunction> CreateNegativeMemberFunction(CLogicalScope& NegativeScope, const CFunction& PositiveFunction);

ULANGTOOLCHAINDEPENDENCIES_API void SetNegativeInterfaceMemberDefinitionTypes(const CInterface& PositiveInterface);

ULANGTOOLCHAINDEPENDENCIES_API void SetNegativeMemberDefinitionType(CFunction& NegativeFunction, const CFunction& PositiveFunction);
}
