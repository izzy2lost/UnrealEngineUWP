...
public:
    ...
    ULANGTOOLCHAINDEPENDENCIES_API virtual TOptional<CUTF8String> GetText() const override;
...
public:
    ...
    ULANGTOOLCHAINDEPENDENCIES_API CUTF8StringView GetDirPath() const;

    ULANGTOOLCHAINDEPENDENCIES_API TOptional<TSRef<CSourceFileModule>> FindSubmodule(const CUTF8StringView& ModuleName) const;
    ULANGTOOLCHAINDEPENDENCIES_API TSRef<CSourceFileModule> FindOrAddSubmodule(const CUTF8StringView& ModuleName, const CUTF8StringView& DirPath);

    ULANGTOOLCHAINDEPENDENCIES_API TOptional<TSRef<CSourceFileSnippet>> FindSnippetByFilePath(const CUTF8StringView& FilePath, bool bRecursive) const;
...
public:
    ...
    ULANGTOOLCHAINDEPENDENCIES_API CSourceFilePackage(const CUTF8String& PackageFilePath, const TSRef<IFileSystem>& FileSystem, const TSRef<CDiagnostics>& Diagnostics);
    ULANGTOOLCHAINDEPENDENCIES_API CSourceFilePackage(const SPackageDesc& PackageDesc, const TSRef<IFileSystem>& FileSystem, const TSRef<CDiagnostics>& Diagnostics);
    ...
    ULANGTOOLCHAINDEPENDENCIES_API CSourceFileModule* GetModuleForFilePath(const CUTF8StringView& FilePath);
    ULANGTOOLCHAINDEPENDENCIES_API TOptional<TSRef<CSourceFileSnippet>> FindSnippetByFilePath(const CUTF8StringView& FilePath) const;
    ULANGTOOLCHAINDEPENDENCIES_API TOptional<TSRef<CSourceFileSnippet>> AddSnippet(const CUTF8StringView& FilePath); // Look up module by file path and add snippet to it - might fail if file path is not under root path
    ULANGTOOLCHAINDEPENDENCIES_API bool RemoveSnippet(const CUTF8StringView& FilePath); // Look up module by file path and remove snippet from it
...
public:
     ...
     */
    ULANGTOOLCHAINDEPENDENCIES_API CSourceFileProject(const CUTF8String& ProjectFilePath, const TSRef<IFileSystem>& FileSystem, const TSRef<CDiagnostics>& Diagnostics);

    /**
     * This ctor will create vproject given the path and packages
     */
    ULANGTOOLCHAINDEPENDENCIES_API CSourceFileProject(const CUTF8String& Name, const TSRef<IFileSystem>& FileSystem, const TArray<SPackageDesc>& Packages, const TSRef<CDiagnostics>& Diagnostics);

    virtual const CUTF8String& GetFilePath() const override { return _FilePath; }

    ULANGTOOLCHAINDEPENDENCIES_API bool WriteProjectFile(const CUTF8String& ProjectFilePath, const TSRef<CDiagnostics>& Diagnostics);
    ULANGTOOLCHAINDEPENDENCIES_API bool WriteVSCodeWorkspaceFile(const CUTF8String& WorkspaceFilePath, const CUTF8String& ProjectFilePath, const TSRef<CDiagnostics>& Diagnostics);

    ULANGTOOLCHAINDEPENDENCIES_API TOptional<TSRef<CSourceFileSnippet>> FindSnippetByFilePath(const CUTF8StringView& FilePath) const;
    ULANGTOOLCHAINDEPENDENCIES_API TOptional<TSRef<CSourceFileSnippet>> AddSnippet(const CUTF8StringView& FilePath); // Look up module by file path and add snippet to it - might fail if file path is not under root path
    ULANGTOOLCHAINDEPENDENCIES_API bool RemoveSnippet(const CUTF8StringView& FilePath); // Look up module by file path and remove snippet from it

    ULANGTOOLCHAINDEPENDENCIES_API static bool IsSnippetFile(const CUTF8StringView& FilePath);
    ULANGTOOLCHAINDEPENDENCIES_API static bool IsModuleFile(const CUTF8StringView& FilePath);
    ULANGTOOLCHAINDEPENDENCIES_API static bool IsPackageFile(const CUTF8StringView& FilePath);
    ULANGTOOLCHAINDEPENDENCIES_API static bool IsProjectFile(const CUTF8StringView& FilePath);
    ULANGTOOLCHAINDEPENDENCIES_API static bool IsValidModuleName(const CUTF8StringView& ModuleName);
    ULANGTOOLCHAINDEPENDENCIES_API static bool IsValidSnippetFileName(const CUTF8StringView& FileName);
