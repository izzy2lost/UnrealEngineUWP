...
public:
     ...
     **/
    ULANGTOOLCHAINDEPENDENCIES_API bool WriteProject(const CSourceProject& Project, const CUTF8String& DestinationDir, CUTF8String* ResultProjectFilePath = nullptr) const;

    /**
     * Write only a vproject file to the given file path
     **/
    ULANGTOOLCHAINDEPENDENCIES_API bool WriteProjectFile(const SProjectDesc& ProjectDesc, const CUTF8String& ProjectFilePath) const;

    /**
     * Write a .code-workspace file to the given file path
     **/
    ULANGTOOLCHAINDEPENDENCIES_API bool WriteVSCodeWorkspaceFile(const SWorkspaceDesc& WorkspaceDesc, const CUTF8String& WorkspaceFilePath) const;

    /**
     * Derive a project desc from a source project
     **/
    ULANGTOOLCHAINDEPENDENCIES_API static SProjectDesc GetProjectDesc(const CSourceProject& Project);

    /**
     * Derive a workspace desc from a source project
     **/
    ULANGTOOLCHAINDEPENDENCIES_API static SWorkspaceDesc GetWorkspaceDesc(const CSourceProject& Project, const CUTF8String& ProjectFilePath);
