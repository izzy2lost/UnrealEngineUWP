...
public:
    ...
    ULANGTOOLCHAINDEPENDENCIES_API CUTF8StringView GetNameFromFile() const;
    ULANGTOOLCHAINDEPENDENCIES_API static CUTF8StringView GetNameFromFile(const CUTF8StringView& ModuleFilePath);

    ULANGTOOLCHAINDEPENDENCIES_API TOptional<TSRef<CSourceModule>> FindSubmodule(const CUTF8StringView& ModuleName) const;

    ULANGTOOLCHAINDEPENDENCIES_API void AddSnippet(const uLang::TSRef<ISourceSnippet>& Snippet);
    ULANGTOOLCHAINDEPENDENCIES_API bool RemoveSnippet(const uLang::TSRef<ISourceSnippet>& Snippet, bool bRecursive);
...
public:
    ...
    virtual const CUTF8String& GetFilePath() const { return CUTF8String::GetEmpty(); }
    ULANGTOOLCHAINDEPENDENCIES_API int32_t GetNumSnippets() const;
    ...
    ULANGTOOLCHAINDEPENDENCIES_API void SetDependencyPackages(TArray<CUTF8String>&& PackageNames);
    ULANGTOOLCHAINDEPENDENCIES_API void AddDependencyPackage(const CUTF8StringView& PackageName);
    ULANGTOOLCHAINDEPENDENCIES_API void TruncateVniDestDir(); // Strip parent path from VniDestDir

    ULANGTOOLCHAINDEPENDENCIES_API bool RemoveSnippet(const uLang::TSRef<ISourceSnippet>& Snippet);
...
public:
    ...
    ULANGTOOLCHAINDEPENDENCIES_API CSourceProject(const CSourceProject& Other);
    ...
    ULANGTOOLCHAINDEPENDENCIES_API const SPackage* FindPackage(const CUTF8StringView& PackageName, const CUTF8StringView& PackageVersePath) const;
    ULANGTOOLCHAINDEPENDENCIES_API const SPackage& FindOrAddPackage(const CUTF8StringView& PackageName, const CUTF8StringView& PackageVersePath);
    ULANGTOOLCHAINDEPENDENCIES_API void AddSnippet(const uLang::TSRef<ISourceSnippet>& Snippet, const CUTF8StringView& PackageName, const CUTF8StringView& PackageVersePath);
    ULANGTOOLCHAINDEPENDENCIES_API bool RemoveSnippet(const uLang::TSRef<ISourceSnippet>& Snippet);

    ULANGTOOLCHAINDEPENDENCIES_API void TruncateVniDestDirs(); // Strip parent path from all VniDestDirs
