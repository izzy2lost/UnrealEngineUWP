...
ULANGTOOLCHAINDEPENDENCIES_API const SDiagnosticInfo& GetDiagnosticInfo(EDiagnostic Diagnostic);
ULANGTOOLCHAINDEPENDENCIES_API EDiagnostic GetDiagnosticFromReferenceCode(uint16_t ReferenceCode);
...
struct SGlitchResult
{
    ...
    ULANGTOOLCHAINDEPENDENCIES_API SGlitchResult(EDiagnostic ResultId);
...
struct SGlitchLocus
{
    ...
    ULANGTOOLCHAINDEPENDENCIES_API SGlitchLocus(const Verse::Vst::Node* VstNode);
    ULANGTOOLCHAINDEPENDENCIES_API SGlitchLocus(const CAstNode* AstNode);
     ...
     **/
    ULANGTOOLCHAINDEPENDENCIES_API CUTF8String AsFormattedString() const;
};
...
struct SGlitch : public CSharedMix
{
    ...
    // Methods 
        ...
        ULANGTOOLCHAINDEPENDENCIES_API static CUTF8String FormattedString(const char* Message, const char* Path, const STextRange& Range, EDiagnosticSeverity Severity = EDiagnosticSeverity::Error, EDiagnostic Diagnostic = EDiagnostic::Ok);

};  // SGlitch

/** Convert a Row/Col offset to a byte offset from beginning of 'Source' */
ULANGTOOLCHAINDEPENDENCIES_API TOptional<int32_t> ScanToRowCol(CUTF8StringView const& Source, const STextPosition& Position);

/** Given a 'Range', return a corresponding string subview of 'Source' */
ULANGTOOLCHAINDEPENDENCIES_API CUTF8StringView TextRangeToStringView(CUTF8StringView const& Source, STextRange const& Range);
