...
ULANGPARSER_API CUTF8StringView GetReservedSymbol(const EReservedSymbol Identifier);

/// Gets the type of reservation this symbol has.
ULANGPARSER_API EIsReservedSymbolResult GetReservationForSymbol(const EReservedSymbol Identifier, const uint32_t CurrentVerseVersion, const uint32_t CurrentUploadedAtFNVersion);
/// @overload
ULANGPARSER_API EIsReservedSymbolResult GetReservationForSymbol(const CSymbol& Identifier, const uint32_t CurrentVerseVersion, const uint32_t CurrentUploadedAtFNVersion);

/// Gets all reserved tokens (both the parser and semantic analyzer).
ULANGPARSER_API TSet<CUTF8String> GetReservedSymbols(const uint32_t CurrentVerseVersion, const uint32_t CurrentUploadedAtFNVersion);
}    // namespace uLang
