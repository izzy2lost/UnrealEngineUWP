...
extern void appUWPKeyEvent(uint32 InCode, bool bInWasPressed);
extern void appUWPCharEvent(uint32 InCode, bool bInWasPressed);
...
void ViewProvider::OnAcceleratorKeyActivated(CoreDispatcher ^sender, AcceleratorKeyEventArgs ^args)
{
	if (args->VirtualKey == VirtualKey::Menu)
	{
		...
		{
			appUWPKeyEvent(args->KeyStatus.IsExtendedKey ? VK_RMENU : VK_LMENU, true);
		}
		else if (args->EventType == CoreAcceleratorKeyEventType::SystemKeyUp && args->KeyStatus.WasKeyDown == true)
		{
			appUWPKeyEvent(args->KeyStatus.IsExtendedKey ? VK_RMENU : VK_LMENU, false);
		}
		args->Handled = true;
	}
	...
	{
		FUWPApplication* UWPApplication = FUWPApplication::GetUWPApplication();
		if (UWPApplication)
		{
			TSharedPtr< FUWPWindow > NativeWindow = UWPApplication->GetUWPWindow();
	...
	else if (args->VirtualKey == VirtualKey::Control)
	{
		...
		{
			appUWPKeyEvent(args->KeyStatus.IsExtendedKey ? VK_RCONTROL : VK_LCONTROL, true);
		}
		...
		{
			appUWPKeyEvent(args->KeyStatus.IsExtendedKey ? VK_RCONTROL : VK_LCONTROL, false);
		}
		args->Handled = true;
	}
	else if (args->VirtualKey == VirtualKey::Shift)
	{
		if ((args->EventType == CoreAcceleratorKeyEventType::SystemKeyDown || args->EventType == CoreAcceleratorKeyEventType::KeyDown) && args->KeyStatus.WasKeyDown == false)
		{
			appUWPKeyEvent(args->KeyStatus.ScanCode == 0x36 ? VK_RSHIFT : VK_LSHIFT, true);
		}
		else if ((args->EventType == CoreAcceleratorKeyEventType::SystemKeyUp || args->EventType == CoreAcceleratorKeyEventType::KeyUp) && args->KeyStatus.WasKeyDown == true)
		{
			appUWPKeyEvent(args->KeyStatus.ScanCode == 0x36 ? VK_RSHIFT : VK_LSHIFT, false);
		}
		args->Handled = true;
	}
}
	...
	_In_ CharacterReceivedEventArgs^ args)
{
	...
	{
		appUWPCharEvent(args->KeyCode, true);
	}
	args->Handled = true;
}
...
int32 GuardedMain( const TCHAR* CmdLine, HINSTANCE hInInstance, HINSTANCE hPrevInstance, int32 nCmdShow )
{
	...
	while (!GIsRequestingExit)
	{
		...
		GViewProvider->ProcessEvents();
        FUWPApplication::GetUWPApplication()->GetCursor()->ProcessDeferredActions();
