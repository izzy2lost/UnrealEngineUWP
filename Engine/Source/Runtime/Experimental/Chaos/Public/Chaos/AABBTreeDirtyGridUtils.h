...
namespace Chaos
{
	...
	{
		int64 XsampleCount = GetDirtyCellIndexFromWorldCoordinate(AABB.Max().X, DirtyElementGridCellSizeInv) - GetDirtyCellIndexFromWorldCoordinate(AABB.Min().X, DirtyElementGridCellSizeInv) + 1;
		int64 YsampleCount = GetDirtyCellIndexFromWorldCoordinate(AABB.Max().Y, DirtyElementGridCellSizeInv) - GetDirtyCellIndexFromWorldCoordinate(AABB.Min().Y, DirtyElementGridCellSizeInv) + 1;
	...
	FORCEINLINE_DEBUGGABLE bool DoForOverlappedCellsExclude(const TAABB<FReal, 3>& AABB, const TAABB<FReal, 3>& AABBExclude, FReal DirtyElementGridCellSize, FReal DirtyElementGridCellSizeInv, FunctionType Function)
	{
		...
		int64 NewCellStartX = GetDirtyCellIndexFromWorldCoordinate(AABB.Min().X, DirtyElementGridCellSizeInv);
		int64 NewCellStartY = GetDirtyCellIndexFromWorldCoordinate(AABB.Min().Y, DirtyElementGridCellSizeInv);

		int64 NewCellEndX = GetDirtyCellIndexFromWorldCoordinate(AABB.Max().X, DirtyElementGridCellSizeInv);
		int64 NewCellEndY = GetDirtyCellIndexFromWorldCoordinate(AABB.Max().Y, DirtyElementGridCellSizeInv);

		int64 OldCellStartX = GetDirtyCellIndexFromWorldCoordinate(AABBExclude.Min().X, DirtyElementGridCellSizeInv);
		int64 OldCellStartY = GetDirtyCellIndexFromWorldCoordinate(AABBExclude.Min().Y, DirtyElementGridCellSizeInv);

		int64 OldCellEndX = GetDirtyCellIndexFromWorldCoordinate(AABBExclude.Max().X, DirtyElementGridCellSizeInv);
		int64 OldCellEndY = GetDirtyCellIndexFromWorldCoordinate(AABBExclude.Max().Y, DirtyElementGridCellSizeInv);
