...
namespace Chaos
{
	...
	FReal Chaos_GBFCharacteristicTimeRatio = 1.0f;
	...
	void FPBDCollisionConstraint::UpdateManifoldContacts()
	{
		if ((GetSolverBody0() != nullptr) && (GetSolverBody1() != nullptr))
		{
			for (int32 Index = 0; Index < ManifoldPoints.Num() ; Index++)
			{
				...
				GetWorldSpaceManifoldPoint(ManifoldPoint, GetSolverBody0()->P(), GetSolverBody0()->Q(), GetSolverBody1()->P(), GetSolverBody1()->Q(), ManifoldPoint.ContactPoint.Location, ManifoldPoint.ContactPoint.Normal, ManifoldPoint.ContactPoint.Phi);
