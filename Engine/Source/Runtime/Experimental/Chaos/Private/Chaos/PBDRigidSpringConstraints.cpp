...
namespace Chaos
{
	...
	void FPBDRigidSpringConstraints::InitDistance(int32 ConstraintIndex, const FVec3& Location0, const FVec3& Location1)
	{
		...
		Distances[ConstraintIndex][0] = Particle0->GetR().Inverse().RotateVector(Location0 - Particle0->X());
		Distances[ConstraintIndex][1] = Particle1->GetR().Inverse().RotateVector(Location1 - Particle1->X());
	}
