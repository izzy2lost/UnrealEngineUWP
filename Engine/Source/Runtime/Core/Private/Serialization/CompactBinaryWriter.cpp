...
void FCbWriter::BeginField()
{
	...
	else
	{
		checkf((State.Flags & EStateFlags::Name) == EStateFlags::Name,
			TEXT("A new field cannot be written until the previous field '%s' is finished."),
			StringCast<TCHAR>(GetActiveName().GetData(), GetActiveName().Len()).Get());
	}
}
...
FCbWriter& FCbWriter::SetName(const FUtf8StringView Name)
{
	...
	checkf((State.Flags & EStateFlags::Array) != EStateFlags::Array,
		TEXT("It is invalid to write a name for an array field. Name '%s'"), StringCast<TCHAR>(Name.GetData(), Name.Len()).Get());
	...
	checkf((State.Flags & (EStateFlags::Name | EStateFlags::Field)) == EStateFlags::None,
		TEXT("A new field '%s' cannot be written until the previous field '%s' is finished."),
		StringCast<TCHAR>(Name.GetData(), Name.Len()).Get(), StringCast<TCHAR>(GetActiveName().GetData(), GetActiveName().Len()).Get());
...
{
	checkf(!TypeName.IsEmpty(), TEXT("Field '%s' requires a non-empty type name for its custom type."),
		StringCast<TCHAR>(GetActiveName().GetData(), GetActiveName().Len()).Get());
