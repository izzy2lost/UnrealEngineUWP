...
class FOutputDeviceError* FUWPOutputDevices::GetError()
{
	static FOutputDeviceUWPError Singleton;
	return &Singleton;
}
...
class FOutputDeviceConsole* FUWPOutputDevices::GetLogConsole()
{
	...
	return new FOutputDeviceConsoleUWP();
}

class FFeedbackContext* FUWPOutputDevices::GetWarn()
{
...
//////////////////////////////////
// FOutputDeviceUWPError
//////////////////////////////////

FOutputDeviceUWPError::FOutputDeviceUWPError()
:	ErrorPos(0)
{}

void FOutputDeviceUWPError::Serialize( const TCHAR* Msg, ELogVerbosity::Type Verbosity, const class FName& Category )
{
	...
	{   
		...
		{ 
			UE_LOG(LogUWPOutputDevices, Error, TEXT("appError called: %s"), Msg );

			// UWP error.
			UE_LOG(LogUWPOutputDevices, Error, TEXT("UWP GetLastError: %s (%i)"), FPlatformMisc::GetSystemErrorMessage(ErrorBuffer,1024,Error), Error );
		} 
...
// 		ErrorPos = appStrlen(GErrorHist);
	...
	{
		UE_LOG(LogUWPOutputDevices, Error, TEXT("Error reentered: %s"), Msg );
	}
...
void FOutputDeviceUWPError::HandleError()
{
	...
	if (NewCallCount != 1)
	{
		UE_LOG(LogUWPOutputDevices, Error, TEXT("HandleError re-entered.") );
		return;
	}
...
//		UE_LOG(LogUWPOutputDevices, Log, TEXT("=== Critical error: ===") LINE_TERMINATOR TEXT("%s"), GErrorHist);
		GLog->Flush();
...
FOutputDeviceConsoleUWP::FOutputDeviceConsoleUWP()
	: ConsoleHandle(0)
{
}
...
void FOutputDeviceConsoleUWP::SaveToINI()
{
}

void FOutputDeviceConsoleUWP::Show(bool ShowWindow)
{
}

bool FOutputDeviceConsoleUWP::IsShown()
{
	return ConsoleHandle != NULL;
}

void FOutputDeviceConsoleUWP::Serialize( const TCHAR* Data, ELogVerbosity::Type Verbosity, const class FName& Category )
{
	if( ConsoleHandle )
	{
		static bool Entry=0;
