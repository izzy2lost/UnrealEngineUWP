...
/**
* UWP file handle implementation
**/
class CORE_API FFileHandleUWP : public IFileHandle
{
...
public:
	FFileHandleUWP(HANDLE InFileHandle = NULL)
		: FileHandle(InFileHandle)
	{
	}
	virtual ~FFileHandleUWP()
	{
...
/**
* UWP File I/O implementation
**/
class CORE_API FUWPPlatformFile : public IPhysicalPlatformFile
{
protected:
...
public:
	...
	virtual FDateTime GetTimeStamp(const TCHAR* Filename) override
	{
		...
		return UWPEpoch + TimeSinceEpoch;
	}
	...
	virtual void SetTimeStamp(const TCHAR* Filename, FDateTime DateTime) override
	{
		...
		Times.modtime = (DateTime - UWPEpoch).GetTotalSeconds();
	...
	virtual IFileHandle* OpenRead(const TCHAR* Filename, bool bAllowWrite = false) override
	{
		...
		FPlatformMisc::LowLevelOutputDebugStringf(TEXT("UWPFile::OpenRead normalized [%s] into [%s]\n"), Filename, *normalizedFileName);
		HANDLE Handle = CreateFile2(*normalizedFileName, Access, WinFlags, Create, nullptr);
		if (Handle != INVALID_HANDLE_VALUE)
		{
			return new FFileHandleUWP(Handle);
		}
		return NULL;
	}
...
{
	static FUWPPlatformFile Singleton;
	return Singleton;
}
