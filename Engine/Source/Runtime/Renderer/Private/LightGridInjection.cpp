...
static void PackLocalLightData(
	...
	const bool bHandledByStochasticDirectLighting,
	const float VolumetricScatteringIntensity)
{
	...
	// Since lights don't use VSM and Stochastic Direct Lighting simultaneously and
	// currently PrevLocalLightIndex is only accessed by Stochastic Direct Lighting shaders
	...
	const int32 VirtualShadowMapIdOrPrevLocalLightIndex = bHandledByStochasticDirectLighting ? PrevLocalLightIndex : VirtualShadowMapId;
...
FComputeLightGridOutput FSceneRenderer::ComputeLightGrid(FRDGBuilder& GraphBuilder, bool bCullLightsToGrid, FSortedLightSetSceneInfo& SortedLightSet)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		int32 StochasticDirectLightingSupportedStart = 0;
		...
		if (bCullLightsToGrid)
		{
			...
			StochasticDirectLightingSupportedStart = MAX_int32;
			...
			for (int32 SortedIndex = SimpleLightsEnd; SortedIndex < SortedLights.Num(); ++SortedIndex)
			{
				...
				if (LightSceneInfo->ShouldRenderLight(View))
				{
						...
						(SortedLightInfo.SortKey.Fields.LightType == LightType_Rect && ViewFamily.EngineShowFlags.RectLights))
					{
						...
						if (SortedLightInfo.SortKey.Fields.bHandledByStochasticDirectLighting && StochasticDirectLightingSupportedStart == MAX_int32)
						{
							StochasticDirectLightingSupportedStart = ForwardLocalLightData.Num() - 1;
						}
		...
		ForwardLightData->StochasticDirectLightingSupportedStartIndex = FMath::Min<int32>(StochasticDirectLightingSupportedStart, NumLocalLightsFinal);
...
FComputeLightGridOutput FDeferredShadingSceneRenderer::GatherLightsAndComputeLightGrid(FRDGBuilder& GraphBuilder, bool bNeedLightGrid, FSortedLightSetSceneInfo& SortedLightSet)
{
	...
	const bool bCullLightsToGrid = GLightCullingQuality 
		&& (IsForwardShadingEnabled(ShaderPlatform) || bAnyViewUsesForwardLighting || IsRayTracingEnabled() || ShouldUseClusteredDeferredShading() ||
			bAnyViewUsesLumen || ViewFamily.EngineShowFlags.VisualizeMeshDistanceFields || VirtualShadowMapArray.IsEnabled() || StochasticDirectLighting::IsEnabled());
