...
static TAutoConsoleVariable<float> CVarStochasticShadowsTemporalNeighborhoodClampScale(
	TEXT("r.StochasticShadows.Temporal.NeighborhoodClampScale"),
	1.0f,
...
static TAutoConsoleVariable<int> CVarStochasticShadowsCandidateLightMask(
	...
	1,
	TEXT("#ss_todo: finish and pick one shader path."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
static TAutoConsoleVariable<int> CVarStochasticShadowsLightFunctions(
	TEXT("r.StochasticShadows.LightFunctions"),
	0,
	TEXT("Whether to support light functions."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);

static TAutoConsoleVariable<int> CVarStochasticShadowsIESProfiles(
	TEXT("r.StochasticShadows.IESProfiles"),
	0,
	TEXT("Whether to support IES profiles."),
	ECVF_Scalability | ECVF_RenderThreadSafe
);
...
namespace StochasticShadows
{
	...
	{
		return IsEnabled() && CVarStochasticShadowsLightFunctions.GetValueOnRenderThread() != 0;
	}
...
class FGenerateSamplesCS : public FGlobalShader
{
	...
	class FTileType : SHADER_PERMUTATION_INT("TILE_TYPE", (int32)StochasticShadows::ETileType::SHADING_MAX);
	...
	using FPermutationDomain = TShaderPermutationDomain<FTileType, FIESProfile, FLightFunctionAtlas, FTexturedRectLights, FNumSamplesPerPixel1d, FShadowMaskReprojectionWeights, FShadowFactorEstimate, FCandidateLightMask, FDebugMode>;
...
IMPLEMENT_GLOBAL_SHADER(FClearLightSamplesCS, "/Engine/Private/StochasticShadows/StochasticShadowsSampling.usf", "ClearLightSamplesCS", SF_Compute);
...
void FDeferredShadingSceneRenderer::RenderStochasticShadows(FRDGBuilder& GraphBuilder, const FSceneTextures& SceneTextures)
{
	...
	const FIntPoint ShadowMaskPageTableSize = ShadowMaskPageTablePerLightSize * StochasticShadows::MaxLocalLightIndexXY;
	...
	const bool bUseLightFunctionAtlas = StochasticShadows::IsUsingLightFunctions()
		&& LightFunctionAtlas::IsEnabled(View, LightFunctionAtlas::ELightFunctionAtlasSystem::StochasticShadows);
	...
	FStochasticShadowsParameters StochasticShadowsParameters;
	{
		...
		StochasticShadowsParameters.ForwardLightData = View.ForwardLightingResources.ForwardLightUniformBuffer;
		StochasticShadowsParameters.LightFunctionAtlas = LightFunctionAtlas::BindGlobalParameters(GraphBuilder, View);
		...
		StochasticShadowsParameters.TemporalNeighborhoodClampScale = CVarStochasticShadowsTemporalNeighborhoodClampScale.GetValueOnRenderThread();
	...
	// #ss_todo: merge classification passes or reuse downsampled one to create full res tiles
	...
	// Generate new candidate light samples
	{
		...
		// Clear tiles which don't contain any lights or geometry
		{
			...
			FComputeShaderUtils::AddPass(
				...
				(int32)StochasticShadows::ETileType::Empty * sizeof(FRHIDispatchIndirectParameters));
		}

		for (int32 TileType = 0; TileType < (int32)StochasticShadows::ETileType::SHADING_MAX; ++TileType)
		{
			...
			PermutationVector.Set<FGenerateSamplesCS::FIESProfile>(CVarStochasticShadowsIESProfiles.GetValueOnRenderThread() != 0);
	...
	// Shade light samples
	{
		...
		for (int32 TileType = 0; TileType < (int32)StochasticShadows::ETileType::SHADING_MAX; ++TileType)
		{
			...
			PermutationVector.Set<FShadeLightSamplesCS::FIESProfile>(CVarStochasticShadowsIESProfiles.GetValueOnRenderThread() != 0);
