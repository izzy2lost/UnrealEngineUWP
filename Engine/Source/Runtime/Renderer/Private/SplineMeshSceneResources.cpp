...
class FSplineMeshTextureFillCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_TEXTURE_UAV(RWTexture<float3>, SplineTextureOut)
...
FSplineMeshSceneResources::FSplineMeshSceneResources(FScene& InScene)
	: Scene(InScene)
{
	check(UseSplineMeshSceneResources(GetFeatureLevelShaderPlatform(Scene.GetFeatureLevel())));
}

void FSplineMeshSceneResources::PreSceneUpdate(FRDGBuilder& GraphBuilder, const FScenePreUpdateChangeSet& ChangeSet)
{
	for (FPrimitiveSceneInfo* PrimitiveSceneInfo : ChangeSet.RemovedPrimitiveSceneInfos)
	{
		if (PrimitiveSceneInfo->Proxy->IsSplineMesh())
		{
			Unregister(*PrimitiveSceneInfo);
		}
	}
	...
	SlotAllocator.Consolidate();
}

void FSplineMeshSceneResources::PostSceneUpdate(FRDGBuilder& GraphBuilder, const FScenePostUpdateChangeSet& ChangeSet)
{
	...
	for (FPrimitiveSceneInfo* PrimitiveSceneInfo : ChangeSet.UpdatedPrimitiveSceneInfos)
	{
		if (PrimitiveSceneInfo->Proxy->IsSplineMesh())
		{
			RequestUpdate(RegisteredPrimitives.FindChecked(PrimitiveSceneInfo));
		}
	}

	if (NumRegisteredPrimitives() > 0)
	{
		...
		const uint32 CurSize = SlotAllocator.GetMaxSize();
...
void FSplineMeshSceneResources::Update(FRDGBuilder& GraphBuilder, FSceneUniformBuffer& SceneUniforms)
{
	if (NumRegisteredPrimitives() == 0)
	{
	...
	bool bForceUpdate)
{
	...
	const uint32 NumThreadGroups = NumUpdateRequests ? NumUpdateRequests : SlotAllocator.GetMaxSize();
...
void FSplineMeshSceneResources::Register(const FPrimitiveSceneInfo& PrimitiveSceneInfo)
{	
...
void FSplineMeshSceneResources::Unregister(const FPrimitiveSceneInfo& PrimitiveSceneInfo)
{
...
void FSplineMeshSceneResources::AllocTextureSpace(const FPrimitiveSceneInfo& PrimitiveSceneInfo, uint32 NumSplines, FPrimitiveSlot& OutSlot)
{
	check(NumSplines > 0);
...
uint32 FSplineMeshSceneResources::GetNumSplines(const FPrimitiveSceneInfo& SceneInfo)
{
...
void FSplineMeshSceneResources::AssignCoordinates(const FPrimitiveSceneInfo& SceneInfo, const FPrimitiveSlot& Slot)
{
...
void FSplineMeshSceneResources::AssignCoordinates(TSplineMeshSceneProxy* SceneProxy, const FPrimitiveSlot& Slot)
{
	for (uint32 i = 0; i < Slot.NumSplines; ++i)
	{
...
void FSplineMeshSceneResources::RequestUpdate(const FPrimitiveSlot& Slot)
{
	for (uint32 i = 0; i < Slot.NumSplines; ++i)
	{
...
void FSplineMeshSceneResources::DefragTexture()
{
...
FRDGBufferSRVRef FSplineMeshSceneResources::GetInstanceIdLookupSRV(FRDGBuilder& GraphBuilder, bool bForceUpdate)
{
