...
void TransposeTransforms_SSE(FMatrix3x4* DstTransforms, const FMatrix44f* SrcTransforms, int64 Count)
{
...
void TransposeTransforms_AVX2(FMatrix3x4* DstTransforms, const FMatrix44f* SrcTransforms, int64 Count)
{
...
void FSkinningSceneExtension::FUpdater::FinalizeSkinningUploads(FRDGBuilder& GraphBuilder)
{
	if (SceneData->IsEnabled())
	{
		...
		auto UploadTransformData = [this](const FPrimitiveData& Data)
		{
			...
			const int64 PrefReferenceToLocalCount = int64(PrevReferenceToLocal.Num());
			const FMatrix44f* PrevReferenceToLocalPtr = PrevReferenceToLocal.GetData();

		#if PLATFORM_ALWAYS_HAS_AVX_2
			TransposeTransforms_AVX2(CurrentBoneTransforms, ReferenceToLocalPtr, ReferenceToLocalCount);
			TransposeTransforms_AVX2(PreviousBoneTransforms, PrevReferenceToLocalPtr, PrefReferenceToLocalCount);
		#else
			TransposeTransforms_SSE(CurrentBoneTransforms, ReferenceToLocalPtr, ReferenceToLocalCount);
			TransposeTransforms_SSE(PreviousBoneTransforms, PrevReferenceToLocalPtr, PrefReferenceToLocalCount);
		#endif
		};
