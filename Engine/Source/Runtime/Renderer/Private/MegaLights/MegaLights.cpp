...
class FVolumeGenerateLightSamplesCS : public FGlobalShader
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FNumSamplesPerVoxel1d, FDebugMode, FLightSoftFading>;
...
class FShadeLightSamplesCS : public FGlobalShader
{
	...
	class FDebugMode : SHADER_PERMUTATION_BOOL("DEBUG_MODE");
	using FPermutationDomain = TShaderPermutationDomain<FTileType, FNumSamplesPerPixel1d, FOutputVisibleLightMask, FDebugMode>;
...
void FDeferredShadingSceneRenderer::RenderMegaLights(FRDGBuilder& GraphBuilder, const FSceneTextures& SceneTextures)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < AllViews.Num(); ++ViewIndex)
	{
		...
		const uint32 LightMaskBufferSize = LightMaskSizeInTiles.X * LightMaskSizeInTiles.Y * MegaLights::VisibleLightHashSize;
