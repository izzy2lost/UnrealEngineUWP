...
namespace UE::Interchange
{
	...
	public:
		...
		INTERCHANGEENGINE_API uint64 GetTaskId() const
		{
			return TaskId;
		}

		INTERCHANGEENGINE_API virtual EInterchangeTaskThread GetTaskThread() const
		{
			return EInterchangeTaskThread::GameThread;
		}

		INTERCHANGEENGINE_API const TArray<uint64>& GetPrerequisiteTasks() const
		{
			return PrerequisiteTasks;
		}

		INTERCHANGEENGINE_API EInterchangeTaskStatus GetTaskStatus() const
		{
		 ...
		 */
		INTERCHANGEENGINE_API virtual void Execute() = 0;
		 ...
		 */
		INTERCHANGEENGINE_API void Wait() const;
	protected:

		//Only friend FInterchangeTaskSystem can set a new status
		bool SetTaskStatus(EInterchangeTaskStatus NewTaskStatus);

		void SetPrerequisites(const TArray<uint64>& InPrerequisiteTasks);
	...
	public:
		...
		static INTERCHANGEENGINE_API FInterchangeTaskSystem& Get();
		 ...
		 */
		INTERCHANGEENGINE_API uint64 AddTask(TSharedPtr<FInterchangeTaskBase, ESPMode::ThreadSafe> Task, TArray<uint64>& TaskPrerequisites);
		 ...
		 */
		INTERCHANGEENGINE_API uint64 AddTask(TSharedPtr<FInterchangeTaskBase, ESPMode::ThreadSafe> Task);

		/** Return the current task status */
		INTERCHANGEENGINE_API EInterchangeTaskStatus GetTaskStatus(const uint64 TaskId) const;
		 ...
		 */
		INTERCHANGEENGINE_API void CancelTask(const uint64 TaskId, const bool bCancelPrerequisites);
		 ...
		 * move the wait before you get to a unsafe call stack place to wait.
		 */
		INTERCHANGEENGINE_API void WaitUntilTasksComplete(const TArray<uint64>& TasksToComplete);
