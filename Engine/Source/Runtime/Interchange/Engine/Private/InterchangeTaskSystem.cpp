...
void UE::Interchange::FInterchangeTaskSystem::Tick()
{
	...
	auto UpdateWaitingTask = [this, &DoneTasks](const TArray<uint64>& TaskIds)
		{
	...
	//Update GameThread
	UpdateWaitingTask(WaitingGameThreadTick);

	//Update TaskGraph task
	UpdateWaitingTask(WaitingTaskGraph);
