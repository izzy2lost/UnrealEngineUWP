...
Write-Output "Skipping installing Windows Device Portal Wrapper from Nuget... because it's outdated, and we already have a source version integrated"
...
if ($existingLiveExtSdk -eq $null)
{
    ...
    # Attempting silent install via PowerShell - Add arguments for silent installation if available.
    Write-Output "Attempting to silently install XboxLivePlatformExt.exe..."
    Start-Process -FilePath $installExe -ArgumentList "/quiet", "/norestart" -Wait -NoNewWindow
